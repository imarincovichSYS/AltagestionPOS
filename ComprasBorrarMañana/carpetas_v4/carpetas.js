...
function Cargar_Form_Ingresar(v_num_carpeta){
  $.ajax({
    ...
    success : function(html){
      ...
      $("#tipo_moneda").change(function(){Actualizar_Datos_Generales($(this));});
      ...
      $("#contenedor1").blur(function(){Actualizar_Datos_Generales($(this));});
      $("#contenedor2").blur(function(){Actualizar_Datos_Generales($(this));});
...
function Actualizar_Factura(){
  ...
  if($("#entidad_comercial").val() == "")
    ...
    str_msg_error = "Ingrese monto moneda origen"
  else if($("#num_linea_inicial").val() == "")
    str_msg_error = "Ingrese N° de línea inicial"
  else if(parseFloat($("#num_linea_inicial").val()) == 0)
    str_msg_error = "Ingrese N° de línea inicial"
  else if($("#num_linea_final").val() == "")
    str_msg_error = "Ingrese N° de línea final"
  else if(parseFloat($("#num_linea_final").val()) == 0)
    str_msg_error = "Ingrese N° de línea final"
  if(str_msg_error != "")
  {
  ...
  if(confirm("¿Está seguro que desea actualizar los datos de esta factura?"))
  {
    $.ajax({
      ...
      data      : "accion=update"           +
                  ...
                  "&num_linea_inicial="     + $("#num_linea_inicial").val()         +
                  "&num_linea_final="       + $("#num_linea_final").val(), 
      type      : "post", 
      async     : true,
      dataType  : "json",
      success   : function(data){
