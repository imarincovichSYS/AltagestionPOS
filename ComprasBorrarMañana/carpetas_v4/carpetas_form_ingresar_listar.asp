...
strSQL="select A.numero_linea, A.entidad_comercial, " &_
       ...
       "A.id_tipo_factura, A.numero_factura, A.fecha_factura, A.monto_moneda_origen, A.paridad, A.monto_total_us$, A.monto_final_us$, " &_
       "A.porcentaje_prorrateo, IsNull(A.num_linea_inicial,0) as num_linea_inicial, IsNull(A.num_linea_final,0) as num_linea_final " &_
...
if not rs.EOF then
  ...
  <tr>
    ...
    <tr valign="top" height="16">
      ...
      <td style="width:<%=w_num_linea_ini%>px; text-align:center;"  id='grid_TH_1'>N° item<br>inicial</td>
      <td style="width:<%=w_num_linea_fin%>px; text-align:center;"  id='grid_TH_1'>N° item<br>final</td>
      <td style="width:<%=w_cant_itemes%>px; text-align:center;"    id='grid_TH_1'>Cant.<br>ítemes</td>
    ...
    do while not rs.EOF
      ...
      cantidad_itemes = cdbl(rs("num_linea_final")) - cdbl(rs("num_linea_inicial")) + 1
      %>
    <tr align="center" id="tr_detalle_<%=fila%>" name="tr_detalle_<%=fila%>" style="height:18px;">
      ...
      <td style="width:<%=w_num_linea_ini%>px;" id="grid_TD_1" align="right"><%=FormatNumber(rs("num_linea_inicial"),0)%>&nbsp;</td>
      <td style="width:<%=w_num_linea_fin%>px;" id="grid_TD_1" align="right"><%=FormatNumber(rs("num_linea_final"),0)%>&nbsp;</td>
      <td style="width:<%=w_cant_itemes%>px;" id="grid_TD_1" align="right"><%=FormatNumber(cantidad_itemes,0)%>&nbsp;</td>
