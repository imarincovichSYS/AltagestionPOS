...
if numero_linea <> "" then
  ...
  strSQL="select entidad_comercial, id_tipo_factura, numero_factura, fecha_factura, " &_
         "monto_moneda_origen, paridad, monto_total_us$, monto_final_us$, " &_
         "porcentaje_prorrateo, IsNull(num_linea_inicial,0) as num_linea_inicial, IsNull(num_linea_final,0) as num_linea_final " &_
         "from carpetas_final_detalle " &_
         "where documento_respaldo='" & documento_respaldo & "' and anio_mes='"  &fec_anio_mes & "' " &_
         "and num_carpeta=" & num_carpeta & " and numero_linea = " & numero_linea
  ...
  if not rs.EOF then
    ...
    num_linea_inicial     = trim(rs("num_linea_inicial"))
    num_linea_final       = trim(rs("num_linea_final"))
...
<tr style="height: <%=h_tr%>; color:<%=color%>;">
  ...
  <td><input value="<%=fecha_factura%>" id="fecha_factura" name="fecha_factura" style="font-size:10px; width:80px;" readonly></td>
</tr>
<tr style="height: <%=h_tr%>; color:<%=color%>;">
...
<tr style="height: <%=h_tr%>; color:<%=color%>;">
  <td align="right"><b>N° linea inicial:</b>&nbsp;</td>
  <td><input onkeypress="return Valida_Digito(event);" maxlength=4 value="<%=num_linea_inicial%>" id="num_linea_inicial" name="num_linea_inicial" style="font-size:10px; width:90px; text-align:right; background-color:#EEEEEE;"></td>
</tr>
<tr style="height: <%=h_tr%>; color:<%=color%>;">
  <td align="right"><b>N° linea final:</b>&nbsp;</td>
  <td><input onkeypress="return Valida_Digito(event);" maxlength=4 value="<%=num_linea_final%>" id="num_linea_final" name="num_linea_final" style="font-size:10px; width:90px; text-align:right; background-color:#EEEEEE;"></td>
</tr>
