...
function Mostrar_Form_Carpeta(v_band){
  if(v_band)
  {
	...
	td_cab_6.style.visibility=""	
	td_cab_7.style.visibility=""		
  ...
  else
  {
    ...
    td_cab_6.style.visibility="hidden"
    td_cab_7.style.visibility="hidden"
  }
}
...
function Editar_Carpeta(){
  if(Radio_Es_Arreglo(document.getElementsByName("radio_carpetas")))
  {
    for(i=0;i<=document.getElementsByName("radio_carpetas").length-1;i++)
      if(document.getElementsByName("radio_carpetas")[i].checked==true)
      { 
        v_documento_respaldo  = document.getElementsByName("radio_carpetas")[i].getAttribute("documento_respaldo")
        v_anio                = document.getElementsByName("radio_carpetas")[i].getAttribute("anio")
        v_mes                 = document.getElementsByName("radio_carpetas")[i].getAttribute("mes")
        v_num_carpeta         = document.getElementsByName("radio_carpetas")[i].getAttribute("num_carpeta")
        v_id_embarcador       = document.getElementsByName("radio_carpetas")[i].getAttribute("id_embarcador")
        v_id_origen           = document.getElementsByName("radio_carpetas")[i].getAttribute("id_origen")
        v_id_transporte       = document.getElementsByName("radio_carpetas")[i].getAttribute("id_transporte")
        v_fecha_salida        = document.getElementsByName("radio_carpetas")[i].getAttribute("fecha_salida")
        v_fecha_llegada       = document.getElementsByName("radio_carpetas")[i].getAttribute("fecha_llegada")
        v_manifiesto          = document.getElementsByName("radio_carpetas")[i].getAttribute("manifiesto")
        break;
      }
  }
  else
  {
    v_documento_respaldo  = document.getElementsByName("radio_carpetas").getAttribute("documento_respaldo")
    v_anio                = document.getElementsByName("radio_carpetas").getAttribute("anio")
    v_mes                 = document.getElementsByName("radio_carpetas").getAttribute("mes")
    v_num_carpeta         = document.getElementsByName("radio_carpetas").getAttribute("num_carpeta")
    v_id_embarcador       = document.getElementsByName("radio_carpetas").getAttribute("id_embarcador")
    v_id_origen           = document.getElementsByName("radio_carpetas").getAttribute("id_origen")
    v_id_transporte       = document.getElementsByName("radio_carpetas").getAttribute("id_transporte")
    v_fecha_salida        = document.getElementsByName("radio_carpetas").getAttribute("fecha_salida")
    v_fecha_llegada       = document.getElementsByName("radio_carpetas").getAttribute("fecha_llegada")
    v_manifiesto          = document.getElementsByName("radio_carpetas").getAttribute("manifiesto")
  }
  ...
  fecha_llegada.value               = v_fecha_llegada  
  manifiesto.value                  = v_manifiesto
  ...
  capaDatosCarpeta.style.zIndex           = 1
  fieldset_gastos.style.visibility        = ""
  Mostrar_Form_Carpeta(true)
  Cargar_Gastos("INGRESAR",$("documento_respaldo").value,$("anio").value,$("mes").value,$("num_carpeta").value)
}
...
function Buscar_Carpetas(v_async,v_busqueda_aprox){
  ...
  var ajx = new Ajax("carpetas_listar.asp",{
    ...
    onComplete: function(respuesta){
      ...
      {
        SetChecked_Radio(document.getElementsByName("radio_carpetas"),0)
...
function SaveChanges(cell, c_row, c_col, c_type, c_name, c_scale, c_maxlength, c_null, n_tabla){
  ...
  v_id_gasto = $("celda_" + v_num_grid + "_" + c_row + "_2").value
  document.body.style.cursor="wait"  
  var ajx = new Ajax(RutaProyecto+"compras/carpetas/carpetas_gastos_grabar.asp",{
    method:'post', 
    async:false,
...
function Eliminar_Gastos(){
  ...
  if(confirm("¿Está seguro que desea eliminar estos registros?"))
  {
    ...
    strId_Gasto = ""
    for(i=1;i<=t_rows;i++)  
      if($("celda_" + v_num_grid + "_" + i + "_1").checked==true)
      {
        if(strId_Gasto!="")        
          strId_Gasto=strId_Gasto+delimiter
        strId_Gasto  = strId_Gasto+$("celda_" + v_num_grid + "_" + i + "_2").value
      }
...
function Verificar_Check_Tuplas(){
  ...
  for(k=1;k<=t_rows;k++) 
  {
    if($("celda_" + v_num_grid + "_" + k + "_1").checked==true)
      return true;
  }
  return false;
}
...
function Delete_File(c_row, c_col, c_name, c_nom_file){
  ...
  {
    id_gasto_tmp  = $("celda_" + v_num_grid + "_" + c_row + "_2").value
    url_tmp = RutaProyecto+"compras/carpetas/carpetas_archivo_eliminar.asp"
...
function Set_Load_File(cell, c_row, c_col, c_name){
  v_num_grid = 1
  id_gasto_tmp = $("celda_" + v_num_grid + "_" + c_row + "_2").value
  frameArchivo.document.location.href=RutaProyecto+"compras/carpetas/carpetas_archivo.asp?documento_respaldo="+documento_respaldo.value+"&anio="+anio.value+"&mes="+mes.value+
