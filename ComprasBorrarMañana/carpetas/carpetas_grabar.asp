...
if accion = "insertar" then
  ...
  strSQL="insert into "&nom_tabla&"(documento_respaldo, anio_mes,num_carpeta, carpeta) " &_
         "values('"&documento_respaldo&"','"&fec_anio_mes&"',"&nuevo_num_carpeta&",'"&documento_respaldo&"-"&anio&"-"&mes&"-"&nuevo_num_carpeta&"')"
  Response.Write nuevo_num_carpeta
elseif accion = "actualizar" then
  if nom_campo="fecha_salida" or nom_campo="fecha_llegada" then valor = year(cdate(valor)) & "/" & month(cdate(valor)) & "/" & day(cdate(valor))
