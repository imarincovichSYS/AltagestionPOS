...
IF documento_respaldo = "TU" THEN
  strSQL = "select num_carpeta, id_embarcador, id_transporte, id_origen, fecha_salida, fecha_llegada, manifiesto, '' numero_documento_respaldo " &_
...
elseif   documento_respaldo = "R" then
  strSQL = "select num_carpeta, id_embarcador, id_transporte, id_origen, " &_
           ...
           "manifiesto, ' ' as numero_documento_respaldo " &_
           "from carpetas_final CAR where CAR.documento_respaldo='"&documento_respaldo&"' and " &_
           "CAR.anio_mes = '"&fec_anio_mes&"' " & strWhere   &_
           " order by CAR.num_carpeta"
...
ELSE
  strSQL ="select distinct num_carpeta, id_embarcador, id_transporte, id_origen, fecha_salida = isnull(fecha_salida,0), fecha_llegada, manifiesto, A.carpeta, " &_ 
	        "A.documento_respaldo, B.carpeta, B.documento_no_valorizado, B.numero_documento_respaldo " &_
          "from " &_
...
if not rs.EOF then
  ...
  <tr>
    ...
    style="width:<%=width%>; height: 450px; border-left: 1px solid #CCC; border-bottom: 1px solid #CCC; border-right: 1px solid #CCC; overflow: auto;">
