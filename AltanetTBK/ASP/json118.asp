...
Class aspJSON
	...
	Public Function JSONoutput(withVbCrLf)
		...
		if not withVbCrLf then JSONoutput = replace(JSONoutput,Chr(13) & Chr(10),"")
		...
		For Each aj_item In objDict
			Select Case TypeName(objDict.Item(aj_item))
				Case "Dictionary"
					...
					GetDict = GetDict & Space(JSONoutput_level * 1)
					...
					GetDict = GetDict & (GetSubDict(objDict.Item(aj_item)) & Space(JSONoutput_level * 0) & Right(aj_dicttype,1) & aj_InlineIf(aj_item = aj_keyvals(objDict.Count - 1),"" , ",") & Chr(13) & Chr(10))
				Case Else
					aj_keyvals =  objDict.Keys
					...
					GetDict = GetDict & (Space(JSONoutput_level * 0) & aj_InlineIf(aj_IsInt(aj_item), "", """" & aj_JSONEncode(aj_item) & """: ") & WriteValue(objDict.Item(aj_item)) & aj_InlineIf(aj_item = aj_keyvals(objDict.Count - 1),"" , ",") & Chr(13) & Chr(10))
			End Select
		Next
	End Function
