...
function CentrarCapa(id)
{
	...
	an=parseInt(document.getElementById(id).style.width);
...
//Verifica que un campo de entrada no esté vacio
function CampoVacio(id,rutaProy)
{
  if(id.value=="")
  {
    ShowMessage("","        Debe llenar el campo!",3,rutaProy)
    id_campo=id
    return true
  }
  else
...
{
	var key = Navegador_No_IE ? evt.which : evt.keyCode;
	if(key!=8)
...
function Valida_AlfaNumerico(e){
  ...
  {
    x_codtecla = e.which
    if(parseInt(x_codtecla) == 8 || parseInt(x_codtecla) == 0) //Teclas de control (flechas y backspace)
      return true;
  }
  else
  {
    x_codtecla = e.keyCode
    if(parseInt(x_codtecla) == 175 || parseInt(x_codtecla) == 37 || parseInt(x_codtecla) == 38 || parseInt(x_codtecla) == 39 || parseInt(x_codtecla) == 40)  //Teclas de control (flechas y backspace)
      return true;
  }
  regexp = /\w/;
  if(!regexp.test(String.fromCharCode(x_codtecla)))
    return false;
  else
    return true;
}
...
function Get_Navegador(){
  var str_navegador = ""
  str_navegador = window.navigator.userAgent.toUpperCase()
  if(str_navegador.indexOf("CHROME") != -1)
    navegador = "CHROME"
  else if(str_navegador.indexOf("FIREFOX") != -1)
    navegador = "FIREFOX"
  else
    navegador = "IE"
  return navegador;
