...
<%
	...
	cSql = "Exec DOC_ListaDocumentos '" & Documento & "', '" & Glosa & "'"
		...
		Do While Not Rs.eof %>
			<tr>
				<td class="FuenteInput" width=25% align=left>
					<a name ='Link<%=i%>' href="JavaScript:ClipBoard( 'Listado', 'Link<%=i%>', '', 'c');window.blur()"><DIV ID="Codigo<%=i%>" ><%=Rs("Documento")%></DIV></a>
				</td>
				<td class="FuenteInput" width=25%>
					<a Id='dLink<%=i%>' href="JavaScript:ClipBoard( 'Listado', 'Link<%=i%>', '', 'c');window.blur()"><DIV ID="Glosa<%=i%>"><%=Rs("Nombre")%></DIV></a>
				</td>
			</tr>
<%			Rs.MoveNext
			i = i + 1
