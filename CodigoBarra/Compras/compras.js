...
// function Eliminar_Limpiar_Items()
...
function Terminar_Compra(){
  if(confirm("¿Está seguro que desea finalizar la compra?.\n(Traspasar a RCP)"))
  {
    ...
    {
      Config_Msg_Accion("ADVERTENCIA: "+bot_terminar_compra.advertencia+"...",3000,msg_accion_3)
    ...
    onComplete: function(respuesta){
      ...
      if(v_nom_campo=="numero_documento_respaldo")
      {
        if(respuesta=="EXISTE")
        {
          ...
          Config_Msg_Accion("YA EXISTE EL N° ("+numero_documento_respaldo.value+") EN OTRO DOCUMENTO ("+documento_respaldo.value+") DEL AÑO ("+anio.value+")...",3000,msg_accion_3)
          numero_documento_respaldo.focus()
        }
        else
        {
          ...
          Config_Msg_Accion("N° ADUANA ACTUALIZADO...",3000,msg_accion_3)
        }
      }
      else if(v_nom_campo=="proveedor")
      {
        ...
        if(label_proveedor_cabecera.innerText=="")
          Config_Msg_Accion("PROVEEDOR INICIAL INGRESADO...",3000,msg_accion_3)
        else
          Config_Msg_Accion("PROVEEDOR INICIAL ACTUALIZADO...",3000,msg_accion_3)
      ...
      else if(v_nom_campo=="proveedor_2")
      {
        if(v_valor!="")
        {
          ...
          Config_Msg_Accion("PROVEEDOR SECUNDARIO ASIGNADO...",3000,msg_accion_3)
        }
        else
        {
          ...
          Config_Msg_Accion("PROVEEDOR SECUNDARIO ELIMINADO...",3000,msg_accion_3)
        }
      ...
      else if(v_nom_campo=="carpeta")
      {
        ...
        Config_Msg_Accion("CARPETA ASIGNADA SATISFACTORIAMENTE...",3000,msg_accion_3)
      }
...
function Asignar_Carpeta(){
  ...
  {
    Config_Msg_Accion("INGRESE FECHA ADUANA...",3000,msg_accion_3)
    carpeta.value = carpeta.old_value
    return;
  }
  else if(fecha_recepcion.value=="")
  {
    Config_Msg_Accion("INGRESE FECHA RECEPCION...",3000,msg_accion_3)
    carpeta.value = carpeta.old_value
    return;
  }
  else if(parseFloat(monto_neto_US$.value)==0)
  {
    Config_Msg_Accion("INGRESE TOTAL CIF ORI...",3000,msg_accion_3)
    carpeta.value = carpeta.old_value
    return;
  }
  else if(parseFloat(monto_adu_US$.value)==0)
  {
    Config_Msg_Accion("INGRESE TOTAL CIF ADU...",3000,msg_accion_3)
    carpeta.value = carpeta.old_value
    return;
  }
  else if(parseFloat(label_paridad_para_facturacion.innerText) <= 1)
  {
    Config_Msg_Accion("PARIDAD INCORRECTA...",3000,msg_accion_3)
    carpeta.value = carpeta.old_value
    return;
  }
  ...
  var ajx = new Ajax("compras_existe.asp",{
    ...
    onComplete: function(respuesta){
      ...
      if(respuesta=="EXISTE")
      {
        ...
        Editar_Compra()
        Config_Msg_Accion("COMPRA EXISTENTE...",3000,msg_accion_3)
      }
      else
      {
        if(confirm("¿Está seguro que desea grabar esta compra?"))
        {
          ...
          var ajx1 = new Ajax("compras_grabar.asp",{
            ...
            onComplete: function(respuesta){
              ...
              Config_Msg_Accion("COMPRA GRABADA SATISFACTORIAMENTE...",3000,msg_accion_3)
            }
          });
          ajx1.request();
        }
...
function Editar_Compra(){
  ...
  monto_neto_US$.disabled               = true
  monto_neto_US$.style.backgroundColor  = "#EEEEEE"
  if(documento_respaldo.value=="TU" && v_documento_no_valorizado=="TCP")
  {
    monto_neto_US$.disabled               = false
    monto_neto_US$.style.backgroundColor  = ""
  }
...
function Actualizar_Proveedor_Cabecera(){
  ...
  {
    Config_Msg_Accion("EL PROVEEDOR "+codprov_proveedor.value+" YA ESTA INGRESADO EN LA CABECERA DE LA COMPRA...",3000,msg_accion_3)
    return;
  }
  else if(entidad_comercial_2.value==entidad_comercial.value)
  {
    Config_Msg_Accion("EL PROVEEDOR "+codprov_proveedor.value+" ESTA ASIGNADO COMO SEGUNDO PROVEEDOR...",3000,msg_accion_3)
    return;
  }
  else
  {
...
function Grabar_Segundo_Proveedor(v_accion){
  if(v_accion=="asignar")
  {
    ...
    {
      Config_Msg_Accion("EL PROVEEDOR "+codprov_proveedor.value+" YA ESTA ASIGNADO COMO SEGUNDO PROVEEDOR...",3000,msg_accion_3)
      return;
    }
...
function Desagrupar_Items(){
  ...
  if(strNum_Lineas=="")
  {
    Config_Msg_Accion("NO HA SELECCIONADO ITEMES AGRUPADOS...",3000,msg_accion_3)
    return;
  }
  if(confirm("¿Está seguro que desea desagrupar estos ítemes?"))
  {
    ...
    var ajx = new Ajax("compras_productos_grabar.asp",{
      ...
      onComplete: function(respuesta){
        //alert(respuesta)
        Config_Msg_Accion("ITEMES DESAGRUPADOS...",3000,msg_accion_3)
        Cargar_Items("EDITAR","")
      }
    });
    ajx.request();
...
function Agrupar_Items(v_item){
    ...
    if(eval("document.all.celda_" + i + "_" + C_CHECKBOX).checked==true && eval("document.all.celda_" + i + "_" + C_ITEMS_PADRE).value=="")
    {
      ...
      for(j=1;j<=t_rows;j++) 
      {
        if(item_tmp==eval("document.all.celda_" + j + "_" + C_ITEMS_PADRE).value)
        {
          ...
          Config_Msg_Accion("NO SE PUEDE AGRUPAR EL ITEM "+item_tmp+"...",3000,msg_accion_3)
          eval("document.all.celda_" + i + "_" + C_CHECKBOX).checked=false
          items_grupos.value = ""
          return;
        }
  ...
  if(strNum_Lineas=="")
  {
    Config_Msg_Accion("NO HA SELECCIONADO ITEMES...",3000,msg_accion_3)
    items_grupos.value = ""
    return;
  }
  if(confirm("¿Está seguro que desea agrupar estos ítemes en el item "+v_item+"?"))
  {
    ...
    var ajx = new Ajax("compras_productos_grabar.asp",{
      ...
      onComplete: function(respuesta){
        //alert(respuesta)
        Config_Msg_Accion("ITEMES AGRUPADOS...",3000,msg_accion_3)
        Cargar_Items("EDITAR","")
      }
    });
    ajx.request();
  ...
  if(confirm("¿Está seguro que desea actualizar el proveedor de los ítemes seleccionados?"))
  {
    ...
    var ajx = new Ajax("compras_productos_grabar.asp",{
      ...
      onComplete: function(respuesta){
        ...
        Set_Cancelar_Proveedor()
        Cargar_Items("EDITAR","")
        Config_Msg_Accion("ITEMES ACTUALIZADOS...",3000,msg_accion_3)
      }
    });
    ajx.request();
  }
...
function Get_Totales_Costos_Chequea_Items_Para_Pasar_Compra_a_RCP(){
  ...
  for(k=1;k<=t_rows;k++) 
  {
    ...
    if(v_BAND_ITEMS_OK_TO_RCP == true)
    {
      ...
      else if(documento_respaldo.value=="R" && v_volumen == 0)
      {
...
function SaveChanges(cell, c_row, c_col, c_type, c_name, c_scale, c_maxlength, c_null){
  ...
  if(c_name=="unidad_de_medida_compra" || c_name=="cantidad_mercancias" || c_name=="cantidad_um_compra_en_caja_envase_compra" || c_name=="unidad_de_medida_consumo" || c_name=="cantidad_x_un_consumo")
  {
    ...
    /*if(v_cant_m==0 || v_cant_x_m==0 || v_cant_x_un==0)
    {
      ...
      Config_Msg_Accion("LA CANTIDADES DEBEN SER MAYOR QUE 0...",3000,msg_accion_3)
      cell.value = cell.getAttribute("o_value");
      cell.focus();
      return;
    }*/
  ...
  var ajx = new Ajax(RutaProyecto+"compras/"+v_url,{
    ...
    '&tipo_dato='+c_type+
    '&nom_campo='+escape(c_name)+'&valor='+encodeURIComponent(c_value)+
    ...
    onComplete: function(respuesta){
      ...
      {//Movimientos productos
        ...
        else
        {
          ...
          /*if(band_cpa_actualizado)
            Get_CPA_Producto_Anterior_a_Fecha(v_producto,fecha_recepcion.value)
          else
            Config_Msg_Accion("DATOS ACTUALIZADOS...",3000,msg_accion_3)
          */
          Config_Msg_Accion("DATOS ACTUALIZADOS...",3000,msg_accion_3)
