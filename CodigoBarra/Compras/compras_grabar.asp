...
if accion = "insertar" then
  ...
  strSQL="insert into "&nom_tabla&"(Bodega, Documento_no_valorizado, Documento_respaldo, " &_
         "Empleado_responsable, Empresa, Estado_documento_no_valorizado, Fecha_emision, Numero_recepcion_de_compra, " &_
         "Numero_documento_no_valorizado, Numero_documento_respaldo, Observaciones_generales, paridad_conversion_a_dolar, Fecha_recepcion) " &_
         "values(" &_
         "'0010'," &_
...
elseif accion = "actualizar" then
  ...
  strSetMP  = ""
  if nom_campo="fecha_recepcion" or nom_campo="bodega" or nom_campo="paridad_conversion_a_dolar" or nom_campo="proveedor" or nom_campo="proveedor_2" or nom_campo = "numero_documento_respaldo" then 
    ...
    elseif nom_campo="paridad_conversion_a_dolar" then
      ...
      strSetMP = strSetMP & ", Costo_CIF_ADU_$_al_momento_de_la_compra=Round(Costo_CIF_ADU_US$ * " & valor & ",7) " 'Costo_CIF_ADU_$_al_momento_de_la_compra
      strSetMP = strSetMP & ", Costo_CIF_ORI_$=Round(Costo_CIF_ORI_US$ * " & valor & ",7) " 'Costo_CIF_ORI_$ 
      strSetMP = strSetMP & ", Costo_EX_FAB_$=Round(Costo_CIF_ORI_US$ * " & valor & ",7) " 'Costo_EX_FAB_$
      strSetMP = strSetMP & ", Costo_FOB_$=Round(Costo_CIF_ORI_US$ * " & valor & ",7) " 'Costo_FOB_$
    ...
    elseif nom_campo = "numero_documento_respaldo" then 'Se cambió el numero_documento_respaldo (numero de la compra = n° aduana)
      ...
      strSQL="select documento_respaldo from documentos_no_valorizados where empresa='SYS' and year(fecha_recepcion) = '"&anio&"' and " &_
  ...
  if nom_campo="monto_adu_US$" and documento_respaldo <> "TU" then strSet = strSet & ", monto_neto_US$="&valor
