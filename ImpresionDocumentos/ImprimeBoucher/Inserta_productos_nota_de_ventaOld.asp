...
if cdbl(rs("cantidad")) + cdbl(cantidad) < 0 then
  ...
  ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
  ''''''''''''''''''''''''''''''''OBTENGO EL PRECIO, PUEDE SER QUE CLIENTE SEA MAYORISTA''''''''''''''''''''''''''''''''''
  ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
  'sql =  "SELECT Lista_de_precios = isnull(Lista_de_precios,0) FROM VIGENCIAS_DE_LISTAS_DE_PRECIOS WHERE ENTIDAD_COMERCIAL = '" & cliente & "'"
  'set rs = Conn.Execute(sql)
  'if not rs.eof then
  ''    sql = "select A.producto, A.valor_unitario, prom = ISNULL(A.valor_unitario - Monto_descuento,0), PRECIO_MAY = ISNULL(C.Valor_unitario,0), CANTIDAD_MINIMA = ISNULL(Limite_precio,0) from " &_ 
  ...
  ''    
  ''          "(select Producto, Monto_descuento, Promocion from productos_en_promociones where empresa='SYS' and Centro_de_venta='0011' and producto='" & codigo & "') B, " &_
  ''    
  ''          "(select producto, valor_unitario,Limite_precio from productos_en_listas_de_precios where empresa='SYS' and Lista_de_precios = 'LXMAY' and Limite_precio>0 " &_
  ''          "and producto='" & codigo & "') C " &_
  ''          
  ''          "where A.producto *= B.producto " &_
  ''    	    "and B.promocion in (select promocion from promociones where fecha_termino+Hora_termino>getdate()) " &_
  ''    	    "and A.producto *= C.producto "
  ''    set rs = Conn.Execute(sql)
  ''    if cdbl(rs("PRECIO_MAY")) = 0 then
  ...
  if rs_kits.eof then
    ...
    sql = "select A.producto, A.valor_unitario, prom = ISNULL(A.valor_unitario - Monto_descuento,0) from " &_ 
          ...
          "where A.producto *= B.producto " &_
          "and B.promocion in (select promocion from promociones where fecha_termino+Hora_termino>getdate())"
    set rs = Conn.Execute(sql)
    if cdbl(rs("PROM")) = 0 then
      precio = rs("valor_unitario")
    else
  ...
  strSQL="select A.producto,  A.Superfamilia, A.Familia, A.Subfamilia, Genero= Isnull(A.Genero,''), Marca = Isnull((Case when A.Marca = 'OCASIONAL' THEN '' Else A.Marca end),'') ,A.nombre, B.codigo_alternativo_producto, C.valor_unitario, F.liq, " &_
         ...
         "(select Producto, Monto_descuento, Promocion from productos_en_promociones where empresa='SYS' and Centro_de_venta='0011') E, " &_
         
         "(select producto, isnull(sum(cantidad_entrada),0) entrada from movimientos_productos " &_
         "where empresa='SYS' and documento_no_valorizado='TBE' and fecha_movimiento >= getdate()-2 " &_
  	     "and Estado_documento_no_valorizado = 'PRE' group by producto) G " &_
         "where A.producto=B.producto and " &_ 
               "A.producto=C.producto and " &_
               "a.producto = d.producto and  " &_
               "a.producto = F.producto and  " &_
               "E.promocion in (select promocion from promociones where fecha_termino+Hora_termino>getdate()) and " &_
               "a.producto = d1.producto and " &_
               "a.producto = d2.producto and " &_
               "a.producto *= E.producto and " &_ 
               "a.producto *= G.producto"
