...
<table style="font-family:'Times New Roman',Georgia,Serif; font-size:12px;">
	<tr>
		<td>Rubro: </td>
		<td>
		...
		<td style="width:20px;"><input OnMouseOver="$(this).css('cursor','pointer');" type="checkbox" id="check_solo_promocion" name="check_solo_promocion"></td>
		<td style="font-size:11px;"><b><label title="Listar sólo productos en promoción" OnMouseOver="$(this).css('cursor','pointer');"
		onclick="javascript:
			if($('#check_solo_promocion').attr('checked'))
				$('#check_solo_promocion').attr('checked',false);
			else
				$('#check_solo_promocion').attr('checked',true);
			">Sólo promoción</label></b></td>
	</tr>
</table>