	...
	<script type="text/javascript">
		...
		function Cargar_Familia_X_Superfamilia(x_superfamilia){
			...
			$.ajax({
				...
				data: "superfamilia="+escape(x_superfamilia), //variable que le entrego
				type: "POST", //metodo
				async: true,
		...
		function Cargar_Subfamilia_X_Familia_X_Superfamilia(x_superfamilia, x_familia){
			...
			$.ajax({
				url: "subfamilias.asp",
				data: "superfamilia="+escape(x_superfamilia)+"&familia="+escape(x_familia),
				type: "POST",
				async: true,
				dataType: "html"
			}).done(function(data){
					...
					if($("#imprimir_"+i).attr("checked")){
						...
						for(k=1;k<=cantidad_imprimir;k++){
							if(tipo_precio=="fleje")
								zebrafleje(precio_venta, cod_producto, desc_prod, oferta);
							if(tipo_precio=="gancho")
								zebragancho(precio_venta, cod_producto, desc_prod, oferta);
							if(tipo_precio=="grande")
								zebragrande(precio_venta, cod_producto, desc_prod, oferta);
						}
					}			
				}
				if($("#oferta_"+i).attr("checked")){
					var tipo_precio, cantidad_imprimir, oferta;
					cantidad_imprimir	=$("#cantidad_"+i).val();
					tipo_precio			=$("input:radio[name=radio_tipo_precio_"+i+"]:checked'").val();
					...
					if($("#oferta_"+i).attr("checked")){
						if(radio=="rojo" || radio=="verde"){
							oferta="si";
						}
						for(j=1;j<=cantidad_imprimir;j++){
							if(tipo_precio=="fleje")
								zebrafleje(precio_promocion, cod_producto, desc_prod, oferta);
							if(tipo_precio=="gancho")
								zebragancho(precio_promocion, cod_producto, desc_prod, oferta);
							if(tipo_precio=="grande")
								zebragrande(precio_promocion, cod_producto, desc_prod, oferta);
						}
					}
				}
			}	
		}
		function zebrafleje(precio, cod_producto, desc_prod, oferta){
			nom_impresora = "ZebraFleje";
		...
		function zebragancho(precio, cod_producto, desc_prod, oferta){
			nom_impresora = "ZebraGancho";
			tamagno=precio.length;
			precio2 = precio.replace(",",".");
			precio3 = precio2.replace(",",".");
			if(oferta=="si"){
		...
		function zebragrande(precio, cod_producto, desc_prod, oferta){
			nom_impresora = "ZebraGrande";
			tamagno=precio.length;
			precio2 = precio.replace(",",".");
			precio3 = precio2.replace(",",".");
			if(oferta=="si"){
