...
ElseIf tipo_busqueda = "ultimocambio" Then
	...
	strSQL="select A.producto, C.nombre, A.valor_unitario as precio, D.codigo_postal as proveedor from " &_
		   ...
		   "(select producto, nombre, proveedor from productos where empresa='SYS') C, " &_
           ...
           "where A.producto=B.producto and A.producto=C.producto and C.proveedor=D.entidad_comercial order by A.producto"
 ElseIf tipo_busqueda= "numeroguia" Then
	numeroguia = Request.Form("numeroguia")
	strSQL="select A.producto, A.nombre, B.precio, C.codigo_postal as proveedor from " &_
	               "(select producto, nombre from productos) A,"&_
...
<table class="table_cabecera" style="width:<%=w_table%>px;">
	<tr align="center">
		...
		<td rowspan="2" style="width: <%=w_precio_promocion%>px;">PRECIO PROMOCION</td>
	</tr>
	<tr align="center">
		<td><input type="checkbox" name="todos_imprimir" value="todos_imprimir" onclick="selectAllcheck(1);"></td>
		<td align="center"><input OnKeyPress="return Valida_Digito(event)" type="text" name="todos_cantidad" class="todos_cantidad" id="todos_cantidad" style="width:24px;" maxlength=1></td>
