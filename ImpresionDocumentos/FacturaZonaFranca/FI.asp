...
'Response.write( cSql_C & "<br>" )
	...
	Set Rss = Conn.execute( "select Paridad_para_facturacion from Paridades (nolock) where Moneda = 'US$' and convert( char(10) , Fecha_valor , 112) = '" & Ano & Mes & Dia & "'" )
	if not Rss.eof then
   		Valor_paridad_moneda_oficial_SYS = cdbl( Rss("Paridad_para_facturacion") )
	end if
    ...
    Factura = Factura & space(35) & "FACTURA DE IMPORTACION      FI         " & Fecha & Crlf  'space(25)
   ...
   Set Rs = Conn.Execute ( cSql )
    ...
    Factura = Factura & space(25) & padR(Persona_que_presenta_OrdenVtaSUP,26) & space(8) & "S/C" & Crlf  'space(5)
    ...
    Factura = Factura & space(5) & PadR( trim(Direccion) & "," & Nombre_Comuna,40) & "  " & Rut_cliente  & space(7) & " GARANTIA N470 AÑO 2018" & Crlf & Crlf
...
'Response.write( cSql_D & "<br>" )
  ...
  valor_movimiento_pesos = 0
	Do While Not RsD.Eof
	 ...
	 valor_movimiento_pesos = cdbl(valor_movimiento_pesos) + cdbl( RsD("valor_movimiento") )
  ...
  'total_dolares_en_pesos = cdbl(Total_dolares) * cdbl(Valor_paridad_moneda_oficial_SYS)
  'Factura = Factura & space(25) & "MONTO TOTAL $ " & Formato(total_dolares_en_pesos,0)
   Factura = Factura & space(25) & "MONTO TOTAL $ " & Formato(valor_movimiento_pesos,0)
