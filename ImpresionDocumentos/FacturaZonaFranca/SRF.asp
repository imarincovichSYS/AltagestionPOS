...
'Response.write( cSql_C & "<br>" )
    ...
    Persona_que_presenta_OrdenVtaSUP = cambia_chr( RsC("Persona_que_presenta_documento_venta") )
  	...
  	Margen_izquierdo =  7 'Margen izquierdo ?
    Marca_impresora = "IBM"
    Crlf = chr(13)
    SaltoPagina = chr(12)
    ...
    Factura = Factura & space(25) & "SOLICITUD DE REGISTRO FACTURA        SRF         " & Fecha & Crlf
   ...
   Set Rs = Conn.Execute ( cSql )
    ...
    'Tasa de impuesto aduanero
    TASAIMPADU = 0.00
    'cSql = "Exec PAR_ListaParametros 'TASAIMPADU'"
    'Set Rs = Conn.Execute ( cSql )
    'If Not Rs.Eof then
    '    TASAIMPADU = Rs("Valor_numerico")
    'end if
    'Rs.Close
    'Set Rs = Nothing

    Factura = Factura & space(25) & padR(Aduana_presentacion_OrdenVtaSUP,26) & space(8) & Codigo_aduana_presentacion_OrdenVtaSUP & Crlf & Crlf
    Factura = Factura & space(25) & padR(Persona_que_presenta_OrdenVtaSUP,26) & space(8) & "S/C" & Crlf
    ...
    Factura = Factura & space(5) & PadR( trim(Direccion) & "," & Nombre_Comuna,40) & "  " & Rut_cliente & space(7) & " GARANTIA N470 AÑO 2021" & Crlf & Crlf

    'Factura = Factura & PadR(DireccionBodega,25) & Crlf
    'Factura = Factura & PadR(NombreBodega,25) & space(5) & PadR(Bodega,3) & space(5)
    Factura = Factura & "  MANZANA 7 Z.FRANCA" & Crlf
...
'Response.write( cSql_D & "<br>" )
  ...
  Total_cif_US = 0
	Do While Not RsD.Eof
     ...
     Precio = cdbl( RsD("Precio_final_con_descuentos_impreso") )
     Total = Precio * Cantidad
     Total_pesos = Total_pesos + cdbl( RsD("Total") )
	   Factura = Factura & strzero(n,2) & "  " & _
               FormatNumber(PadL(Cantidad,8),2) & "       " & _
               ...
               comprimida &PadR( RsD("Tipo_documento_de_compra") & "" & _
                     ...
                     RsD("Numero_de_linea_en_RCP_o_documento_de_compra") , 73 ) & _
               PadL( Formato(Precio,0) , 10 ) & _
               PadL( Formato(Total,0) , 12 ) & CrLf
     Factura = Factura & space(21)
               '"CIF Uni US$  " & Formato(RsD("Costo_cif_ori_US$"),4) & _
               '"   CIF TOTAL US$ " & Formato( Cif ,2) & CrLf
     n = n + 1
     nl = nl + 1   'Este 3 es porque se imprimen 3 lineas por item

     TASAIMPADU = cdbl(RsD("Tasa_impuesto_aduanero"))
     if isNull( TASAIMPADU ) then TASAIMPADU = 0.00
     Total_impuesto_US = Total_impuesto_US + (Cif * TASAIMPADU / 100 )
  ...
  For n = nl to 30 'Aquí dejar en 30 para hoja real
      Factura = Factura & Crlf
  Next

  'Resumen Impuestos
  Factura = Factura  & CrLf & _
            space(40) & "IMPUESTO " & Formato(TASAIMPADU,2) & "% US$: " & Formato(Total_impuesto_US,2) & CrLf & _
            space(40) & "T/C AL " & Fecha & " $ " & Formato(Tipo_cambio,2) & CrLf & _
            space(40) & "IMPUESTO " & Formato(TASAIMPADU,2) & "% M/N: $ " & Formato(Total_impuesto_pesos,2) & CrLf & CrLf & CrLf

  'Totales finales
  Factura = Factura & space( 85 ) & _
            PadL( Formato(Total_impuesto_US,2) , 22 ) & _
            PadL( Formato(Total_pesos,0) , 22 ) & CrLf & CrLf & CrLf & CrLf & CrLf & CrLf
  Factura = Factura & space( 97 ) & _
            PadL( Formato(Total_pesos,0) , 32 )
