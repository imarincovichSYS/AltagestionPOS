...
<%
  ...
  if session("IP") = "192.168.0.215" then
    cSql = "Exec PRO_Lista_Imprime_Precios_L01 '" & SuperFamilia & "', '" & Familia & "', '" & SubFamilia & "', '" & Descripcion & "', '" & NULL & "', '" & NULL & "', '" & "', '" & NULL & "', '" & NULL & "', '" & Bodega & "', '" & Proveedor & "', '" & Productos_Certificados & "','" & Producto & "',0" & Guia 
  else
	  cSql = "Exec PRO_Lista_Imprime_Precios_L01 '" & SuperFamilia & "', '" & Familia & "', '" & SubFamilia & "', '" & Descripcion & "', '" & NULL & "', '" & NULL & "', '" & "', '" & NULL & "', '" & NULL & "', '" & Bodega & "', '" & Proveedor & "', '" & Productos_Certificados & "','" & Producto & "',0" & Guia 
	  'cSql = "Exec PRO_Lista_Imprime_Precios '" & SuperFamilia & "', '" & Familia & "', '" & SubFamilia & "', '" & Descripcion & "', '" & NULL & "', '" & NULL & "', '" & "', '" & NULL & "', '" & NULL & "', '" & Bodega & "', '" & Proveedor & "', '" & Productos_Certificados & "','" & Producto & "'" 
  end if
	...
	<table width=60% border=1 align=center cellpadding=2 cellspacing=0><thead>
		<tr class="FuenteCabeceraTabla">
		<td align=left width=5%><b>Imp.</b></td>
		<td align=left width=5%><b>Cant.</b></td>
		<td align=left width=5%><b>F</b></td>
		<td align=left width=5%><b>G</b></td>
		<td align=left width=5%><b>Of.</b></td>
...
<%		i = 1
			...
			<tr>
			  ...
			  <input class=FuenteInput type=checkbox name="Ajuste_<%=i%>" width=5%>
				...
				<td class=FuenteEncabezados align=left width=5%>
					<a><input class=DatoInPut type=text value="1" name="cant_<%=i%>" size=2 maxlength=2 onchange="validarSiNumero(this.value);"></a>
				</td>			  
        <td>
			  <input class=FuenteInput type=Radio name="fleje_<%=i%>" width=5% value=Fleje checked>
			  </td>
			  <td>
			  <input class=FuenteInput type=Radio name="fleje_<%=i%>" width=5% Value=Gancho>
			  </td>				
			  <td>
        ...
        <td class="FuenteInput" align=right width=5%>
          ...
          <input type=hidden value="<%=Rs("CMay")%>" name="CMay_<%=i%>" >
    		<td class="FuenteInput" align=left width=5%>
			  <input class=FuenteEncabezados  width=5% value="<%=formatNumber(Rs("pprom"),0)%>" name="Prom_<%=i%>" readonly>
			  </td>          
		</tr>			
<%			Rs.MoveNext
			End if
