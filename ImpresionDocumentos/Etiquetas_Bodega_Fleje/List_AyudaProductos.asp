...
<%
  ...
  Mostrar_TCP_o_RCP = Request("Mostrar_TCP_o_RCP") 
  periodo = Request("periodo")
  
  
  
	  cSql = "Exec PRO_Lista_Imprime_Etiquetas_Bodega '" & SuperFamilia & "', '" & Familia & "', '" & SubFamilia & "', '" & Descripcion & "', '" & NULL & "', '" & NULL & "', '" & "', '" & NULL & "', '" & NULL & "', '" & Bodega & "', '" & Proveedor & "', '" & Productos_Certificados & "','" & Producto & "',0" & Guia & ",0" & nprecio & ", " & periodo & ", '" & Mostrar_TCP_o_RCP & "'"
	...
	<table  width=40% border=1 align=center cellpadding=2 cellspacing=0><thead>
		...
		<td align=center width=40%><b> <%=date()%> </b></td>
	  <!--<td align=center><b> <%=Bodega%></b></td>-->
	  <!--<td align=center><b> Stock </b></td>-->
	  <td align=left width=10%><b>Prov</b></td>
	  </tr>
...
<%		i = 1
        ...
        <td align=left>
				...
				<!-- <td class=FuenteEncabezados align=left width=1%>
					<input  class=FuenteEncabezados type=hidden value="<%=trim(Rs("Superfamilia"))%>" name="Superfamilia_<%=i%>">
				</td> -->
				<td class="FuenteInput" width=40%>					
        ...
        <td class="FuenteInput" align=left width=5%>
				...
				<input  class=FuenteEncabezados type=hidden value="<%=trim(Rs("Superfamilia"))%>" name="Superfamilia_<%=i%>">
				<input  class=FuenteEncabezados type=hidden value="<%=trim(Rs("familia"))%>" name="familia_<%=i%>">
				<input  class=FuenteEncabezados type=hidden value="<%=trim(Rs("Subfamilia"))%>" name="Subfamilia_<%=i%>">
				<input  class=FuenteEncabezados type=hidden value="<%=trim(Rs("Genero"))%>" name="Genero_<%=i%>">
				<input  class=FuenteEncabezados type=hidden value="<%=trim(Rs("Marca"))%>" name="Marca_<%=i%>">
				<input  class=FuenteEncabezados type=hidden value="<%=trim(Rs("Codigo_antiguo"))%>" name="Codigo_antiguo_<%=i%>">
		</tr>			
<%			Rs.MoveNext
			End if
			i = i + 1
