...
<%
	...
	nprecio = Request("nprecio")
	  ...
	  cSql = "Exec PRO_Lista_Imprime_Precios '" & SuperFamilia & "', '" & Familia & "', '" & SubFamilia & "', '" & Descripcion & "', '" & NULL & "', '" & NULL & "', '" & "', '" & NULL & "', '" & NULL & "', '" & Bodega & "', '" & Proveedor & "', '" & Productos_Certificados & "','" & Producto & "',0" & Guia & ",0" & nprecio 
	...
	<table width=74% border=1  align=center cellpadding=2 cellspacing=0><thead> 
		<tr class="FuenteCabeceraTabla">
		<td align=left width=10px><b>Imp.</b></td>
		<td align=left width=10px><b>Cant.</b></td>
		<td align=left width=10px><b>F</b></td>
		<td align=left width=10px><b>G</b></td>
		<td align=left width=10px><b>Of.</b></td>
		<td align=left width=50px><b>Producto</b></td>
		<td align=Center width=50px><b>&nbsp;RuFaSub</b></td>
		<td align=left width=10px><b>&nbsp;G&nbsp;</b></td>
		<td align=Center width=50px><b>Marca</b></td>
		<td align=center width=300px><b> <%=date()%> </b></td>
	  <!--<td align=center><b> <%=Bodega%></b></td>-->
	   <!--<td align=center><b> Stock </b></td>-->
	  <td align=center width=7px><b> P.Vta </b></td>
	  <td align=left width=7px><b>Prov</b></td>
	  <td align=Center width=20px><b>P.Prom</b></td>
		</tr>
<%		i = 1
       ...
       %>   
        ...
        <td>
			  <input class=FuenteInput type=Radio name="fleje_<%=i%>" width=5% value=Fleje <%=Fleje%>>
			  </td>
			  <td>
			  <input class=FuenteInput type=Radio name="fleje_<%=i%>" width=5% Value=Gancho <%=Gancho%>>
			  </td>				
			  <td>
			  <%if cdbl(Rs("pprom")) = 0  then%>
          <input class=FuenteInput type=checkbox name="oferta_<%=i%>" width=5%>
        <%else%>
          <input class=FuenteInput type=checkbox name="oferta_<%=i%>" width=5% checked>
        <%End if%>  
			  </td>			  
        <td align=left>
					<strong><input  class=FuenteEncabezados type=text readonly value="<%=trim(Rs("Codigo_producto"))%>" name="Producto_<%=i%>" size="9"></strong>
				</td>
		<td class="FuenteInput" type=text Width=30%  name="RuFaSub<%=i%>" align="Center"><%=trim(Rs("Superfamilia"))%>&nbsp;<%=trim(Rs("Familia"))%>&nbsp;<%=trim(Rs("Subfamilia"))%>	
		<td class="FuenteInput" type=text width=10px name="Genero<%=i%>">&nbsp;<%=trim(Rs("Genero"))%>&nbsp;&nbsp;</td>		
		<td class="FuenteInput"> <input class="FuenteInput" readonly type=text width=42% value = "<%=trim(Rs("Marca"))%>" name="Marca_<%=i%>" align="left" ></td>		
		<td><input type=text class=FuenteEncabezados readonly value="<%=Rs("Descripcion")%>" name="Desc_<%=i%>" size="50" align="left"></td>							
        <td class="FuenteInput" align=right width=10px  readonly>
			<input type=text class=FuenteEncabezados readonly value="<%=formatNumber(Rs("Pvta"),0)%>" name="precio_<%=i%>" size=4 maxlength=6></td>
		<td class="FuenteInput" align=left width=10px  readonly>
			<a><input type=text class=FuenteEncabezados value="<%=Rs("Prov")%>" name="Prov_<%=i%>" size=4 maxlength=6></a>
		</td>
		...
		<input type=hidden value="<%=Rs("PMay")%>" name="PMay_<%=i%>" >
        <input type=hidden value="<%=Rs("CMay")%>" name="CMay_<%=i%>" >
   		<td class="FuenteInput" align=left>
			<%if cdbl(Rs("pprom")) = cdbl(Rs("Pvta"))  then%>
	   <input class=FuenteEncabezados  width=20px value="<%="0"%>" name="Prom_<%=i%>" readonly size="6" maxlength=6>
			  <%else%>
	   <input class=FuenteEncabezados  width=20px value="<%=formatNumber(Rs("pprom"),0)%>" name="Prom_<%=i%>" readonly size="6" maxlength=6>
		  <%end if%> 
			  </td>          
		</tr>			
<%			Rs.MoveNext
		...
		i=i-1
		%>
			<td colspan=12 class="FuenteTitulosFunciones" align=center>
