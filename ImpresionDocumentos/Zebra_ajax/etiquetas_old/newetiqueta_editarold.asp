	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if(v_tamagnoEtiqueta=="3X2"){
				...
				$("#etiqueta3x2").css("background-size","165px 110px");
			}
			if(v_tamagnoEtiqueta=="4X3"){
				$("#etiqueta3x2").css("visibility","visible");
				$("#etiqueta3x2").css("background-size","155px 160px");
			}
			if(v_tamagnoEtiqueta=="10X5"){
				$("#etiqueta3x2").css("visibility","visible");
				$("#etiqueta3x2").css("background-size","370px 200px");
			}
			dibujar();
		...
		function dibujar(){
			...
			le=te.length;
			for(i=0;i<le;i++){
				TextoEtiqueta=TextoEtiqueta+"<div id='textolinea'"+[i+1]+" style='font-size:"+ta[i]+"px;'>&nbsp;&nbsp;"+te[i]+"</div>";
		...
		function dibujarLetra(posicion){
			$("#campo_"+posicion+"").keyup(function(){
				...
				for(i=1;i<=totalLineas;i++){
					...
					TextoEtiqueta=TextoEtiqueta+"<div id='textolinea"+i+"' style='font-size:"+tamagnoLinea[i]+"px'>&nbsp;&nbsp;"+LineaEtiqueta[i]+"</div>";
				}
				$("#word_preview").html(TextoEtiqueta.replace(/  /g,"&nbsp;"));
			});
		}
		function AgregarCampos(opcion){
			...
			if(opcion==1){
				campo="<input type='text' size='20' class='linea_editar' id='campo_"+cuentaLinea+"' name='campo_"+cuentaLinea+"' onclick='dibujarLetra("+cuentaLinea+");'><select name='tamanolinea_"+cuentaLinea+"' id='tamanolinea_"+cuentaLinea+"'><option value='8'>8</option><option value='12'>12</option><option value='16'>16</option><option value='15'>20</option><option value='18'>24</option></select><br id=br_"+cuentaLinea+">";
				$("#added").append(campo);
			}
			if(opcion==2){
		...
		function guardar_etiqueta(){
			...
			if(confirm("¿Guardar Cambios en etiqueta "+v_nombre_etiqueta+"?")){
				$.ajax({
					...
					type: "post",
					url: "etiquetas_grabar.asp",
					data: "accion=UPDATE&numero_interno_etiqueta="+v_numero_interno_etiqueta+"&detalle_etiqueta="+etiquetabd+"&nombre_etiqueta="+v_nombre_etiqueta+"&tamagno_linea="+tamagnosbd,
					success: function(data){
		...
		function imprimir(){
			...
			if(v_tamagno=="4X3"){
				...
				if(v_orientacion=="vertical"){
					...
					v_etiqueta4x3A = new Array("37","54","71","88","105","122","139","156","173","190","207","224","244","264","284","304","324","344","364","384","404");
					...
					"^FS^FO0,"+v_etiqueta4x3A[17]+"^A"+tamagno_zebra[18]+"N^FD"+LineaEtiqueta[18]+
					"^FS^FO0,"+v_etiqueta4x3A[18]+"^A"+tamagno_zebra[19]+"N^FD"+LineaEtiqueta[19]+
					"^FS^FO0,"+v_etiqueta4x3A[19]+"^A"+tamagno_zebra[20]+"N^FD"+LineaEtiqueta[20]+
					"^FS^FO0,"+v_etiqueta4x3A[20]+"^A"+tamagno_zebra[21]+"N^FD"+LineaEtiqueta[21]+
					...
					"^FS^FO260,"+v_etiqueta4x3A[17]+"^A"+tamagno_zebra[18]+"N^FD"+LineaEtiqueta[18]+
					"^FS^FO260,"+v_etiqueta4x3A[18]+"^A"+tamagno_zebra[19]+"N^FD"+LineaEtiqueta[19]+
					"^FS^FO260,"+v_etiqueta4x3A[19]+"^A"+tamagno_zebra[20]+"N^FD"+LineaEtiqueta[20]+
					"^FS^FO260,"+v_etiqueta4x3A[20]+"^A"+tamagno_zebra[21]+"N^FD"+LineaEtiqueta[21]+
					...
					"^FS^FO530,"+v_etiqueta4x3A[17]+"^A"+tamagno_zebra[18]+"N^FD"+LineaEtiqueta[18]+
					"^FS^FO530,"+v_etiqueta4x3A[18]+"^A"+tamagno_zebra[19]+"N^FD"+LineaEtiqueta[19]+
					"^FS^FO530,"+v_etiqueta4x3A[19]+"^A"+tamagno_zebra[20]+"N^FD"+LineaEtiqueta[20]+
					"^FS^FO530,"+v_etiqueta4x3A[20]+"^A"+tamagno_zebra[21]+"N^FD"+LineaEtiqueta[21]+
					"^XZ";
					etiquetasABC=etiquetasA;
					for(i=0;i<v_cantidad;i++){
						document.jzebra.findPrinter("zebra");
				...
				<table>
					<tr>
						<td>	
							<div id="added">
								<%For i=0 To count-1%>
									<input type="text" class="linea_editar" value="<%=texto(i)%>" id="campo_<%=i+1%>" name="campo_<%=i+1%>" onclick="dibujarLetra(<%=i+1%>);"><select name="tamanolinea_<%=i+1%>" id="tamanolinea_<%=i+1%>"><option value="<%=tamlinea(i)%>"><%=tamlinea(i)%></option><option value="8">8</option><option value="12">12</option><option value="16">16</option>	<option value="15">20</option>	<option value="18">24</option></select>
								<%Next%>
							</div>
						</td>
					</tr>
