...
<%
	...
	<table width=60% border=1 align=center cellpadding=2 cellspacing=0><thead>
	  ...
	  <td align=center width=10%><b> P.Liq </b></td>
...
<%		i = 1
       ...
       %>   
        ...
        <td class="FuenteInput" align=right width=5%>
          ...
          <input type=hidden value="<%=Rs("Stock")%>" name="Stock_<%=i%>" >

					<input type=hidden value="<%=Rs("PrecioNormal")%>" name="PrecioNormal_<%=i%>" >          
    		<td class="FuenteInput" align=left width=5%>
    		<%if cdbl(Rs("pprom")) = cdbl(Rs("Pvta"))  then%>
			   <input class=FuenteEncabezados  width=5% value="<%="0"%>" name="Prom_<%=i%>" readonly>
			  <%else%>
