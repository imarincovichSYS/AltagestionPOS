...
<Form name="FormularioAyuda" method="post" action="List_AyudaProductos.asp" target="wnd_abajo">
      ...
      <td rowspan = 7 align=center width=10%>            
        ...
        <select class="FuenteInput" name="FuncionesSiIncluidas" size=7  style="width:80%" ondblclick="JavaScript:precios()">     
          ...
          loop %>
			...
			<tr>
		<tr>
			<td colspan=4 width=100% class="FuenteTitulosFunciones" align=center nowrap>Imprime Precios LIQUIDACION</td>
		</tr>		
    ...
    <tr>
		  ...
		  <td>  <input class="FuenteInput" type=text name="nprecio" value="<%=nprecio%>" size=9 maxlength=9> </td>
		</tr>
        <!--cbutendieck------------------------------------------------------------------------------------------>
...
<td nowrap class="FuenteEncabezados" width=10% align=left >Bodega&nbsp;</td>
		...
		<!--cbutendieck------------------------------------------------------------------------------------------>
			...
			<%if session("login") = "13512435" then%>
			   <input class="FuenteEncabezados" type=button name="Imprime1" value=" IMPRIMIR " OnClick="JavaScript:parent.bottom.document.Productos.submit()">&nbsp;
			<%end if%>
