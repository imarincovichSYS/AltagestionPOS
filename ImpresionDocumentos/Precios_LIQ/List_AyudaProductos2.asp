      ...
      IF Request("oferta_"&i)<>"on" then 'EL PRECIO ES FLEJE NORMAL
        ...
        for cant=1 to Request("Cant_"&i)
          if Request("Precio_"&i) >= 100000 then      
              Etiqueta = Etiqueta & "^XA^FO260,30^ARN,160,165^FD" & Request("Precio_"&i) &  " ^FS"  & _
              "^FO390,180^ARN,2,2^FD" & Request("Producto_"&i) & "^FS" & _
              "^FO490,180^ARN,2,2^FD Antes $" & Request("PrecioNormal_"&i)  & "^FS" &_
              "^FO188,230^A0,20,20^FD" & Request("Desc_"&i) & "^FS"
              ...
              if Request("grs_"&i) > 0 then 
                   Etiqueta = Etiqueta & "^FO390,255^A0,20,20^FD Pr.xKilo " & formatNumber((Request("Precio_"&i)*1000)/Request("grs_"&i),0) & "^FS"
              elseif Request("cc_"&i) > 0 then
                   Etiqueta = Etiqueta & "^FO390,255^A0,20,20^FD Pr.xLitro " & formatNumber((Request("Precio_"&i)*1000)/Request("cc_"&i),0) & "^FS"
              end if
              Etiqueta = Etiqueta & "^FO190,285^A0,15,15^FD "&Request("Stock_"&i)&" Unidad(es)^FS" 
              Etiqueta = Etiqueta & "^FO310,285^A0,15,15^FDHasta 2023-07-09 o agotar stock ^FS"
          ...
          elseif Request("Precio_"&i) < 1000 then
              ...
              "^FO400,180^ARN,2,2^FD" & Request("Producto_"&i) & "^FS" &_ 
              "^FO490,180^ARN,2,2^FD Antes $" & Request("PrecioNormal_"&i)  & "^FS" &_
              "^FO188,225^A0,20,20^FD" & Request("Desc_"&i) & "^FS"
          ...
          elseif Request("Precio_"&i) >= 1000 and  Request("Precio_"&i) < 10000 then      
              Etiqueta = Etiqueta & "^XA^FO270,20^ARN,175,180^FD" & Request("Precio_"&i) &  " ^FS"  & _
              "^FO350,180^ARN,2,2^FD" & Request("Producto_"&i) & "^FS" & _
              "^FO490,180^ARN,2,2^FD Antes $" & Request("PrecioNormal_"&i)  & "^FS" &_
              "^FO188,225^A0,20,20^FD" & Request("Desc_"&i) & "^FS"  
              'if Request("PMay_"&i) >0 then
              ''    Etiqueta = Etiqueta & "^FO720,240^A0,25,25^FD" & formatNumber(Request("PMay_"&i),0) & "^FS"
              ...
              'end if
              if Request("grs_"&i) > 0 then 
                   Etiqueta = Etiqueta & "^FO355,255^A0,20,20^FD Pr.xKilo " & formatNumber((Request("Precio_"&i)*1000)/Request("grs_"&i),0) & "^FS"
              elseif Request("cc_"&i) > 0 then
                   Etiqueta = Etiqueta & "^FO355,255^A0,20,20^FD Pr.xLitro " & formatNumber((Request("Precio_"&i)*1000)/Request("cc_"&i),0) & "^FS"
              end if
              Etiqueta = Etiqueta & "^FO190,285^A0,15,15^FD "&Request("Stock_"&i)&" Unidad(es)^FS"  
              Etiqueta = Etiqueta & "^FO310,285^A0,15,15^FDHasta 2023-07-09 o agotar stock ^FS"
              Etiqueta = Etiqueta & "^XZ" 
          else
              Etiqueta = Etiqueta & "^XA^FO260,20^ARN,175,180^FD" & Request("Precio_"&i) &  " ^FS"  & _
              "^FO350,180^ARN,2,2^FD" & Request("Producto_"&i) & "^FS" & _
              "^FO490,180^ARN,2,2^FD Antes $" & Request("PrecioNormal_"&i)  & "^FS" &_
              "^FO188,225^A0,20,20^FD" & Request("Desc_"&i) & "^FS"  
              'if Request("PMay_"&i) >0 then
              ...
              Etiqueta = Etiqueta & "^FO190,285^A0,15,15^FD "&Request("Stock_"&i)&" Unidad(es)^FS"  
              Etiqueta = Etiqueta & "^FO315,285^A0,15,15^FDHasta 2023-07-09 o agotar stock ^FS"
              Etiqueta = Etiqueta & "^XZ" 
          end if        
          %><br><%    
        next
      ...
      Puerto2 = "LPT2"
        for cant=1 to Request("Cant_"&i)
          ...
          elseif Request("Prom_"&i) < 1000 then
              ...
              Etiqueta = Etiqueta & "^FO110,730^A0,20,20^FD 5 Unidad(es)^FS"
              Etiqueta = Etiqueta & "^FO310,730^A0,20,20^FDHasta 2023-07-09 o agotar stock ^FS"
              Etiqueta2 = Etiqueta2 & "^XZ"
