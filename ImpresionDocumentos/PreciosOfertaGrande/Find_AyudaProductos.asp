...
*/
    ...
    function precios()
    {
      var npos = document.FormularioAyuda.FuncionesSiIncluidas.selectedIndex  
      valorText  = document.FormularioAyuda.FuncionesSiIncluidas.options[ npos ].text 
      document.FormularioAyuda.nprecio.value = valorText 
    }
...
<Form name="FormularioAyuda" method="post" action="List_AyudaProductos.asp" target="wnd_abajo">
    ...
    <tr class="FuenteEncabezados">
      <td rowspan = 7 align=center width=10%>            
        <%cSql = "Select distinct folio_cambio_de_precio from cambios_de_precios where fecha>=convert(varchar(12),getdate(),112) and origen = 'Grandes' order by folio_cambio_de_precio desc"
        set rsfolios = Conn.execute(cSql)%>
        <select class="FuenteInput" name="FuncionesSiIncluidas" size=7  style="width:80%" ondblclick="JavaScript:precios()">     
          <%do while not rsfolios.eof %> 
          <option value="<%=rsfolios("folio_cambio_de_precio")%>"><%=rsfolios("folio_cambio_de_precio")%></option>
          <%rsfolios.movenext
          loop %>
        </select>
      </td>  
		...
		<tr>
			<td colspan=4 width=100% class="FuenteTitulosFunciones" align=center nowrap>Imprime Precios Grandes</td>
		</tr>
    ...
    <tr>
		  ...
		  <td>  <input class="FuenteInput" type=text name="producto" onkeypress="JavaScript:EnterSubmit(event)" value="<%=producto%>" size=15 maxlength=9> </td>		

      <td width=10% class="FuenteEncabezados" align=left nowrap>N° Cambio de Precio</td>
		  <td>  <input class="FuenteInput" type=text name="nprecio" value="<%=nprecio%>" size=16 maxlength=9> </td>
		</tr>
        <!--cbutendieck------------------------------------------------------------------------------------------>
        
			<td width=10% class="FuenteEncabezados" align=left nowrap>Rubro</td>
			<td width=34% align=left>
				<select class="FuenteInput" name="Slct_SuperFam" style="width:150"  onchange="JavaScript:Carga_Familia(this.value);fAsigna()">
					<option value=""></option>
			...
			<td nowrap class="FuenteEncabezados" width=10% align=left >Bodega&nbsp;</td>
					<td align=left>
						 <select Class="FuenteInput" style="width:110" name="Bodega">
     ...
     <tr>
		 <td width=10% class="FuenteEncabezados" align=left nowrap>Familia</td>
			<td width=34% align=left>
				<select class="FuenteInput" name="Slct_Familia" style="width:150"  onchange="JavaScript:Carga_SubFamilia(this.value);fAsigna()"> 
					<option value=""></option>
		  ...
		  <td>  <input class="FuenteInput" type=text name="guia" value="<%=guia%>" size=16 maxlength=9>
      </td>
