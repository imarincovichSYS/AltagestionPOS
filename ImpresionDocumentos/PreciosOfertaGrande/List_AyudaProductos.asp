...
<%
	...
	<table width=90% border=1 align=center  cellspacing=0><thead>
		...
		<td align=Center width=25% ><b>Ru Fa Sub</b></td>
		<td align=Center width=10%><b>&nbsp;G&nbsp;</b></td>
		<td align=Center width=60%><b>&nbsp;Marca</b></td>
		<td align=center width=80%><b> <%=date()%> </b></td>
	  <!--<td align=center><b> <%=Bodega%></b></td>-->
	   <!--<td align=center><b> Stock </b></td>-->
	  <td align=left width=6%><b> P.Vta </b></td>
	  <td align=left width=6%><b>Prov</b></td>
	  <td align=left width=6%><b>P. Prom</b></td>
		</tr>
<%		i = 1
      ...
      if guia<>"" then
		...
		<tr>
		  <td>
		  		<%if i = 1 then%>
			<input class=FuenteInput type=checkbox name="Ajuste_<%=i%>" width=5% checked>
			<%else%>
				<input class=FuenteInput type=checkbox name="Ajuste_<%=i%>" width=5% >
			<%end if%>
          ...
          <td>
			  <input class=FuenteInput type=Radio name="fleje_<%=i%>" width=5% value=Verde >
		  </td>
		  <td>
		  	
			<input class=FuenteInput type=Radio name="fleje_<%=i%>" width=5% Value=Rojo checked>
			
		  </td>							
		  <td align=left>
			<strong><input  class=FuenteEncabezados type=text readonly value="<%=trim(Rs("Codigo_producto"))%>" name="Producto_<%=i%>" size="10"></strong>
		  </td>
		  <td align=center class=FuenteInput Type=text readonly width=5% name="RuFaSub_<%=i%>">
		  <%=trim(Rs("Superfamilia"))%>&nbsp;<%=trim(Rs("Familia"))%>&nbsp;<%=trim(Rs("Subfamilia"))%>
		  </td>
		  <td align=center class=FuenteInput Type=text readonly name="Genero<%=i%>">&nbsp;<%=trim(Rs("Genero"))%>&nbsp;</td>
		  
		  <td align=center class=FuenteEncabezados>
			<input Type=text readonly width=25% name="Marca_<%=i%>" value="<%=trim(Rs("Marca"))%>">
		  </td>		  
          ...
          <td class="FuenteInput" align=right width=5%>
			 <input type=text class=FuenteEncabezados readonly value="<%=formatNumber(Rs("Pvta"),0)%>" name="precio_<%=i%>" size="6">
		  </td>
		  <td class="FuenteInput" align=left width=5%>
				<a><input type=text class=FuenteEncabezados value="<%=Rs("Prov")%>" name="Prov_<%=i%>" readonly size="6"></a>
		  </td>
					...
					<input type=hidden value="<%=formatNumber(Rs("Stock"),0)%>" name="Stock_<%=i%>" >
					<input type=hidden value="<%=Rs("fecha_promocion")%>" name="fecha_promocion_<%=i%>" >
  				<input type=hidden value="<%=Rs("Codigo_Ean13")%>" name="Codigo_Ean13_<%=i%>" >
  				

    		<td class="FuenteInput" align=left width=5%>
			  <input class=FuenteEncabezados  width=5% value="<%=Rs("pprom")%>" name="Prom_<%=i%>" readonly size="8">
			  </td>          
		</tr>			
<%			Rs.MoveNext
			End if
