...
<%
  ...
  Set rs = Conn.execute("EXEC PUN_Datos_cliente_para_factura '" & Session("Cliente_boleta") & "', '" & Session( "Vendedor" ) & "'" )
  if not rs.eof then
     ...
     Fono_Casa = rs("fono_Casa") 
     Fono_Celular = rs("fono_celular")
  ...
  if len( Session( "FechaDespacho" ) ) >  0 then
      ...
      space(90) & FechaLarga(now()) & space(7) & "DESPACHO " & FechaLarga( Session( "FechaDespacho" ) ) & Crlf & _ 
      space(81) & "VENDEDOR " & " " & Vendedor &  Crlf & _ 
      space(94) & Fono_Casa & " " & Fono_Celular & Crlf  & Crlf & _ 
      space(5)  & PadR(Nombre_cliente,50) & PadR(Direccion_cliente & "," & Comuna,50)  & PadL(Rut_cliente,12) & Crlf & Crlf 
  ...
  do while not rs.eof
      ...
      x = AjustaTextoFAC( rs("Marca") & " " & rs("Nombre_producto"), 32)
      arrayFAC = Split(x, "|" )

      Factura = Factura & PadL(Cantidad,6) & "     " & _
                PadL(rs("Unidad_de_medida_consumo"),8) & "  " & _
                PadR(rs("Producto"),10) & "  " & _
                PadR( replace( arrayFAC(0), chr(34) ,"P" ) ,32) & "     " & _
      ...
      IF FALSE THEN
      if ubound( arrayFAC ) > 1 then
          Factura = Factura & space(6) & "     " & _
                    space(8) & "  " & _
                    space(10) & "  " & _
                    PadR( replace( arrayFAC(1), chr(34) ,"P" ) ,32) & "     " & Crlf
