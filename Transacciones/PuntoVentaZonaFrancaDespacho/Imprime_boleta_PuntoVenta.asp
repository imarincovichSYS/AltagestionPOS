...
'			TotalNeto    = cStr(Right("000000000" & Redondear(SubTotalNeto+.1,0),9))
	  ...
	  Marca 		= Left(rs("Marca"),12)
	  Superfamilia 	= Left(rs("Superfamilia"),2)
	  Familia 		= Left(rs("Familia"),2)
	  Subfamilia 	= Left(rs("Subfamilia"),3)
...
Set rs = Conn.execute("Exec MOP_Lista_detalle_DVT_impresion_factura_ILA '" & trim(Session("Login")) & "'" )
  ...
  if rs("Monto_ILA_13") > 0 or Rs("Monto_ILA_15") > 0 or Rs("Monto_ILA_27") > 0 then
      'LineaImpuestoILA = "Impto I.L.A.: "
  	  'if cDbl( porcentajeILA_1 ) >  0 then
  	  if rs("Monto_ILA_13") > 0 then
      		'LineaImpuestoILA = LineaImpuestoILA & "(" & ltrim( PadL( FormatNumber( porcentajeILA_1, 2 ), 5 ) ) & ") " 
      		LineaImpuestoILA = LineaImpuestoILA & "(" & ltrim( PadL( "13", 5 ) ) & "%) " & formatNumber(rs("Monto_ILA_13"),0) & " "
  	  end if
  	  if rs("Monto_ILA_15") > 0 then
      		'LineaImpuestoILA = LineaImpuestoILA & "(" & ltrim( PadL( FormatNumber( porcentajeILA_1, 2 ), 5 ) ) & ") " 
      		LineaImpuestoILA = LineaImpuestoILA & "(" & ltrim( PadL( "15", 5 ) ) & "%) " & formatNumber(rs("Monto_ILA_15"),0) & " "
  	  end if
  	  'if cDbl( porcentajeILA_2 ) >  0 then
  	  if rs("Monto_ILA_27") > 0 then
      		'LineaImpuestoILA = LineaImpuestoILA & "(" & ltrim( PadL( FormatNumber( porcentajeILA_2, 2 ), 5 ) ) & ") "
      		LineaImpuestoILA = LineaImpuestoILA & "(" & ltrim( PadL( "27", 5 ) ) & "%) " & formatNumber(rs("Monto_ILA_27"),0)
  	  end if
  end if
...
'msgbox "paso-1:" & xError
        ...
        If InStr(1, xError, ":") > 0 Then
				...
				if Boleta_fiscal_actual <> <%=Boleta_actual%> then
				   xError = "060"
					 Msgbox "Número de boleta incorrecto # " & Boleta_fiscal_actual & " requerido : " & <%=Boleta_actual%> , vbOkOnly + 16, "Error"
				end if
            ...
            aCobros = Split("<%=Cobros%>","|")
    				...
    				ReDim mtxDetalle(Ubound(aDetalle)+Ubound(aCobros)+8)
            ...
            For n=0 to Ubound(aCobros)-1
                ...
                'msgbox( "cobro " & n & " : " & mtxDetalle(a+n+3) )
    				...
    				mtxDetalle(a+n+3) = "271000000000" 'Donación
            mtxDetalle(a+n+4) = "111Cajera:<%=left(session("Nombre_usuario"),20)%> "
            mtxDetalle(a+n+5) = "111Tienda:<%=left(session("xNombre_centro_de_venta"),20)%> "
            mtxDetalle(a+n+6) = "111<%=left(trim(Session("Mensaje_del_dia")),50)%> "

			if "<% = LineaImpuestoILA %>" <> "" then
            	mtxDetalle(a+n+7) = "111Imp. ILA:<% = left( LineaImpuestoILA, 35 ) %> "
   				mtxDetalle(a+n+8) = "99" 'Fin linea con corte de papel
            else
   				mtxDetalle(a+n+7) = "99" 'Fin linea con corte de papel
            end if
					    ...
					    if a <> Ubound(mtxDetalle) then
    						...
    						If Len(Trim(xError)) > 0 Then
    						    ...
    						    Do While Len(Trim(xError)) > 0
    								...
    								elseif Instr(1,xError,"Secuencia de comandos no válida") > 0 then
