...
Cache
	...
	URLProductos = "http://cajas/Altagestion/Imagenes/Productos/"
	
'Response.Write Session("xBloqueado") & " **** "
'Response.Write Session("xEconomato") & " *** "

ModoVendedor = "I"  ' S=Select, I=Input
  ...
  if Conn.Errors.Count = 0 then
     ...
     if Session("Cliente_boleta") <> "CLI_BOLETA" and MensajeError="" then
       ...
       if v_tipEntComer = "C" or (v_tipEntComer = "A" and v_clasDeEmpleado = "") then  
          ...
          if (v_temporada="XMAY" or v_temporada="PATG" or v_temporada="NOV" or v_temporada="CHAO") and v_tipo_lista_precio="LXMAY" then 'Es producto Mayorista y el cliente está asociado a una lista mayorista
	...
	'		 "and lista_de_precios = (select LTRIM(RTRIM(valor_texto)) from parametros where parametro = 'LP_BASE')) - IsNull((select monto_descuento from " &_ 
...
For n = 1 To Detalle_lineas
  ...
  if temporada = "XMAY" or temporada = "PATG" or temporada="NOV" or temporada="CHAO" then 
	...
	Precio_final = ( Precio_final * ( ( 100 - Descuento1 ) / 100 ) * ( ( 100 - Descuento2 ) / 100 ) \ 1 )
...
<div style="position:absolute; top:194px; left:<%=560-delta%>px; z-index:1">
     ...
     <input id="Producto" type="text" name="Producto" size="22" maxlength="20" value="" 
     onchange="javascript:agrega_producto()">
</div>
...
<div style="position:absolute; top:303px; left:<%=800-delta%>px; z-index:1"  style="font-size: 16px; font-family: sans-serif;font-weight: bold; line-height: 150%;">
&nbsp;<input type='hidden' name='Vendedor' value='<% = Session( "Vendedor" ) %>' /><br>
&nbsp;<input type='hidden' name='FechaDespacho' value='<% = Session( "FechaDespacho" ) %>' />
</div>
...
<%=strzero(day(now()),2)&"/"&strzero(month(now()),2)&"/"&strzero(year(now()),4)%>
</div>
<%
