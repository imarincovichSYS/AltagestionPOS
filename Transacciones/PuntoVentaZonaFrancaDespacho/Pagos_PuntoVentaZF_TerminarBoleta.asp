...
if Accion_a_ejecutar = "Terminar" and vacio(MensajeError) and session("Imprimiendo_boleta") <> "Si" then
  ...
  if tipo_cobro_ECO then 'Modificación para productos con descuentos ECONOMATO
                    ...
                    if Session("Porcentaje_descuento_empleado") > 0 then
                        ...
                        if Session("Permite_hacer_descuento_empleado") then
                             if Session("Porcentaje_descuento_empleado") >= 90 and Session("Porcentaje_descuento_empleado") < 100then
                                ...
                                uno=(((cdbl(rs("Costo_cpa_US$"))*cdbl(rs("Valor_paridad_moneda_oficial")))+(cdbl(rs("Costo_cif_adu_US$"))*cdbl(rs("Valor_paridad_moneda_oficial"))*(0.019+0.008+(100-Session("Porcentaje_descuento_empleado"))/100)))*(ILA))
                                dos=(cdbl(rs("Costo_cif_ori_US$"))*cdbl(rs("Valor_paridad_moneda_oficial"))*ILA)
                                if uno>=dos then
                                    precio_con_descuento=(((((cdbl(rs("Costo_cpa_US$"))*cdbl(rs("Valor_paridad_moneda_oficial")))+(cdbl(rs("Costo_cif_adu_US$"))*cdbl(rs("Valor_paridad_moneda_oficial"))*(0.019+0.008+(100-Session("Porcentaje_descuento_empleado"))/100)))*(1+ILA))))
                                    ...
                                    precio_con_descuento=(((((cdbl(rs("Costo_cpa_US$"))*cdbl(rs("Valor_paridad_moneda_oficial")))+(cdbl(rs("Costo_cif_adu_US$"))*cdbl(rs("Valor_paridad_moneda_oficial"))*(0.019+0.008+(100-Session("Porcentaje_descuento_empleado"))/100)))+(cdbl(rs("Costo_cif_ori_US$"))*cdbl(rs("Valor_paridad_moneda_oficial"))*ILA))))
                                    'precio_con_descuento = cint(precio_con_descuento/10)*10
                                    precio_con_descuento = cdbl(precio_con_descuento) - cdbl(precio_con_descuento mod 10)' Mod Cristian
                                    Monto_descuento_empleado = PRECIO - precio_con_descuento  
                                end if    
                                ...
                                Monto_descuento_empleado =  ( ( ( ( ( Precio_sin_ILA - ( Costo_CPA + ( Costo_CIF_ADU * ( 0.019 + 0.008 ) ) ) ) * Porcentaje_descuento_empleado)) * (1+ILA) ) \ 10 ) * 10 'Francisco
                                if Monto_descuento_empleado > Monto_descuento then
                                   Descuento_linea = ( 1 - ( ( Precio - Monto_descuento_empleado ) / Precio ) ) * 100
                                end if                            
                            end if
   ...
   Numero_despacho_de_venta = Numero_documento_no_valorizado
    ...
    '
    FechaDespacho = ""
    IF len( Session( "FechaDespacho" ) ) > 0 THEN
        FD = Session( "FechaDespacho" )
        FechaDespacho = mid( FD, 7, 4 ) & "/" & mid( FD, 4, 2 ) & "/" & mid( FD, 1, 2 )
    END IF
    DVT_activa = Get_Numero_Interno_DVT_Activa_X_Login(Session("Login"))
    SQL = "Exec DNV_Graba_Vendedor_y_FechaDespacho "
    SQL = SQL & "'" & Session( "Empresa_usuario" ) & "', "
    SQL = SQL & "" & DVT_activa & ", "
    SQL = SQL & "'" & Session( "Vendedor" ) & "', "
    if len( FechaDespacho ) >  0 then
        SQL = SQL & "'" & FechaDespacho & "' "
    else
        SQL = SQL & "Null "
    end if
	Conn.Execute( SQL )
...
'	@Numero_impresora_fiscal_a_imprimir int
  ' BOLETA FANTASMA JOMS Sept 2007
    ...
    end if ' if Error = "N" then (Error al grabar el Vendedor y la FechaDespacho)
    ...
    for n = 1 to Numero_documentos_cobro
        ...
        'Pago con dolares
        Numero_interno_documento_valorizado_UEI = 0
        if Documento_valorizado_ingresado = "UEI" then
        ...
        'Comprobante de Ingreso ?
           ...
           'Monto_a_pagar = Monto_a_pagar * Session("Tipo_cambio_boleta")
           ...
           sql="Detalle pesos egresados pordolares!"
                ...
                Numero_interno_documento_valorizado_UEI = rsUEI( "Documento" )
                rsUEI.close
        ...
        cEconomato = ""
        if len(trim(aDocumento_cobro(3))) > 0 then
            aDatos = Split( aDocumento_cobro(3), "¬")
            cEconomato = aDatos(0)
            DiaCierre  = aDatos(1)
            DiaCobro   = aDatos(2)
        end if
...
'-------------------------- DEI o NCV -------------------------        
        if Documento_valorizado_ingresado <> "DEI" and Documento_valorizado_ingresado <> "NCV" and cDbl( aDocumento_cobro(2) ) = 0 then
        ...
        'Comprobante de Ingreso en Documento Valorizado
          ...
          'response.write Numero_interno_DocFav & chr(13)
                IF Numero_interno_documento_valorizado_UEI <> 0 THEN
                    cSQL = "UPDATE Documentos_valorizados SET Cliente = Cliente, Numero_comprobante_de_ingreso = " & Numero_documento_no_valorizado& " WHERE Numero_interno_documento_valorizado = " & Numero_interno_documento_valorizado_UEI
					conn.Execute(cSQL)
					if len(trim(Err.Description)) > 0 then
						Error = "S"
			            MensajeError = Err.Description
    ...
    next
 	...
 	sql="exec Actualizar_Reserva_Ashley_Doc_Venta " &	Numero_despacho_de_venta 
		Conn.Execute(sql)
