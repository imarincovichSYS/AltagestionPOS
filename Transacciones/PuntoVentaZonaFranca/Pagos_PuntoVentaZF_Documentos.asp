...
' PARTBK = "N"
' Set Rs = Conn.Execute("Exec PAR_ListaParametros 'PARTBK'")
' If Not Rs.EOF then
'   PARTBK = Rs("Valor_texto")
'     if IsNull( PARTBK ) Then PARTBK = "N"
' end if
' Rs.close
' set Rs = nothing
...
do While Not Rs.EOF
    ...
    if (rs("documento") = "UEI" or rs("documento") = "TBK") then
...
set Rs = nothing
    ...
    '    Response.write( u & ": " & Documento_cobranza(u) & " - " & now() & "<br>" )
    'next

Total_por_pagar = Total_pesos
'response.write "Total por pagar : " & Total_por_pagar & " vuelto : " & Vuelto
    ...
    for u = 1 to Documentos_cobranza
        ...
        <%  else
                if cDbl(Numero_interno_documento_cobranza(u)) > 0 then
                    'if cDbl(Numero_interno_documento_cobranza(u)) = cDbl(Numero_documento_cobro(n)) then 
...
<%if Tipo_documento_cobro(n) <> "DEI" and Tipo_documento_cobro(n) <> "NCVs" and Tipo_documento_cobro(n) <> "EFI" and Tipo_documento_cobro(n) <> "UEI" and Tipo_documento_cobro(n) <> "DDC" then%>
   <td align=center>&nbsp;
...
<%end if%>
   ...
   Class="FuenteInput" type="text" name="Numero_documento_cobro_<%=n%>" size="12" maxlength="10" 
   value="<%=Numero_documento_cobro(n)%>"></td>
<%elseif Tipo_documento_cobro(n) = "DEI" or Tipo_documento_cobro(n) = "NCV" then%>
<%else%>
   <td colspan="3">&nbsp;</td>
<%end if
  if Tipo_documento_cobro(n) <> "DEI" and Tipo_documento_cobro(n) <> "NCVs" and Tipo_documento_cobro(n) <> "EFI" and Tipo_documento_cobro(n) <> "UEI" and Tipo_documento_cobro(n) <> "DDC" and n = Numero_documentos_cobro then%>
   <td><input onkeypress="return Valida_Digito(event);"
   Class="FuenteInput" type="text" name="Numero_documentos_adicionales" size="3" maxlength="2" value="1" 
