...
if Request("Accion") = "Elimina_Todos" then
	...
	strSQL="exec ODV_Desasociar_voucher_venta_multiple " & v_num_int_DVT_activa
	...
	set rs_graba = Conn.execute(strSQL)
	strSQL="update entidades_comerciales set numero_interno_dvt_activa=null, Cantidad_productos_eliminados = Null " &_
	       "where empresa='SYS' and entidad_comercial='"&Session("Login")&"'"
	set rs_graba = Conn.execute(strSQL)
	
	'################################################################
  '##### RESETEA LA OPCION DE DESCUENTOS MANUALES DE CAJERA #######
  if Session("Login") <> "10027765" and Session("Login") <> "13884785" and Session("Login") <> "15308847" and Session("Login") = "15309968" and Session("Login") = "13971954" and Session("Login") = "15582502" and Session("Login") = "17238275" and Session("Login") = "15923212" and Session("Login") = "13859206"  and Session("Login") = "17893185"  Or Session("Login") = "19424203" then
    ...
    if Conn.Errors.Count = 0 then
        ...
        if Conn.Errors.Count <> 0 then
          ...
          Eliminado = false
            ...
            strSQL = "UPDATE Entidades_comerciales "
            strSQL = strSQL & "SET Cantidad_productos_eliminados =  1 "
            strSQL = strSQL & "WHERE Entidad_comercial = '" & Session("Login") & "' "
            Conn.Execute(strSQL)
