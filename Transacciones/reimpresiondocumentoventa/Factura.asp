...
if MsgError <> "" Then
	...
	cSql_C = "Exec FAC_ListaCabeceraFactura '" & Session("Empresa_usuario") & "', " & NroInterno & ", " & Documento & ", " & NroFactura
'Response.Write cSql_C & "<br>"
  ...
  if RsC("Tipo_documento_zona_franca") <> "" then
	   if trim(RsC("Tipo_documento_zona_franca")) = "AD" then
		 ...
		 elseif trim(RsC("Tipo_documento_zona_franca")) = "DIH" then
		 		 response.redirect "../../ImpresionDocumentos/FacturaZonaFranca/DIH.asp?Documento=FAE&NroFactura=" & trim(RsC("NroFactura")) & "&Funcion=Reimpresion&Ticket=N&NroFac=" & trim(RsC("Numero_interno_documento_valorizado"))		 
		 else
