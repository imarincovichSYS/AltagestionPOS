...
if Accion_a_ejecutar = "Terminar" and vacio(MensajeError) and session("Imprimiendo_boleta") <> "Si" then
                    ...
                    if Session("Porcentaje_descuento_empleado") > 0 then
                        ...
                        if Session("Permite_hacer_descuento_empleado") then
                             if Session("Porcentaje_descuento_empleado") >= 90 and Session("Porcentaje_descuento_empleado") < 100then
                                ...
                                uno=(((cdbl(rs("Costo_cpa_US$"))*cdbl(rs("Valor_paridad_moneda_oficial")))+(cdbl(rs("Costo_cif_adu_US$"))*cdbl(rs("Valor_paridad_moneda_oficial"))*(0.022+0.0038+(100-Session("Porcentaje_descuento_empleado"))/100)))*(ILA))
                                dos=(cdbl(rs("Costo_cif_ori_US$"))*cdbl(rs("Valor_paridad_moneda_oficial"))*ILA)
                                if uno>=dos then
                                    precio_con_descuento=(((((cdbl(rs("Costo_cpa_US$"))*cdbl(rs("Valor_paridad_moneda_oficial")))+(cdbl(rs("Costo_cif_adu_US$"))*cdbl(rs("Valor_paridad_moneda_oficial"))*(0.022+0.0038+(100-Session("Porcentaje_descuento_empleado"))/100)))*(1+ILA))))
                                    ...
                                    precio_con_descuento=(((((cdbl(rs("Costo_cpa_US$"))*cdbl(rs("Valor_paridad_moneda_oficial")))+(cdbl(rs("Costo_cif_adu_US$"))*cdbl(rs("Valor_paridad_moneda_oficial"))*(0.022+0.0038+(100-Session("Porcentaje_descuento_empleado"))/100)))+(cdbl(rs("Costo_cif_ori_US$"))*cdbl(rs("Valor_paridad_moneda_oficial"))*ILA))))
                                    'precio_con_descuento = cint(precio_con_descuento/10)*10
                                    precio_con_descuento = cdbl(precio_con_descuento) - cdbl(precio_con_descuento mod 10)' Mod Cristian
                                    Monto_descuento_empleado = PRECIO - precio_con_descuento  
                                end if    
                                ...
                                Monto_descuento_empleado =  ( ( ( ( ( Precio_sin_ILA - ( Costo_CPA + ( Costo_CIF_ADU * ( 0.022 + 0.0038 ) ) ) ) * Porcentaje_descuento_empleado)) * (1+ILA) ) \ 10 ) * 10 'Francisco
                                if Monto_descuento_empleado > Monto_descuento then
                                   Descuento_linea = ( 1 - ( ( Precio - Monto_descuento_empleado ) / Precio ) ) * 100
                                end if                            
                            end if
  ...
  'se genera documento valorizado
   ...
   if Session("flagFechaBoleta") then
	   sql="exec DOV_Graba_pago_documento_venta '" & SessionEmpresa_usuario              & "', 0" & +_
								Numero_documento & ", '"  & +_
								Slct_DocDescpacho & "', '" & +_
								Codigo_moneda & "', '" & +_
								Session("Login") & "', '" & +_
	   ...
	   sql="exec DOV_Graba_pago_documento_venta '" & SessionEmpresa_usuario              & "', 0" & +_
								Numero_documento & ", '"&Slct_DocDescpacho&"', '" & +_
								Codigo_moneda & "', '" & +_
								Session("Login") & "', '" & +_
								Session("xBodega") & "', 01, 0" & +_
								Neto & ", 0"  & +_
		...
		if Conn.Errors.Count <> 0 then
			Error = "S"
			MensajeError = Err.Description & "336"
    ...
    if session("Boleta_actual") <> "0" and session("Fiscal_actual") <> "0" and  Slct_DocDescpacho = "BOV" then 'and  Slct_DocDescpacho = "BOV"
       sql="Grabando boleta fantasma!"
       sql="exec PUN_Graba_boleta_fiscal_impresora_termica " & _
                 " 0" & +Numero_despacho_de_venta & ", '"  & +_
                 session("IP") & "'"
    ...
    for n = 1 to Numero_documentos_cobro
        ...
        if Documento_valorizado_ingresado = "UEI" or Documento_valorizado_ingresado = "DDC" then
          ...
          '--------------------------------------------------------------------
          sql = "exec DNV_GrabaComprobanteIngreso '" & SessionEmpresa_usuario & "', 0" & +_
...
'response.write "<br>*6*<br>" & Error
          ...
          'response.write sql
   				sql = "exec DNV_GrabaComprobanteEgreso '" & SessionEmpresa_usuario & "', 0" & +_
										Numero_documento_no_valorizado & ",'" & Fecha_emision & "','O',Null,'" & +_
										"$" & "','EFE',0" & +_
          					Monto_a_Pagar & ",null,'','" & Session("Login") & "'"
          set RsManejo=Conn.Execute(sql)
          ...
          IF len(Numero_despacho_de_venta) = 0 then Numero_despacho_de_venta = "Null"
					if Session("flagFechaBoleta") then
					   sql = "Exec DOV_GrabaComprobanteEgreso '" & _
								 SessionEmpresa_usuario & "', 'EFE', 0" & +_
								   Numero_documento_ingresado & ", 0" & +_
...
'---------------- INICIO GRABACIÓN DE ECONOMATO ASOCIADO AL CLIENTE --------------
        if len(trim(cEconomato)) > 0 then
            if Instr(1, cCadEconomato, "¬" & cEconomato & "¬" ) = 0 then
                ...
                cSql = "Exec ASE_Graba_Asociado '" & SessionEmpresa_usuario & "', "
...
'            cSql = "Exec TMU_Graba_Asociado '" & SessionEmpresa_usuario & "', "
'            cSql = cSql & "'" & Session("Cliente_boleta") & "', 'A', Null"
'            Conn.Execute ( cSql )
'			if len(trim(Err.Description)) > 0 then
'                Error = "S"
...
'-------------------------- DEI o NCV -------------------------        
    ...
    if Session("Tipo_documento_venta") = "FAV" then
        ...
        sql="exec ODV_Actualizar_voucher_venta_factura " &    Numero_despacho_de_venta 
        Conn.Execute(sql)
         'Se actualiza el folio en la tabla de control de folios
       Sql = "Actualiza folio para facturas"
       Sql = "EXEC PUN_Actualiza_folio 0" & Folio_disponible & ",'" & SessionEmpresa_usuario & "','" & Session("Login") & "'"
       Conn.Execute(Sql )
       if len(trim(Err.Description)) > 0 then
          Error = "S"
	    		MensajeError = Err.Description
    ...
    if Error = "N" then
       ...
       if Slct_DocDescpacho = "BOV" then
            ...
            If Not Rs.Eof then
                ...
                session("Boleta_actual") = FolioBoleta
            end if
