...
for n = 1 to Numero_documentos_cobro
    ...
    if aDocumento(0) = "UEI" or aDocumento(0) = "DDC" then
       ...
       ELSE
          if Documento_para_vuelto = "" then Documento_para_vuelto = "DDC"
       END IF
    end if
    ...
    if cDbl( aDocumento(3) ) >  0 then
    Numero_documento_cobro(n) = aDocumento(3)
    else
   ...
   'JOMS 27-11-2007 Nuevo para ECO
    ...
    if Tipo_documento_cobro(n) = "EXP" then 
      ...
      If Not Rs.Eof then
          ...
          Fecha_documento_cobro(n) = Rs("Fecha_cobro")
          'Fecha_documento_cobro(n) =DateAdd("m", 1,Rs("Fecha_cobro"))
      end if
      Rs.Close
      Set Rs = Nothing
    end if
    ...
    if not vacio(Ultima_fecha_documento_cobro) then
      if Tipo_documento_cobro(n) <> "EFI" and Tipo_documento_cobro(n) <> "UEI" and Tipo_documento_cobro(n) <> "DDC" then
        if cdate(Ultima_fecha_documento_cobro) > cdate(Fecha_documento_cobro(n)) then
           MensajeError = "Error en fechas de pago"
        end if
      end if
    end if
	if not Session("flagFechaBoleta") then
    ...
    if Total_pesos_por_cobrar <> 0 and Accion_a_ejecutar <> "AgregaCheques" and Accion_a_ejecutar <> "Modificar" and Accion_a_ejecutar <> "ModificarPrecio" then
       if Tipo_documento_cobro(n) = "EFI" and Total_pesos_por_cobrar < Monto_documento_cobro(n) then
          ...
          Exit For
       elseif n = Numero_documentos_cobro and Total_pesos_por_cobrar <> 0 and Tipo_documento_cobro(n) <> "DEP" and Tipo_documento_cobro(n) <> "NCV" and Tipo_documento_cobro(n) <> "UEI" and Tipo_documento_cobro(n) <> "DDC" then
