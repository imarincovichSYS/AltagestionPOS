...
strSQL="insert into msg_impresora(login,ip,msg) values ('"&Session("Login")&"','"&session("IP")&"','"&request("msg")&"')"
...
Sub LeeDetalle()
    ...
    If not Detalle_leido then
       'response.write "Exec MOP_Lista_detalle_DVT '" & trim(Session("Login")) & "'<br>"
        Set rs1 = Conn.execute("Exec MOP_Lista_detalle_DVT '" & trim(Session("Login")) & "'" )
...
function Get_Total_Precio_Mayoristas_Lista_DVT(v_num_int_DVT_activa)
  ...
  strSQL="select Sum(cantidad_salida*precio_de_lista) resultado from movimientos_productos with(nolock) " &_
	       "where empresa='SYS' and numero_interno_documento_no_valorizado="&v_num_int_DVT_activa&" " &_
	       "and (temporada='XMAY' or temporada='PATG' or temporada='NOV' or temporada='CHAO' or temporada='500A OS')"
	'--------------------------------------------------------------------------------------------------------
	set rs_get = Conn.Execute(strSQL) : resultado = ""
	if not rs_get.EOF then resultado = rs_get("resultado")
	rs_get.Close
...
if Request("Accion") = "Agregar" then
  ...
  if Conn.Errors.Count = 0 then
     ...
     if Total_cif_ori >= Session("Maximo_cif_bov") and Session("Tipo_documento_venta") = "BOV" then
        MensajeError = "Boleta supera los US$ " & Session("Maximo_cif_bov") & " costo CIF"
        Conn.RollbackTrans
     elseif Total_cif_ori >= Session("Maximo_cif_fav") and Session("Tipo_documento_venta") = "FAV" then
     ...
     if Session("Cliente_boleta") <> "CLI_BOLETA" and MensajeError="" then
       ...
       if v_tipEntComer = "C" or (v_tipEntComer = "A" and v_clasDeEmpleado = "") then  
          'S lo se pueden aplicar descuentos de mayoristas a Clientes
          ...
          if (v_temporada="XMAY" or v_temporada="PATG" or v_temporada="NOV" or v_temporada="CHAO" or v_temporada="500A OS") and v_tipo_lista_precio="LXMAY" then 'Es producto Mayorista y el cliente est  asociado a una lista mayorista
            
	          v_num_int_DVT_activa = Get_Numero_Interno_DVT_Activa_X_Login(Session("Login"))
	          session("producto_eliminado") = Get_Cantidad_productos_eliminados_DVT_Activa_X_Login(Session("Login"))
...
'Rescatando el  ltimo producto ingresado
...
sql = "exec MOP_actualiza_descripcion_KITAR '" & Session("Login") & "'"
conn.execute(sql)
...
For n = 1 To Detalle_lineas
  ...
  if temporada = "XMAY" or temporada = "PATG" or temporada="NOV" or temporada="CHAO" or temporada="500A OS" then 
	...
	'####################################################################################################################
	'No se debe hacer mod 10 cuando los productos son KIT porque a veces los precios unitarios est n con pesos (casos de $195 0 $265)
	'if Ucase(trim(Detalle_KIT(n))) = "N" then Precio_final = cdbl(Precio_final) - cdbl(Precio_final Mod 10) 'MODIFICACION
	'Se coment  la l nea porque se considerar  mostrar los pesos al igual que en el listado de productos (pagos)
	'####################################################################################################################
...
if Detalle_lineas > 0 then
  if Es_Kit then 'No se encontr  el  ltimo producto ingresado puede ser un KIT o el ultimo eliminado fue un KIT
...
<div style="position:absolute; top:194px; left:<%=560-delta%>px; z-index:1">
   ...
   <%if numero_de_lineas >= 12 and Session("Tipo_documento_venta") = "FAV"  then%>
...
<%if Session("Tipo_documento_venta") = "FAV" then response.write "Factura" else response.write "Bol:" & session("Boleta_Actual")%><br>
...
{
	alert("No se puede agregar mas lineas a la factura (M ximo 12). Por Favor Terminar Venta, que tenga un buen dia.");
	Pagos();
}
