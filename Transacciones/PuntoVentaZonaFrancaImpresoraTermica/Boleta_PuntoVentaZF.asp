...
Session("flagFechaBoleta") = false
IF Session("Login") = "15309968" or Session("Login") = "13971954" or Session("Login") = "15308847" or Session("Login") = "17387039" THEN
	Session("flagFechaBoleta") = true
END IF
...
Session("Nro_caja") = 0
Session("Fecha_boleta") = strzero(day(now()),2)&"/"&strzero(month(now()),2)&"/"&strzero(year(now()),4) & " " & mid( now(), 12, 8 )
if ( Request("Accion") = "Cliente" or Request("Accion") = "Iniciar" ) and len(trim(Request("Accion"))) > 0 then
   ...
   Session("Fecha_boleta") = Request("Fecha_boleta")
   Session("Nro_caja") = Request("Nro_caja")
end if
...
Session( "FechaHoy" ) = cambia_fecha( mid( Now(),  1, 10 ) )
Session( "FechaInicioDoctoDDC" ) = ""
cSQL = "Exec PAR_ListaParametros 'PTOVTADDC'"
Set rs = Conn.Execute ( cSQL )
If Not rs.Eof then
    Session( "FechaInicioDoctoDDC" ) = cambia_fecha( mid( rs("Valor_Fecha"),  1, 10 ) )
end if
rs.Close
set rs = Nothing
...
if Conn.Errors.Count > 0 then
   MensajeError = "Error al iniciar proceso de venta, debe llamar a asistencia t�cnica"
end if
...
if Numero_interno_DVT_activa > 0 and Request("Imprimiendo") & "*" <> "Si*" then
   ...
   if not rs.eof then
      ...
      if rs("Documento_valorizado") = "BOV" then
         'response.write " a�n sin liberar"
         lReimprimir = true
      end if
   end if
   rs.close
...
if Session("Cliente_boleta") <> Request("Cliente_boleta_anterior") then  
   ...
   if not rs.eof then
      ...
      elseif left(Session("Cliente_boleta"),1)= " " then
        MensajeError	                        = "No se debe anteponer un valor vac�o en el Rut"
      else
...
'Verifica si la caja (IP) est� autorizada para vender con descuento de personal 
if session("tipo_entidad_comercial") = "E" then
  ...
  if rs.EOF then
    MensajeError = "Esta caja no est� autorizada para realizar descuento de personal"
    ControlFoco = "Cliente_boleta"
  end if
  rs.close
  set rs = nothing
...
IP_EQUIPO = Request.ServerVariables("REMOTE_ADDR")
Set rs = Conn.execute("EXEC CAJ_Validar_venta_segun_apertura '" & IP_EQUIPO  & "','" & trim(Session("Login")) & "'" )
if not rs.eof then
  if rs("error") = 1 then
    MensajeError  = rs("Mensaje")
...
if ( Request("Accion") = "Iniciar" or Ucase(Trim(Session("Cliente_boleta"))) = "CLI_BOLETA" ) and len(trim(Request("Accion"))) > 0 then
   ...
   if len(trim(Direccion_cliente)) = 0 then
      MensajeError	= "Debe ingresar la direcci�n del cliente"
   ...
   Set rs = Conn.execute( "EXEC PUN_Valor_paridad_dia_actual '" & FechaBoleta & "'" )
   if rs.eof then
   ...
   set rs = nothing
    ...
    IF true THEN
		IF Session("flagFechaBoleta") Then
			Set rsDolar = Conn.execute( "EXEC DCA_RescataUltimoDolarCambio  '" & FechaBoleta & "'" )
		ELSE
  ...
  'Verificar que el responsable es un empleado (Validaci�n s�lo para cajas que TIENEN AUTORIZACION para modificar el responsable)
  if Permite_cambio_responsable then 'Computaci�n (Digitaci�n) y Supervisoras
    if request("Login_punto_venta") = "" then
      ...
      z_strSQL="select entidad_comercial from entidades_comerciales with(nolock) where empresa='SYS' and entidad_comercial = '"&rut_responsable&"' and Tipo_entidad_comercial='E' and (Clasificacion_de_empleado like 'CAJER%' or Clasificacion_de_empleado like 'JMod%' ) "
      set rs = Conn.Execute(z_strSQL)
      if rs.EOF then
  ...
  if len(trim(MensajeError)) = 0 then
   ...
   ' M�ximo descuento a la cabecera
   ...
   ' Valores CIF m�ximos de boletas
...
clienteIP = Request.ServerVariables("REMOTE_ADDR")
z_strSQL = "EXEC CAJ_Revisa_Apertura '" & clienteIP & "'"
set rs = Conn.Execute(z_strSQL)
If Not rs.EOF Then
    ' Comprobar si el valor retornado es 0 (indicando un error)
    If rs.Fields(0).Value = 0 Then
        MensajeError = "Debe iniciar periodo en aperturas de cajas"
        ControlFoco = "No_apertura"
    End If
End If
rs.close
set rs = nothing
...
<%=CajaConBordes(110,210,550,295)%>

<div style="position:absolute; top:145px; left:230px; z-index:1"  style="font-size: 15px; font-family: sans-serif;font-weight: bold; line-height: 150%;">
Documento<br>
Cliente<br>
...
<div style="position:absolute; top:145px; left:390px; z-index:1"  style="font-size: 16px; font-family: sans-serif;font-weight: bold; line-height: 150%;">
     &nbsp;<select class="FuenteInput" name="Tipo_documento_venta" >
...
<%rs.movenext
	...
	<% if Session("flagFechaBoleta") then %>
	<span style='font-size: 25px;'>&nbsp;</span><input id="Fecha_boleta" type="text" name="Fecha_boleta" size="20" maxlength="20" value="<% = Session("Fecha_boleta") %>"> <br>
	<!-- <span style='font-size: 25px;'>&nbsp;</span><input id="Nro_caja" type="text" name="Nro_caja" size="10" maxlength="10" value="<% = Session("Nro_caja") %>"> <br> -->
	<input id="Nro_caja" type="hidden" name="Nro_caja" value="0"> <br>
	<% else %>
	<input id="Fecha_boleta" type="hidden" name="Fecha_boleta" value="<% = strzero(day(now()),2)&"/"&strzero(month(now()),2)&"/"&strzero(year(now()),4) %>"> <br>
	<input id="Nro_caja" type="hidden" name="Nro_caja" value="0"> <br>
	<% end if %>
...
<div style="position:absolute; top:380px; left:220px; width=800px; z-index:1; color=#ff0000; font-weight: bold; font-family: Verdana, Sans Serif, Arial, Helvetica; font-size: 18px; text-decoration: blink;"><%=MensajeError%></div>
...
//alert ( 6 )
         ...
         <%elseif ControlFoco = "No_apertura" then%>
         if (document.attachEvent) {
            window.attachEvent("onload", deshabilitarFormulario);
         } else {
            window.addEventListener("load", deshabilitarFormulario);
         }
        <%else%>
//alert ( 7 )
            document.Formulario.Comuna_cliente.focus();
