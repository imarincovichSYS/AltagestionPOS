...
<div style="position:absolute; overflow:auto; top:145px; left:<%=28-delta%>px;  z-index:2; width=<%=ancho_caja_bg_derecha%>px; height=126px; background-color: #999966;" >
  <table border="0" cellspacing="1" cellpadding="0">
    ...
    if Session("xBloqueado") = "S" Then 
        ...
        cSql = "EXEC PUN_Documentos_de_cobranza '" & Session("empresa_usuario") & "','" & Session("Cliente_boleta") & "'"
    end if
    ...
    do While Not Rs.EOF
      ...
      if Rs("documento") = "EXP" then
       ...
       if (rs("documento") = "UEI") or (rs("documento") = "TAR") or (rs("documento") = "TAC") or (rs("documento") = "TAM") then
        agrega_tipo_cobro = false
      end if
    ...
    for n = 1 to Numero_documentos_cobro
      ...
      <tr bgcolor="<%=ColorLinea%>" class="FuenteInput">
        <td width="20" align="center"><%=n%>
          ...
          <input type=hidden name="NumeroSeleccionado_<%=n%>" value="<%=Numero_documento_cobro(n)%>">
        </td>
        <td>&nbsp;
          <%IF Session( "FechaHoy" ) >= Session( "FechaInicioDoctoDDC" ) THEN %>
            <select class="FuenteInput" name="Tipo_documento_cobro_<%=n%>" style="width:150" Onchange="if( this.value == 'DDC;0;0;0;' ) { document.getElementById('Monto_documento_cobro_<%=n%>').value = 0;    };javascript:fVerificaDoc(this)" qonchange="submitform()">
          <%ELSE %>
            <select class="FuenteInput" name="Tipo_documento_cobro_<%=n%>" style="width:150" Onchange="if( this.value == 'UEI;0;0;0;' ) { document.getElementById('Monto_documento_cobro_<%=n%>').value = 0;    };javascript:fVerificaDoc(this)" qonchange="submitform()">
          <%END IF 
          ...
          for u = 1 to Documentos_cobranza
            ...
            if Documento_cobranza(u) <> "TAR" then
              if Documento_cobranza(u) <> "TAC" then
                if Seleccionado_cobranza(u) = "n" then 'and Total_por_pagar > Saldo_a_favor_cobranza(u) then
                  ...
                  if Instr(1, Economato(u), Session("xEconomato") ) > 0 then 
                    ...
                    <%else %>
                      <option value='<%=Documento_cobranza(u)&";"&Saldo_a_favor&";"&Numero_interno_documento & ";" & Numero_documento_cobranza(u) & ";" & Economato(u)%>' <%if Tipo_documento_cobro(n) = Documento_cobranza(u) then response.write "selected"%>><%=Nombre_cobranza(u)%>&nbsp;(<%=Documento_cobranza(u)%>)</option>
                    <%end if %>
                  <%elseif InStr(1, session("Economatos"), Documento_cobranza(u) ) > 0 and Session("xEconomato") <> "" then %>
                  <%else
                    if cDbl(Numero_interno_documento_cobranza(u)) > 0 then
                      if cDbl(Numero_documento_cobranza(u)) = cDbl(Numero_documento_cobro(n)) then 
                        flagSelected = true %>
                        <%if false then %>
                          <option value='<%=Documento_cobranza(u)&";"&Saldo_a_favor&";"&Numero_interno_documento & ";" & Economato(u)%>' <%if Tipo_documento_cobro(n) = Documento_cobranza(u) then response.write "selected"%>><%=Nombre_cobranza(u)%>&nbsp;(<%=Documento_cobranza(u)%>)</option>
                        <%else %>
                      ...
                      <%else %>
                        <%if false then %>
                          <option value='<%=Documento_cobranza(u)&";"&Saldo_a_favor&";"&Numero_interno_documento & ";" & Economato(u)%>' <%if Tipo_documento_cobro(n) = Documento_cobranza(u) then response.write ""%>><%=Nombre_cobranza(u)%>&nbsp;(<%=Documento_cobranza(u)%>)</option>
                        <%else %>
                          <option value='<%=Documento_cobranza(u)&";"&Saldo_a_favor&";"&Numero_interno_documento & ";" & Numero_documento_cobranza(u) & ";" & Economato(u)%>' <%if Tipo_documento_cobro(n) = Documento_cobranza(u) then response.write ""%>><%=Nombre_cobranza(u)%>&nbsp;(<%=Documento_cobranza(u)%>)</option>
                        <%end if %>
                      <%end if
                    else %>
                      <%if false then %>
        ...
        <td>
          &nbsp;<input onkeypress="return Valida_Digito(event);" Class="FuenteInput" type="text" id="Monto_documento_cobro_<%=n%>" name="Monto_documento_cobro_<%=n%>" size="12" maxlength="10" value="<%=Monto_documento_cobro(n)%>" onchange="submitform()">
          ...
          <%if Tipo_documento_cobro(n) = "UEI" OR Tipo_documento_cobro(n) = "DDC" then
          ...
          if (Tipo_documento_cobro(n) = Documento_para_vuelto OR Tipo_documento_cobro(n) = "DDC") and Vuelto > 0 then
            if Billete <> "" then
        ...
        <%if Tipo_documento_cobro(n) <> "DEI" and Tipo_documento_cobro(n) <> "NCVs" and Tipo_documento_cobro(n) <> "EFI" and Tipo_documento_cobro(n) <> "UEI" and Tipo_documento_cobro(n) <> "DDC" and Tipo_documento_cobro(n) <> "TBK" then%>
          <td align=center>&nbsp;
            <%if Tipo_documento_cobro(n) = "TMU" and not PrimerTMU then%>
              <input onkeypress="return Valida_Caracteres_Fecha(event);" Class="FuenteInput" type="text" id="Fecha_documento_cobro_<%=n%>" name="Fecha_documento_cobro_<%=n%>" size="12" maxlength="10" value="<%=Fecha_documento_cobro(n)%>" onBlur="DateFormat(this,this.value,event,false,'3')" onchange="submitform()">
            ...
            elseif Tipo_documento_cobro(n) = "TMU" and PrimerTMU then%>
              ...
              <%else %>
                <input onkeypress="return Valida_Caracteres_Fecha(event);" Class="FuenteInput" type="text" id="Fecha_documento_cobro_<%=n%>" name="Fecha_documento_cobro_<%=n%>" size="12" maxlength="10" value="<%=Fecha_documento_cobro(n)%>" onBlur="DateFormat(this,this.value,event,false,'3')">
              <%end if%>
            <%end if%>
          &nbsp;</td>
          <td>&nbsp;
            <%if InStr(1, session("Economatos"), Tipo_documento_cobro(n) ) = 0 then 
              if Tipo_documento_cobro(n) <> "EXP" then%>   
                <select Class="FuenteInput" name="Banco_documento_cobro_<%=n%>" style="width:300" >
                  <%for u = 1 to Bancos
          ...
          <td>&nbsp;
            <input onkeypress="return Valida_Digito(event);" Class="FuenteInput" type="text" name="Numero_documento_cobro_<%=n%>" size="12" maxlength="10" value="<%=Numero_documento_cobro(n)%>">
          </td>
        <%elseif Tipo_documento_cobro(n) = "DEI" or Tipo_documento_cobro(n) = "NCV" then%>
        <%else%>
          <td colspan="3">&nbsp;</td>
        <%end if
        if Tipo_documento_cobro(n) <> "DEI" and Tipo_documento_cobro(n) <> "NCVs" and Tipo_documento_cobro(n) <> "EFI" and Tipo_documento_cobro(n) <> "UEI" and Tipo_documento_cobro(n) <> "DDC" and n = Numero_documentos_cobro and Tipo_documento_cobro(n) <> "TBK" then%>
          <td><input onkeypress="return Valida_Digito(event);" Class="FuenteInput" type="text" name="Numero_documentos_adicionales" size="3" maxlength="2" value="1" onchange="agrega_cheques()"></td>
        <%else%>
          <td></td>
        <%end if%>
