...
if Session("flagFechaBoleta") then
	sql="exec DOV_Graba_pago_documento_venta '" & Session("empresa_usuario")              & "', 0" & +_
							...
							Valor_paridad_moneda_oficial & ",'" & left(Slct_DocDescpacho,2) & "', 'NO'"
else ' version normal
	sql="exec DOV_Graba_pago_documento_venta '" & Session("empresa_usuario")              & "', 0" & +_
							Numero_documento & ", '"  & +_
							Slct_DocDescpacho & "', '" & +_
...
for n = 1 to Numero_documentos_cobro
	...
	if aDocumento_cobro(0) = "TBK" then
		cCadenaDoctoCobros = cCadenaDoctoCobros & "'" & aDocumento_cobro(0) & "',"
		cMontos_a_pagar = cMontos_a_pagar & Monto_a_Pagar & ","
	end if
next
cCadenaDoctoCobros = left(cCadenaDoctoCobros,len(cCadenaDoctoCobros)-1)
cMontos_a_pagar = left(cMontos_a_pagar,len(cMontos_a_pagar)-1)
'--// 
'--// =============================================================================================================================================
	...
	if Error = "N" then
		...
		if Slct_DocDescpacho = "BOV" then
			...
			'if instr(0,cCadenaDoctoCobros,"TBK") > 0 and len(cCadenaDoctoCobros) > 0 then 
			if len(trim(cCadenaDoctoCobros)) > 0 then
				response.write("<s" & "cript>")
				response.write("window.location='Pago_transbank_PuntoVentaZF.asp?Folio_disponible=0&DocumentoZF=BOV&DocumentosCobros=" & replace(cCadenaDoctoCobros,"'","""") & "&Montos_a_pagar=" & cMontos_a_pagar & "&Numero_documento_valorizado=" & Numero_documento_valorizado_DOCVTA & "&Numero_documento_no_valorizado=" & Numero_documento_no_valorizado & "';")
				response.write("</s" & "cript>")
				response.end()
			else 
			...
			if Slct_DocDescpacho = "FAV" then
				if len(trim(cCadenaDoctoCobros)) > 0 then
					response.Write("<s" & "cript>")
					response.write("window.location='Pago_transbank_PuntoVentaZF.asp?Folio_disponible=" & Folio_disponible & "&DocumentoZF=FAV&Numero_interno_documento_Factura_ingresada=" & Numero_interno_documento_Factura_ingresada & "&Numero_documento_Factura_ingresada=" & Numero_documento_Factura_ingresada & "&DocumentosCobros=" & replace(cCadenaDoctoCobros,"'","""") & "&Montos_a_pagar=" & cMontos_a_pagar & "&Numero_documento_valorizado=" & Numero_documento_valorizado_DOCVTA & "&Numero_documento_no_valorizado=" & Numero_documento_no_valorizado & "';")				
					response.Write("</s" & "cript>")
					response.End()
				else
				...
				response.redirect("Main_PuntoVentaZF.asp?Liberar=1")
				response.End()	
			end if
		end if
	else
