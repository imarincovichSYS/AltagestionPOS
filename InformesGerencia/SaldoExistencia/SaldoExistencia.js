function Mostrar_SaldoExistencia(v_async, v_busqueda_aprox, jlinea) {
    strCargando = "<br><br><center><img src='" + RutaProyecto + "imagenes/new_loader.gif' width='32' height='32' "
    strCargando += "border='0' align='top'><br><br><font color='#848284'><b>Buscando informe...</b></font><br><br></center>"

    grilla_carpetas.innerHTML = strCargando

    document.body.style.cursor = "wait"
    Fecha_desde.disabled = true
    Fecha_desde.style.backgroundColor = "#EEEEEE"

    bot_buscar.disabled = true
    SetBackgroundImageInput(bot_buscar, RutaProyecto + "imagenes/ico_buscar_24X24_off.gif")

    var ajx = new Ajax("Mostrar_SaldoExistencia.asp", {
        method: 'post',
        async: eval(v_async),
        data: 'busqueda_aprox=' + v_busqueda_aprox + '&Fecha_desde=' + Fecha_desde.value,
        ...
        onComplete: function (respuesta) {
            ...
            if (respuesta == "")
                Cancelar_SaldoExistencia(jlinea)
            else {
                if (jlinea) {
...
function Exportar_SaldoExistencia() {
    par = ""; //'Fecha_desde=' + Fecha_desde.value;
    servidor = Servidor.value;
    TipoInforme = window.parent.frames[1].document.getElementById("Informe").value;
    Agrupado = window.parent.frames[1].document.getElementById("xgrupado").value;
    par += "servidor=" + servidor + "&TipoInforme=" + TipoInforme + "&Agrupado=" + Agrupado;
    if (servidor == "P") { ruta_informe = "Exportar_SaldoExistencia"; }
    if (servidor == "Replica") { ruta_informe = "Exportar_SaldoExistencia"; }
    if (servidor == "Test") { ruta_informe = "Exportar_SaldoExistencia"; }
    //alert(ruta_informe);
	...
	//alert(ruta_informe + ".asp?" + par);
    WinInforme = open(ruta_informe + ".asp?" + par, "Inf", str, "replace=1")
}

function Cancelar_SaldoExistencia(jlinea) {
    Fecha_desde.disabled = false
    Fecha_desde.style.backgroundColor = ""
...
function Cancelar_Busqueda_Carpeta(jlinea){
