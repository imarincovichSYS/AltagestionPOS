...
sql = " SELECT  superfamilia = Isnull(F.Superfamilia,0), NetoDolar = Isnull(A.NetoDolar,0), totcia = Isnull(A.totcia,0), TotCpa= Isnull(A.TotCpa,0), " &_ 
      ...
      " DIH_1 = sum(imp_dih), " &_
    	...
    	" from _estadistica1  " &_
    	" where convert(varchar(10),fecha_movimiento,103) = '" & fecha_Hasta & "' group by superfamilia) B, " &_
    	" (SELECT superfamilia,  " &_
    	...
    	"      end " &_
    	" from _estadistica1 " &_ 
    	" where convert(varchar(10),fecha_movimiento,103)='" & cdate(fecha_Hasta) -1 & "' group by superfamilia) C, " &_
    	" (select Periodo, Superfamilia, Meta  from metas where periodo = '"&periodo&"') D, " &_
    	" (select Periodo, Superfamilia, Total, Transito_mes = isnull(Transito_mes,0), Transito_mes_mas_uno = isnull(Transito_mes_mas_uno,0), Transito_mes_mas_dos = isnull(Transito_mes_mas_dos,0), " &_ 
      " transito_futuro = isnull(transito_futuro,0)  from compras where periodo = '"&periodo&"') E, " &_
      " (select Periodo, Superfamilia, IsNull(Exi,0) as Exi  from Existencias where periodo = '"&periodo&"') F, " &_
      " (select Superfamilia, Nombre from superfamilias) G, " &_
      " (select Rubro, porcentaje  from proyectado_ano_anterior where periodo = '"&periodo_1&"' and dia =" & day(fecha_Hasta) & " ) H " &_      
      " where F.superfamilia *= A.superfamilia " &_ 
      " and F.superfamilia *= b.superfamilia " &_
    	" and F.superfamilia *= c.superfamilia " &_
    	" and F.superfamilia *= d.superfamilia " &_
    	" and F.superfamilia *= e.superfamilia " &_
    	" and F.superfamilia *= G.superfamilia " &_
    	" and F.superfamilia *= H.Rubro " & concatena_sql    	
