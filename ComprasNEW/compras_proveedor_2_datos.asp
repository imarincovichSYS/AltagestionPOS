...
strSQL="select A.proveedor, B.apellidos_persona_o_nombre_empresa nombre, B.codigo_postal codprov from " &_
       "(select proveedor from "&nom_tabla&" with(nolock) where " &_
       ...
       "(select entidad_comercial, apellidos_persona_o_nombre_empresa, codigo_postal from entidades_comerciales with(nolock) where empresa='SYS') B " &_
