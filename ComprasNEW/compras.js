...
// function Set_Total_CIF_ADU_From_Facturamayors_Carpeta(x_carpeta)
...
var C_PROD_PROV = 33,   C_PROV_ORI    = 31,   C_COD_CAJA    = 32,   C_COD_ANTIGUO = 33
var C_CANT_ORI  = 34,   C_UN_ORI      = 35,   C_EX_FAB      = 36,   C_TOT_EX_FAB  = 37
//var C_FOB             = 38,   C_TOT_FOB       = 39

var C_PROD_PROV_VISTA_PEDIDO    = 7,  C_COD_CAJA_VISTA_PEDIDO   = 8
var C_COD_ANTIGUO_VISTA_PEDIDO  = 9,  C_PROV_ORI_VISTA_PEDIDO   = 10
var C_CANT_ORI_VISTA_PEDIDO     = 11, C_UN_ORI_VISTA_PEDIDO     = 12
var C_EX_FAB_VISTA_PEDIDO       = 13, C_TOT_EX_FAB_VISTA_PEDIDO = 14
...
function Get_Total_Cif_ORI_Y_ADU(){
  var ajx_get_total_cif_ori_y_adu = new Ajax(RutaProyecto+"comprasNEW/compras_get_total_cif_ORI_Y_ADU.asp",{
    method:'post', 
    async:false,
...
function Get_Total_Cif_ORI_ADU_EXFAB_FOB(){
  var ajx_get_total_cif_ori_adu_exfab_fob = new Ajax(RutaProyecto+"comprasNEW/compras_get_total_cif_ORI_ADU_EXFAB_FOB.asp",{
    method:'post', 
    async:false,
    data: 'documento_respaldo='+$("documento_respaldo").value+
    '&numero_documento_respaldo='+$("numero_documento_respaldo").value+
    ...
    onComplete: function(respuesta){
      ...
      for(k=1;k<=t_rows;k++)   //jaime
      {
        if($("documento_respaldo").value=="TU" || $("documento_respaldo").value=="DS")
        {
          v_TOTAL_COSTO_CIF_ORI = v_TOTAL_COSTO_CIF_ORI + parseFloat(Replace($("celda_" + v_num_grid + "_" + k + "_" + C_TOT_CIF_ORI).value,",",""))
        }
        //console.log(v_TOTAL_COSTO_CIF_ORI)
      }
      $("label_total_cif_ori").innerHTML = Formateo_Completo_Numero(parseFloat(v_TOTAL_COSTO_CIF_ORI).toFixed(2))
      Verificar_Totales_Costos();
...
function Terminar_Compra() {
    ...
    var documento_respaldo = document.getElementById("buscar_documento_respaldo");
	
    if (fecRecep == undefined) {fecRecep = ""; }
    if (fecha_recepcion.value == "") {
        alert("Falta la Fecha de Recepcion.\n\nEsta debe ser ingresada en Administracion de Carpetas.\n\nCarpeta: " + carpeta.value + "");
        return false;
    }
    else {
        if (fecRecep == "" && documento_respaldo.value != "TU") {
            alert("Este documento tiene Fecha de Recepcion, pero la Carpeta no la tiene.\n\nDebe actualizar la Fecha de Recepcion en Administracion de Carpetas.\n\nCarpeta: " + carpeta.value + "");
            return false;
        }
        else {
            if (fecha_recepcion.value != fecRecep && documento_respaldo.value != "TU") {
                alert("Este documento tiene Fecha de Recepcion distinta de la Fecha de Recepcion de la Carpeta.\n\nDebe actualizar la Fecha de Recepcion en Administracion de Carpetas.\n\nCarpeta: " + carpeta.value + "");
                return false;
            }
            else {
                if (confirm("¿Está seguro que desea finalizar la compra?.\n(Traspasar a RCP)")) {
                    if ($("bot_terminar_compra").getAttribute("advertencia") != "") {
                        Config_Msg_Accion(" Advertencia: " + $("bot_terminar_compra").getAttribute("advertencia") + "...", 5000, $("msg_accion_3"))
                        if (confirm("ADVERTENCIA: " + $("bot_terminar_compra").getAttribute("advertencia") + "\n¿Desea cancelar el ingreso de la compra RCP?\n(Para finalizar la compra haga clic en \"Cancelar\")"))
                            return;
                    }
                    document.body.style.cursor = "wait"
                    strCargando = "<br><br><br><br><br><br><center><img src='" + RutaProyecto + "imagenes/new_loader.gif' width='32' height='32' "
                    ...
                    var ajx = new Ajax("compras_grabar_RCP.asp", {
                        ...
                        onComplete: function (respuesta) {
                            ...
                            if (respuesta == "") {
                                ...
                                document.location.href = RutaProyecto + "comprasNEW/compras.asp"
...
function CambiarBodega(bodega){
  var ajx = new Ajax("compras_cambiar_bodega.asp",{
    method: "POST",
    async:false,
    data: 'numero_interno_documento_no_valorizado=' + $('numero_interno_documento_no_valorizado').value +
          '&bodega='+bodega,
    onComplete: function (respuesta) {
      Config_Msg_Accion(" Bodega asignada...", 5000, $("msg_accion_3"))
    }
...
function Actualizar_Datos_Compra(v_nom_campo, v_valor) {
    var Xcarpeta = document.getElementById("carpeta");
    var numero_documento_respaldo = document.getElementById("numero_documento_respaldo");
    var fecha_emision = document.getElementById("fecha_emision");
    var fecha_recepcion = document.getElementById("fecha_recepcion");
    var fecha_factura = document.getElementById("fecha_factura");
    var oBulto = document.getElementById("bultos");

    //alert(v_nom_campo + ' - ' + v_valor + ': ' + fecha_factura.value + ' - ' + numero_documento_respaldo.value + ' - ' + Xcarpeta.value + ' -- ' + fecha_recepcion.value);
    if (numero_documento_respaldo.value != "" && Xcarpeta.value != "") {
        ...
        var ajx4 = new Ajax("compras_grabar.asp", {
            ...
            onComplete: function (respuesta) {
                ...
                if (v_nom_campo == "numero_documento_respaldo") {
                    ...
                    if (v_nom_campo == "fecha_factura") {
                        var parts1 = fecha_emision.value.split('/');
                        var fechaADU = parts1[2] + parts1[1] + parts1[0] //new Date(parseInt(parts1[2]), parseInt(parts1[1]) - 1, parseInt(parts1[0]));
                        var parts2 = v_valor.split('/');
                        var fechaFAC = parts2[2] + parts2[1] + parts2[0] //new Date(parseFloat(parts2[2]), parseFloat(parts2[1]) - 1, parseFloat(parts2[0]));
                        if (fechaFAC > fechaADU) {
                            alert("La Fecha de Factura no puede ser mayor que la Fecha de Aduana.");
                            fecha_factura.value = "";
                        }
                        else {
        ...
        if (v_nom_campo == "fecha_emision" ) {
            if (fecha_recepcion.value == "") { fecha_recepcion.value = v_valor.toString(); }
        }
    }
}

function Cargar_Carpetas() {
    var anio = document.getElementById("buscar_anio");
    var documento_respaldo = document.getElementById("buscar_documento_respaldo");
...
function Asignar_NumAduana() {
    var objNumeroAduana = document.getElementById("numero_documento_respaldo");

    objNumeroAduana.setAttribute("disabled", "");
    if (numAduana != "" || Manifiesto != "") {
        var vCarpeta = document.getElementById("carpeta");
        var vDocumentoRespaldo = document.getElementById("documento_respaldo");
        var ajx = new Ajax("compras_numeros_aduanas.asp", {
            method: 'post',
            data: 'carpeta=' + vCarpeta.value + '&documento_respaldo=' + vDocumentoRespaldo.value + '&numAduana=' + numAduana + '&Manifiesto=' + Manifiesto,
            onComplete: function (respuesta) {
                arraError = respuesta.split(',');
                if (arraError[0] == "S") {
                    objNumeroAduana.value = numAduana;
                    objNumeroAduana.select();
                    objNumeroAduana.blur();
                }
                else {
                    objNumeroAduana.setAttribute("disabled", "disabled");
                    alert(arraError[1]);
                }
            }
        });
...
function Asignar_Carpeta() {
    var documento_respaldo = document.getElementById("documento_respaldo");
    var Anio = document.getElementById("anio");
    var numero_documento_respaldo = document.getElementById("numero_documento_respaldo");
    documento_respaldo.setAttribute("disabled", "");
    Anio.setAttribute("disabled", "");
    numero_documento_respaldo.setAttribute("disabled", "");

    var carpeta = document.getElementById("carpeta");
    var arrayCarpeta = carpeta.value.split('-');
    if (documento_respaldo.value == "DS") {arrayCarpeta[0] = documento_respaldo.value; }
    fecRecep = carpeta.options[carpeta.options.selectedIndex].getAttribute("fecha_recepcion");
    fecAduana = carpeta.options[carpeta.options.selectedIndex].getAttribute("fecha_aduana");
    numAduana = carpeta.options[carpeta.options.selectedIndex].getAttribute("numero_aduana");
    totalDocto = carpeta.options[carpeta.options.selectedIndex].getAttribute("total_documento");
    totalBulto = carpeta.options[carpeta.options.selectedIndex].getAttribute("totalBultos");
    Proveedor = carpeta.options[carpeta.options.selectedIndex].getAttribute("Proveedor");
    Manifiesto = carpeta.options[carpeta.options.selectedIndex].getAttribute("Manifiesto");
    //if (fecRecep == "") {fecRecep = fecAduana; }
    ...
    // Documento_respaldo
    var x = document.getElementById("documento_respaldo");
    for (i = 0; i < x.length; i++) {
        if (arrayCarpeta[0] == x.options[i].value) {
            x.selectedIndex = i;
            break;
        }
    }
    ...
    for (i = 0; i < x.length; i++) {
        if (arrayCarpeta[1] == x.options[i].value) {
            x.selectedIndex = i;
            break;
        }
    }
    ...
    var objFechaRecepcion = document.getElementById("fecha_recepcion");
    if (documento_respaldo.value == "Z") {
        objFechaRecepcion.value = fecRecep;

        var objFechaAduana = document.getElementById("fecha_emision");
        objFechaAduana.value = fecAduana;
        Actualizar_Datos_Compra("fecha_emision", fecAduana)

        var objMonto_adu_US = document.getElementById("monto_adu_US$");
        objMonto_adu_US.value = totalDocto;

        var objBultos = document.getElementById("bultos");
        objBultos.value = totalBulto;

        var objProveedor = document.getElementById("entidad_comercial");
        objProveedor.value = Proveedor;
    }

    //if (arrayCarpeta[0] == "TU") { objFechaRecepcion.disabled = false; }
    //else { objFechaRecepcion.disabled = true; }

    Cargar_Paridades_X_Fecha();

    numero_documento_respaldo.focus();
    numero_documento_respaldo.select();
    numero_documento_respaldo.click();
}

function Set_Total_CIF_ADU_From_Facturas_Carpeta(x_carpeta){
    var ajx = new Ajax(RutaProyecto + "comprasNEW/compras_get_total_cif_adu_from_facturas_carpeta.asp", {
    method:'post', 
    async:false,
...
function Cargar_Paridades_X_Fecha() {
    ...
    if (fecha_recepcion.value != "") { fechaBuscar = fecha_recepcion.value; }
    else if (fecha_factura.value != "") { fechaBuscar = fecha_factura.value; }
    else { fechaBuscar = fecha_emision.value; }

    var ajx = new Ajax(RutaProyecto + "funciones_SYS_AJAX/AJAX_Get_Paridad_X_Fecha.asp", {
        method: 'post',
        async: false,
        data: 'fecha=' + fechaBuscar,
        onComplete: function (respuesta) {
            var paridades = respuesta.split("~")
            $("label_paridad_para_facturacion").innerHTML = paridades[0]
            $("paridad_margen").value = paridades[1]
            if (parseInt(paridades[0]) > 0) {
                Actualizar_Datos_Compra("paridad_conversion_a_dolar", paridades[0])
            }
            else { alert("No hay paridad para el dia " + $("fecha_factura").value); }
        }
    });
  ajx.request();
}
...
function Grabar_Compra_Inicial(){
  ...
  //se deben cargar la compra del numero de respaldo y configurar el formulario como si se estuviera
    //editando la compra
    var Xcarpeta = document.getElementById("carpeta");
    var Xdocumento_respaldo = document.getElementById("documento_respaldo");
    if (Xcarpeta.value == "" || Xdocumento_respaldo.value == "") {
        alert("Debe indicar la Carpeta y el Nro. de Aduana.");
    }
    else {
        document.body.style.cursor = "wait"
                    ...
                    if (confirm("¿Está seguro que desea grabar esta compra?")) {
                        ...
                        var ajx1 = new Ajax("compras_grabar.asp", {
                            ...
                            onComplete: function (respuesta) {
                                ...
                                var oMonto_neto_US = document.getElementById("monto_neto_US$");
                                var oMonto_adu_US = document.getElementById("monto_adu_US$");
                                var oFecha_factura = document.getElementById("fecha_factura");
                                var oNumero_factura = document.getElementById("numero_factura");
                                var oBultos = document.getElementById("bultos");
                                var oEntidad_comercial = document.getElementById("entidad_comercial");

                                Actualizar_Datos_Compra('monto_neto_US$', oMonto_neto_US.value)
                                Actualizar_Datos_Compra('monto_adu_US$', oMonto_adu_US.value)
                                Actualizar_Datos_Compra('fecha_factura', oFecha_factura.value)
                                Actualizar_Datos_Compra('numero_factura', oNumero_factura.value)
                                Actualizar_Datos_Compra('bultos', oBultos.value)
                                Actualizar_Datos_Compra('proveedor', oEntidad_comercial.value)
...
function Mostrar_Form_Compra(v_band){
    var Xdocumento_respaldo = document.getElementById("buscar_documento_respaldo");
    ...
    if (v_band) {
        ...
        if (Xdocumento_respaldo.value == "TU" || Xdocumento_respaldo.value == "DS") {
            $("td_cab_1_1").style.visibility = "hidden"
            $("td_cab_1_2").style.visibility = "hidden"
        }
        ...
        if (Xdocumento_respaldo.value == "Z") {
            var objTotal_ex_fab = document.getElementById("total_ex_fab");
            objTotal_ex_fab.focus();
        }
    }
  else {
      if (true) {
          if (Xdocumento_respaldo.value == "TU") {
              $("td_cab_1_1").style.visibility = "hidden"
              $("td_cab_1_2").style.visibility = "hidden"
          }
        ...
        var oDocumento_respaldo = document.getElementById("documento_respaldo");
        var oAnio = document.getElementById("anio");
        var oFecha_recepcion = document.getElementById("fecha_recepcion");
        var oFecha_emision = document.getElementById("fecha_emision");
        var oNumero_documento_respaldo = document.getElementById("numero_documento_respaldo");
        oDocumento_respaldo.setAttribute("disabled", "disabled");
        oAnio.setAttribute("disabled", "disabled");
        oFecha_recepcion.setAttribute("disabled", "disabled");
        oFecha_emision.setAttribute("disabled", "disabled");
        oNumero_documento_respaldo.setAttribute("disabled", "disabled");
    }
  }
}

function Editar_Compra(modo) {
  ...
  if(Radio_Es_Arreglo(document.getElementsByName("radio_compras"))) {
    for(i=0;i<=document.getElementsByName("radio_compras").length-1;i++)
      if(document.getElementsByName("radio_compras")[i].checked==true) { 
        ...
        v_fecha_aduana                            = document.getElementsByName("radio_compras")[i].getAttribute("fecha_aduana")
        break;
      }
  }
  else {
    ...
    v_fecha_aduana                            = document.getElementsByName("radio_compras").getAttribute("fecha_aduana")
}
  ...
  $("fecha_emision").value = v_fecha_aduana
  $("fecha_emision").setAttribute("o_value", v_fecha_aduana)
  ...
  if(v_proveedor!="") {
    ...
    if(v_documento_no_valorizado=="RCP") {
...
$("td_cab_2_10").style.visibility = ""; // "hidden"
  $("fieldset_items").style.visibility = "hidden"
  if(v_documento_no_valorizado=="TCP") {
    $("label_accion_modulo").innerHTML         = "EDITANDO COMPRA - DA"
  ...
  if(Radio_Es_Arreglo(document.getElementsByName("radio_compras"))) {
    for(i=0;i<=document.getElementsByName("radio_compras").length-1;i++)
      if(document.getElementsByName("radio_compras")[i].checked==true) {      
        ...
        v_fecha_aduana = document.getElementsByName("radio_compras")[i].getAttribute("fecha_aduana")
        v_Manifiesto = document.getElementsByName("radio_compras")[i].getAttribute("Manifiesto")
        v_NDP = document.getElementsByName("radio_compras")[i].getAttribute("NDP")
        break;
      }
  }
  else {
    ...
    v_fecha_aduana = document.getElementsByName("radio_compras").getAttribute("fecha_aduana")
    v_Manifiesto = document.getElementsByName("radio_compras").getAttribute("Manifiesto")
    v_NDP = document.getElementsByName("radio_compras").getAttribute("NDP")
}
    ...
    parametros += "&fecha_aduana=" + v_fecha_aduana;
    parametros += "&Manifiesto=" + v_Manifiesto;
    parametros += "&NDP=" + v_NDP;
...
function Get_CPA_Producto_Anterior_a_Fecha(x_producto,x_fecha_recepcion){
    var ajx = new Ajax(RutaProyecto + "comprasNEW/compras_get_cpa_anterior_producto.asp", {
    method:'post', 
    async:false,
...
function Cargar_Items(v_accion, v_num_linea) {
  ...
  fecEmision = $("fecha_emision").value;
  if (v_accion == "NUEVO") {
      if (fecEmision == "") {
          //------------------------------------------
          var Fecha = new Date();
...
var ajx = new Ajax("compras_productos_grilla.asp", {
    ...
    '&fecha_emision=' + fecEmision + //$("fecha_emision").value +
    '&documento_no_valorizado=' + $("documento_no_valorizado").value +
    '&numero_documento_no_valorizado=' + $("numero_documento_no_valorizado").value +
    '&documento_respaldo=' + $("documento_respaldo").value +
    '&numero_documento_respaldo=' + $("numero_documento_respaldo").value +
    ...
    if(confirm(v_msg))
    {
      ...
      var ajx = new Ajax("compras_productos_grabar.asp", {
        ...
        onComplete: function(respuesta){
          ...
          //$("label_total_cif_ori").innerHTML=respuesta
          //return
          if(v_accion="eliminar_item")
              v_msg_accion  = respuesta
          //***************************************************************************************
          ...
          Config_Msg_Accion(v_msg_accion,6000,$("msg_accion_3"))
        }
      });
      ajx.request();
    }
...
function Set_Totales_Costos(){
  ...
  //alert(TOTAL_COSTO_EX_FAB_CABECERA)
  ...
  if($("documento_no_valorizado").value=="TCP" && t_rows>0)
  {
    ...
    else if($("documento_respaldo").value !="Z" && TOTAL_COSTO_CIF_ORI != TOTAL_COSTO_CIF_ORI_CABECERA)
    {
    ...
    else if($("documento_respaldo").value !="Z" && TOTAL_COSTO_CIF_ADU != TOTAL_COSTO_CIF_ADU_CABECERA)
    {
...
function Get_Totales_Costos_Chequea_Items_Para_Pasar_Compra_a_RCP(){
  ...
  for(k=1;k<=t_rows;k++)   //jaime
  {
  //if($("celda_" + v_num_grid + "_" + k + "_" + C_ITEMS_PADRE).value=="")
    //{  
    if($("documento_respaldo").value=="TU" || $("documento_respaldo").value=="DS")
    {
  ...
  //}
    if(v_BAND_ITEMS_OK_TO_RCP == true)
    {
      ...
      else if($("documento_respaldo").value=="Z" && v_ex_fab == 0)
      {
  ...
  var ajx1 = new Ajax("Get_superfamilia_x_producto.asp", {
      ...
      onComplete: function (respuesta1) {
          ...
          if (c_name == "unidad_de_medida_compra" || c_name == "cantidad_mercancias" || c_name == "cantidad_um_compra_en_caja_envase_compra" || c_name == "unidad_de_medida_consumo" || c_name == "cantidad_x_un_consumo") {
              ...
              if (!$("checkbox_vista_pedidos").checked) {//Vista Normal
                  col_tmp = C_CANT_M + y_col_todos_1
                  v_cant_m = parseFloat(Replace($("celda_" + v_num_grid + "_" + c_row + "_" + col_tmp).value, ",", ""))
                  col_tmp = C_CANT_X_M + y_col_todos_1
                  ...
                  if (v_cantidad > 0) {
                      ...
                      if (col_dif == 2) {
                          if ($("documento_respaldo").value == "DS" || $("documento_respaldo").value == "TU"){
                            c_cant_x_un = parseFloat(Replace($("celda_" + v_num_grid + "_" + c_row + "_8").value, ",", ""))
                            if( c_cant_x_un != v_cant_x_un || $("documento_respaldo").value == "TU")
                          ...
                          v_cif_ori = roundNumber(v_total_cif_ori / v_cantidad, cant_decimales_cif_ori)
                      ...
                      v_cif_adu = roundNumber(v_total_cif_adu / v_cantidad, cant_decimales_cif_ori)
                      ...
                      v_costo_flete_transporte = 93.33
              ...
              if (col_dif == 2) {
                  ...
                  col_tmp = C_TOT_CIF_ORI + y_col_todos_1
                  v_total_cif_ori = parseFloat(Replace($("celda_" + v_num_grid + "_" + c_row + "_" + col_tmp).value, ",", ""))
                  v_cif_ori = roundNumber(v_total_cif_ori / v_cantidad, 4)
                  c_name2 = "costo_cif_ori_us$"
                  c_value2 = v_cif_ori
                  col_tmp = C_CIF_ORI
                  v_cif_ori = roundNumber(v_cif_ori, cant_decimales_cif_ori)
              ...
              if ($("checkbox_vista_pedidos").checked)
                  col_tmp = C_CANT_ORI_VISTA_PEDIDO
              v_cantidad = parseFloat(Replace($("celda_" + v_num_grid + "_" + c_row + "_" + col_tmp).value, ",", ""))
              if (v_cantidad == 0) {
                  alert("La cantidad de origen debe ser mayor que 0")
                  Config_Msg_Accion(" La cantidad de origen debe ser mayor que 0...", 5000, $("msg_accion_3"))
          ...
          */
          ...
          if ($("checkbox_vista_pedidos").checked)
              v_vista = "PEDIDO"
          document.body.style.cursor = "wait"
          var ajx = new Ajax(RutaProyecto + "comprasNEW/" + v_url, {
              method: 'post',
              async: false,
                  ...
                  if (nom_tabla == "entidades_comerciales") {
                      ...
                      if (c_name == "producto") {
                          if (respuesta != "NO_EXISTE") {
                              ...
                              col_tmp = C_ITEMS + y_col_todos_1
                              cell_item_final = $("celda_" + v_num_grid + "_" + c_row + "_" + col_tmp);
                              cell_item_final.focus()
...
function Set_Deshabilitar_Celdas_Edicion_Items(c_row,v_band){
  ...
  if(!$("checkbox_vista_pedidos").checked)
  {
    ...
    {
      //U/M ORI
      col_tmp = C_UN_ORI + x_col_todos_1
      Bloquea_Celda(c_row,col_tmp,v_band,true)
...
function Set_Actualizar_Producto(){  
  ...
  var ajx = new Ajax(RutaProyecto + "comprasNEW/compras_get_datos_producto.asp", {
    method:'post', 
    data: 'producto='+v_producto,
    async:false,
    onComplete: function(respuesta){
...
function Cargar_Familias1(v_id_td, z_familia, z_subfamilia){
  ...
  var ajx = new Ajax(RutaProyecto + "comprasNEW/select_familias_x_superfamilia1.asp", {
    method:'post', 
...
function Cargar_SubFamilias1(v_id_td, z_subfamilia){
    var ajx = new Ajax(RutaProyecto + "comprasNEW/select_subfamilias_x_familia1.asp", {
    method:'post', 
...
function Actualizar_Producto(){
  ...
  { 
    ...
    var ajx = new Ajax(RutaProyecto + "comprasNEW/compras_actualizar_producto.asp", {
      method:'post', 
      data: 'documento_no_valorizado='                  + $('documento_no_valorizado').value+
            '&numero_documento_no_valorizado='          + $('numero_documento_no_valorizado').value+
            '&documento_respaldo='                      + $('documento_respaldo').value+
      ...
      onComplete: function(respuesta){
        ...
        items_grupos.disabled=true
        var ajx = new Ajax(RutaProyecto + "comprasNEW/compras_productos_grabar.asp", {
          method:'post', 
          data: 'accion=actualizar&documento_no_valorizado='+$("documento_no_valorizado").value+
            '&numero_documento_no_valorizado='          + $('numero_documento_no_valorizado').value+
            '&documento_respaldo='                      + $('documento_respaldo').value+
            '&numero_documento_respaldo='               + $('numero_documento_respaldo').value+
            '&numero_interno_documento_no_valorizado='  + $('numero_interno_documento_no_valorizado').value+
            '&producto='                                +escape(v_producto)+
            '&nom_campo=Porcentaje_ila'                 +
            '&numero_linea='                            + v_num_linea,
        async:false,
        onComplete: function(respuesta){
        Config_Msg_Accion(" Producto actualizado...",5000,msg_accion_3);
...
function validaFechaEmisionAduana(objeto,valor, anio) {
    // 02/02/2012
    return true;
    /*
    anioFecha = valor.substring(6);
    if (anio != anioFecha) {
        alert("La Fecha Emision Aduana debe ser del año " + anio);
        objeto.value = "";
        return false;
    }
    else {
...
function NroAduana(valor) {
    ...
    if (documento_respaldo.value != "Z") {
        if (valor == "") {
            ...
            var objTDNroAduana = document.getElementById("TDNroAduana");
            var objTDSPNAduana = document.getElementById("TDSPNAduana");

            objTDNroAduana.style.display = "none";
            objTDSPNAduana.style.display = "block";

            var ajx = new Ajax("compras_numeros_aduanas.asp", {
                method: 'post',
                data: 'carpeta=' + vCarpeta.value +
                      '&documento_respaldo=' + vDocumentoRespaldo.value,
                //update: $("grilla_productos"),
                onComplete: function (respuesta) {
                    //alert(respuesta)
                    objTDSPNAduana.innerHTML = respuesta;
                }
            });
            ajx.request();
        }
...
function changeNroAduana(valor) {
    if (valor != "") {
        arrayData = valor.split('_');

        var vnumero_documento_respaldo = document.getElementById("numero_documento_respaldo");
        var vfecha_factura = document.getElementById("fecha_factura");
        var vfecha_recepcion = document.getElementById("fecha_recepcion");
        var vmonto_adu_US = document.getElementById("monto_adu_US$");
        var ventidad_comercial = document.getElementById("entidad_comercial");
        var vnumero_factura = document.getElementById("numero_factura");

        var vfecha_aduana = document.getElementById("fecha_emision");
        var vBultos = document.getElementById("bultos");

        vnumero_documento_respaldo.value = arrayData[0];
        vfecha_factura.value = arrayData[2];
        vfecha_recepcion.value = arrayData[2];
        vmonto_adu_US.value = arrayData[3];
        ventidad_comercial.value = arrayData[1];
        vnumero_factura.value = arrayData[4];
        vfecha_aduana.value = arrayData[5];
        vBultos.value = arrayData[6];

        var objTDNroAduana = document.getElementById("TDNroAduana");
        var objTDSPNAduana = document.getElementById("TDSPNAduana");

        objTDNroAduana.style.display = "block";
        objTDSPNAduana.style.display = "none";

        vnumero_documento_respaldo.select();
        //vnumero_documento_respaldo.blur();
    }
}
