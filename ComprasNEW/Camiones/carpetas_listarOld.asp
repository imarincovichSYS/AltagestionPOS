        ...
        <td>
          ...
          <div name="capaEncListaCarpeta" id="capaEncListaCarpeta" style="width: <%=width-18%>px; border-left: 1px solid #CCC; border-top: 1px solid #CCC; border-right: 1px solid #CCC; overflow: auto; overflow-x:hidden;">          
            <table style="width:<%=width-18%>px;" border=0 cellpadding=0 cellspacing=0">
              ...
              if pend <> "true" then
                if documento_respaldo = "R" then
                  strSQL =  "Select Numero_documento_respaldo, Codigo_postal, documento_no_valorizado, Fecha_recepcion, fecha_emision, entidad_comercial, " &_
                    ...
                    IF FALSE THEN
                  strSQL =  "Select Numero_documento_respaldo, Codigo_postal, documento_no_valorizado, Fecha_recepcion, fecha_emision, entidad_comercial, " &_
                            "paridad_conversion_a_dolar paridad, isnull(monto_neto_us$,0) total_cif_ori, isnull(monto_adu_us$,0) total_cif_adu, Apellidos_persona_o_nombre_empresa nombre_proveedor, Documento_respaldo, " &_
                            ...
                            "from DOCUMENTOS_NO_VALORIZADOS dnv, entidades_comerciales ent " &_
                            "where  dnv.proveedor = ent.entidad_comercial and dnv.empresa='SYS' and documento_no_valorizado in ('TCP','RCP') and convert(varchar(4),Fecha_recepcion,112) = '" & anio  &"' " &_ 
  	                        "and (documento_respaldo='R' or documento_respaldo='DS') " &_
  	                        " and carpeta='" & rs_carpetas("carpeta") & "' ORDER BY Codigo_postal, Numero_documento_respaldo"
                    END IF
                else
                  ...
                  strSQL =  "Select Numero_documento_respaldo, Codigo_postal, documento_no_valorizado, Fecha_recepcion, fecha_emision, entidad_comercial, " &_
                    ...
                    IF FALSE THEN
                      strSQL =  "Select Numero_documento_respaldo, Codigo_postal, documento_no_valorizado, Fecha_recepcion, fecha_emision, entidad_comercial, " &_
                                ...
                                "from DOCUMENTOS_NO_VALORIZADOS dnv, entidades_comerciales ent " &_
                                "where  dnv.proveedor = ent.entidad_comercial and dnv.empresa='SYS' and documento_no_valorizado in ('TCP','RCP') and convert(varchar(4),Fecha_recepcion,112) >= '" & anio-1 &"' " &_ 
  	                            "and (documento_respaldo='R' or documento_respaldo='DS') " &_
  	                            " and carpeta='" & rs_carpetas("carpeta") & "' ORDER BY Codigo_postal, Numero_documento_respaldo"
                    END IF
                  else
              ...
              <tr height="15" OnDblClick="Generar_Informe('<%=rs("Numero_documento_respaldo")%>','<%=rs("documento_no_valorizado")%>','<%=rs("Documento_respaldo")%>','<%=rs("Fecha_recepcion")%>','<%=rs("entidad_comercial")%>','<%=rs("nombre_proveedor")%>','<%=rs("fecha_emision")%>','<%=rs("paridad")%>','<%=rs("total_cif_ori")%>','<%=rs("total_cif_adu")%>','<%=v_carpeta%>', '<% = Fecha_factura %>')">
