...
if pend = "true" then 
  if documento_respaldo = "R" then
    strWhere = strWhere & " and carpeta in (select carpeta 	from documentos_no_valorizados with(nolock) where empresa='SYS' and documento_no_valorizado='TCP' " &_
                          " and (documento_respaldo ='R' or documento_respaldo ='DS') )"
  else
    strWhere = strWhere & " and carpeta in (select carpeta 	from documentos_no_valorizados with(nolock) where empresa='SYS' and documento_no_valorizado='TCP' and documento_respaldo ='" & documento_respaldo & "')"
  end if
end if
...
if pend <> "true" then
  str_count_carpetas = "select count(*) as cant_carpetas from carpetas_final with(nolock) " &_
  ...
  if documento_respaldo = "R" then
    str_carpetas = "select carpeta, num_carpeta, anio_mes , fecha_salida = convert(varchar(8),fecha_salida,5), fecha_llegada= convert(varchar(8),fecha_llegada,5), manifiesto, NDP , " &_
    " Fisico = (select COUNT(*)  from carpetas_final_detalle cfd with(nolock) where rtrim(ltrim(cfd.documento_respaldo)) + '-' + cast(YEAR(cfd.anio_mes) as varchar(4)) + '-' +cast(Month(cfd.anio_mes) as varchar) + '-' + cast(cfd.num_carpeta as varchar) = c.Carpeta and Fisico = 'S')  " &_
                   "from carpetas_final c with(nolock) " &_ 
                   "where convert(varchar(6),anio_mes,112) =  '" & anio & right("0"+mes,2) &"' and (documento_respaldo='R' or documento_respaldo='DS')  " & strWhere &_
                   "order by SUBSTRING(convert(varchar(8),fecha_llegada,5),4,2) asc , LEFT(convert(varchar(8),fecha_llegada,5),2) asc "
  else
    str_carpetas = "select carpeta, num_carpeta, anio_mes , fecha_salida = convert(varchar(8),fecha_salida,5), fecha_llegada= convert(varchar(8),fecha_llegada,5), manifiesto , NDP ,  " &_
    " Fisico = (select COUNT(*)  from carpetas_final_detalle cfd with(nolock) where rtrim(ltrim(cfd.documento_respaldo)) + '-' + cast(YEAR(cfd.anio_mes) as varchar(4)) + '-' +cast(Month(cfd.anio_mes) as varchar) + '-' + cast(cfd.num_carpeta as varchar) = c.Carpeta and Fisico = 'S')  " &_
                   "from carpetas_final c with(nolock) " &_ 
  ...
  'Response.Write str_count_carpetas
  'Response.End
  set rs_count_carpetas = Conn.Execute(str_count_carpetas)
  
  if documento_respaldo = "R" then
    str_carpetas = "select carpeta, num_carpeta, anio_mes , fecha_salida = convert(varchar(8),fecha_salida,5), fecha_llegada= convert(varchar(8),fecha_llegada,5), manifiesto, NDP , " &_
    " Fisico = (select COUNT(*)  from carpetas_final_detalle cfd where rtrim(ltrim(cfd.documento_respaldo)) + '-' + cast(YEAR(cfd.anio_mes) as varchar(4)) + '-' +cast(Month(cfd.anio_mes) as varchar) + '-' + cast(cfd.num_carpeta as varchar) = c.Carpeta and Fisico = 'S')  " &_
                   "from carpetas_final c with(nolock) " &_ 
      ...
      <table align="Left" cellpadding=0 cellspacing=0 border=0 >
        <td>
          ...
          <div name="capaEncListaCarpeta" id="capaEncListaCarpeta" style="width: <%=width-18%>px; border-left: 1px solid #CCC; border-top: 1px solid #CCC; border-right: 1px solid #CCC; overflow: auto; overflow-x:hidden;">          
            <table style="width:<%=width-18%>px;" border=0 cellpadding=0 cellspacing=0">
              ...
              <tr  height="18">
                <%if documento_respaldo = "Z" then%>
                  <td style="width:60px;" id='grid_TD_1'><%="NDP"%></td>
                  <td style="width:60px;" id='grid_TD_1'><%=rs_carpetas("NDP")%></td>
                <%else%>
              ...
              if pend <> "true" then
                if documento_respaldo = "R" then
                  strSQL =  "Select Numero_documento_respaldo, Codigo_postal, documento_no_valorizado, Fecha_recepcion, fecha_emision, entidad_comercial, " &_
                            ...
                            "prov_mop = Codigo_postal, Fecha_factura, " &_ 
                            "Fisico = (select  COUNT(*) from carpetas_final_detalle cfd with(nolock) where rtrim(ltrim(cfd.documento_respaldo)) + '-' + cast(YEAR(cfd.anio_mes) as varchar(4)) + '-' +cast(Month(cfd.anio_mes) as varchar) + '-' + cast(cfd.num_carpeta as varchar) = dnv.Carpeta and id_tipo_factura = 1 and Numero_aduana = dnv.Numero_documento_respaldo and Fisico = 'S') " &_
		
                            "from DOCUMENTOS_NO_VALORIZADOS dnv with(nolock), entidades_comerciales ent with(nolock) " &_
                            "where case when dnv.Proveedor_2 is not null and dnv.Proveedor <> dnv.Proveedor_2 then dnv.Proveedor_2 else  dnv.Proveedor end = ent.entidad_comercial and dnv.empresa='SYS' and documento_no_valorizado in ('TCP','RCP') and case when Fecha_recepcion is not Null then convert(varchar(4),Fecha_recepcion,112) else convert(varchar(4), ( select Fecha_llegada from carpetas_final carp (nolock) where carp.Carpeta = dnv.Carpeta ) ,112) end = '" & anio  &"' " &_ 
                  ...
                  strSQL =  "Select Numero_documento_respaldo, Codigo_postal, documento_no_valorizado, Fecha_recepcion, fecha_emision, entidad_comercial, " &_
                            ...
                            "from DOCUMENTOS_NO_VALORIZADOS dnv with(nolock), entidades_comerciales ent with(nolock) " &_
                            "where  case when dnv.Proveedor_2 is not null and dnv.Proveedor <> dnv.Proveedor_2 then dnv.Proveedor_2 else  dnv.Proveedor end = ent.entidad_comercial and dnv.empresa='SYS' and documento_no_valorizado in ('TCP','RCP') and case when Fecha_recepcion is not Null then convert(varchar(4),Fecha_recepcion,112) else convert(varchar(4), ( select Fecha_llegada from carpetas_final carp (nolock) where carp.Carpeta = dnv.Carpeta ) ,112) end >= '" & anio-1 &"' " &_ 
  	                        "and (documento_respaldo='R' or documento_respaldo='DS') " &_
  	                        " and carpeta='" & rs_carpetas("carpeta") & "' ORDER BY Codigo_postal, Numero_documento_respaldo"
                  else
                    strSQL =  "Select Numero_documento_respaldo, Codigo_postal, documento_no_valorizado, Fecha_recepcion, fecha_emision, entidad_comercial, " &_
              ...
              do while not rs.EOF                
                ...
                if isNull( Fecha_recepcion ) then Fecha_recepcion = Fecha_emision
                if isNull( Fecha_factura ) then Fecha_factura = Fecha_emision
              %>
              ...
              <tr height="15" OnDblClick="Generar_Informe('<%=rs("Numero_documento_respaldo")%>','<%=rs("documento_no_valorizado")%>','<%=documento_respaldo%>','<%=rs("Fecha_recepcion")%>','<%=rs("entidad_comercial")%>','<%=rs("nombre_proveedor")%>','<%=rs("fecha_emision")%>','<%=rs("paridad")%>','<%=rs("total_cif_ori")%>','<%=rs("total_cif_adu")%>','<%=v_carpeta%>', '<% = Fecha_factura %>')">
                <%if  rs("documento_no_valorizado") = "RCP" then%>
                  <td style="width:50px;" id="grid_TD_1">
                    <%if Cint(rs("Fisico")) > 0 then%>
                    <span style="background-color:#FA5858">F&iacute;sico</span> 
                    <%end if%>
                    <%=rs("numero_documento_respaldo")%>
                  </td>
                  <td style="width:70px;" id="grid_TD_1"><%=ltrim(rtrim(rs("prov_mop")))%>&nbsp;</td>
                <%else%>
                  <td style="width:50px;" id="grid_TH_2"><%if Cint(rs("Fisico")) > 0 then%>
                    <span style="background-color:#FA5858">F&iacute;sico</span> 
                    <%end if%><%=rs("numero_documento_respaldo")%></td>
            ...
            <table style="width:<%=width-18%>px;" border=0 cellpadding=0 cellspacing=0">
              <tr valign="top" height="30">
                <td style="width:100px;" id='grid_TD_1' colspan=1>
                  <%="N Docs = " & fila%>
                </td>                
              </tr>
            </table>
