...
function Get_Nombre_Entidad_Comercial(v_entidad_comercial)
  strSQL="select Nombres_persona, Apellidos_persona_o_nombre_empresa from entidades_comerciales with(nolock) where empresa='SYS' and entidad_comercial='"&v_entidad_comercial&"'"
...
strSQL="select top 1 Fecha_emision fecha, Monto_venta_neta_moneda_oficial valor from Documentos_valorizados with(nolock) where " &_
...
strSQL="select sum(Monto_venta_neta_moneda_oficial) total from Documentos_valorizados with(nolock) where " &_
       "empresa = 'SYS' AND empleado_responsable='"&entidad_comercial&"' AND Tipo_documento = 'VTA' " &_
