...
v_codigo_LPBase                    = Get_Codigo_Lista_Precio_LP_BASE
...
do while not rs_itemes.EOF
  ...
  '###### L02 ######
  if v_codigo_LPBase = "L02" then
    '###### L02 ######
      strSQL3 = strSQL3 & "Exec PLP_Borra_Producto_en_Lista_de_precios '"&v_codigo_LPrecio2&"', '"&producto&"'; "
  
      strSQL4 = strSQL4 & "Exec PLP_Graba_Producto_en_Lista_de_precios 'SYS', '"&v_codigo_LPrecio2&"', '"&producto&"', 0, '$', "&valor&", 0; "
  end if
...
IF FALSE THEN
	IF LEN(strSQL1) > 0 THEN
	  ...
	  if err <> 0 then
		Conn.RollbackTrans
		Response.Write err.Source & ", " & err.number & ", " & err.Description & ", Linea:282"
		Response.End
	  end if
	END IF
	IF LEN(strSQL2) > 0 THEN
	  ...
	  if err <> 0 then
		Conn.RollbackTrans
		Response.Write err.Source & ", " & err.number & ", " & err.Description & ", Linea:290"
		Response.End
	  end if
	END IF
ELSE
	IF LEN(strSQL1) > 0 THEN
		...
		for i =  0 to ubound(arraySQL) - 1
			IF LEN( arraySQL(i) ) > 0 THEN
			  ...
			  if err <> 0 then
				Conn.RollbackTrans
				Response.Write err.Source & ", " & err.number & ", " & err.Description & ", Linea:302"
				Response.End
			  end if
			END IF
		next
	...
	IF LEN(strSQL2) > 0 THEN
		...
		for i =  0 to ubound(arraySQL) - 1
			IF LEN( arraySQL(i) ) > 0 THEN
			  ...
			  if err <> 0 then
				Conn.RollbackTrans
				Response.Write err.Source & ", " & err.number & ", " & err.Description & ", Linea:315"
				Response.End
			  end if
			END IF
		next
	END IF
  IF LEN(strSQL3) > 0 THEN
    arraySQL = Split( strSQL3, ";" )
    for i =  0 to ubound(arraySQL) - 1
      IF LEN( arraySQL(i) ) > 0 THEN
        Conn.Execute(arraySQL(i))
        if err <> 0 then
        Conn.RollbackTrans
        Response.Write err.Source & ", " & err.number & ", " & err.Description & ", Linea:328"
        Response.End
        end if
      END IF
    next
  END IF
  IF LEN(strSQL4) > 0 THEN
    arraySQL = Split( strSQL4, ";" )
    for i =  0 to ubound(arraySQL) - 1
      IF LEN( arraySQL(i) ) > 0 THEN
        Conn.Execute(arraySQL(i))
        if err <> 0 then
        Conn.RollbackTrans
        Response.Write err.Source & ", " & err.number & ", " & err.Description & ", Linea:341"
        Response.End
        end if
      END IF
    next
