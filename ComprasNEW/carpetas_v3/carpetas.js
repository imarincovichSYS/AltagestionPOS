...
// function Cargar_Form_Ingresar_Form_Ingreso_buque(v_numero_linea)
// function Cargar_AutoCompleteProveedores()
// function Agregar_Factura()
// function Agregar_Buque()
// function Actualizar_Factura()
// function Actualizar_Buque()
...
function Cargar_Form_Ingresar(v_num_carpeta, v_numero_documento_respaldo) {
    ...
    $.ajax({
        ...
        success: function (html) {
            ...
            $("#idTipoCarga").change(function () { Actualizar_Datos_Generales($(this)); });
            $("#Numero_aduana").change(function () { Actualizar_Datos_Generales($(this)); });
            $("#NDP").change(function () { Actualizar_Datos_Generales($(this)); });
            $("#Booking").change(function () { Actualizar_Datos_Generales($(this)); });
            ...
            if (false) {
                ...
                $("#fecha_llegada").datepicker({
                    ...
                    onSelect: function (selectedDate) { 
						if (validarFechas(this) == true) { 
							Actualizar_Datos_Generales($(this));	  DESCOMENTAR
						}
								//var now = new Date();
								//if(selectedDate > now){
								//	Actualizar_Datos_Generales($(this));	
								//}else{
								//	$("#td_msg").html("<center><b><label id='label_msg' name='label_msg' style='font-size:11px; color:#AA0000; background-color: #FFCC33;'>La Fecha de Estimada debe ser mayor a la Fecha de Hoy.</label></b></center>");
                ...
                $("#fecha_aduana").on({
                    blur: function () {
                        if (CompletarFecha(this) !== undefined) { Actualizar_Datos_Generales($(this)); Actualiza_Estado(); }
                    },
                    focus: function () {
                        $(this).select();
                    },
                ...
                $("#fecha_llegada").on({
                    blur: function () {
                        if (CompletarFecha(this) !== undefined) {
                            if (validarFechas(this) == true) { 
								Actualizar_Datos_Generales($(this));
								//var selectedDate =  $("#fecha_llegada").val();
								//var now = new Date();
								//if(selectedDate > now){
								//	Actualizar_Datos_Generales($(this));	
								//}else{
            ...
            f = $("#Arrival_in").val(); if (f == "01-01-1900") { $("#Arrival_in").val(""); }

        
            //$("#label_buquesitos").css("title", "Agregar buque");
            $("#lbl_movimiento_buque").on({
                mouseover: function () {
                    $(this).css("cursor", "pointer");
                    $(this).css("background-color", "#60B060");
                },
          ...
          $.ajax({
              ...
              error: function (html) {
                  ...
                  if (html.responseText.indexOf('La Fecha de') != -1) {
                      ...
                      setTimeout(function () { $("#label_msg").fadeOut(900); }, 2200);
                  }
                  else {
                      ...
                      setTimeout(function () { $("#label_msg").fadeOut(2000); }, 6000);
...
function Cargar_Form_Ingresar_Form_Ingreso(v_numero_linea){
    ...
    $.ajax({
        ...
        //                $("#paridad").val(1.00);
            ...
            $("#paridad").on({
                blur: function () {
                  if ( $("#id_tipo_factura").val() == "1" ) {
                      if ($("#tipo_moneda").val() == "$") {
                          Calcular_Monto_Total_Pesos();
                      }
                  }
                },
                focus: function () {
...
function Cargar_Form_Ingresar_Form_Ingreso_buque(){
    var auxNumero_aduana = document.getElementById("Numero_aduana");
    $.ajax({
        url: "carpetas_form_ingresar_form_ingreso_buque.asp",
        data: "documento_respaldo=" + $("#documento_respaldo").val() +
              "&anio=" + $("#anio").val() +
              "&mes=" + $("#mes").val() +
              "&num_carpeta=" + $("#num_carpeta").val() +
...
function Agregar_Factura() {
    ...
    if (flag) { alert(msg); }
        ...
        if (confirm("�Est� seguro que desea agregar una factura a la carpeta?")) {
            $.ajax({
...
function Agregar_Buque() {
    ...
    if (confirm("¿Desea agregar un movimiento de buque en la carpeta?")) {
        Cargar_Form_Ingresar_Form_Ingreso_buque();
    }
}

function Actualizar_Buque(btnObj) {
    //debugger
    var idObj = btnObj.id.replace("lbl_actualizar_movimiento", "");
    var Lugar = $("#Lugar"+idObj).val();
    var Fecha_llegada = $("#Fecha_llegada"+idObj).val();
    var Fecha_salida = $("#Fecha_Salida"+idObj).val();
    var Buque_Base = $("#Buque_Base"+idObj).val();
    var Viaje = $("#Viaje"+idObj).val();
    if(Lugar == "" || Lugar == "undefined" || Lugar == undefined){
        Lugar = null;
    }else{
        Lugar = "'" + Lugar + "'";
    }
    if(Fecha_llegada == "" || Fecha_llegada == "undefined" || Fecha_llegada == undefined){
        Fecha_llegada = null;
    }
    if(Fecha_salida == "" || Fecha_salida == "undefined" || Fecha_salida == undefined){
        Fecha_salida = null;
    }
    if(Buque_Base == "" || Buque_Base == "undefined" || Buque_Base == undefined){
        Buque_Base = null;
    }else{
        Buque_Base = "'" + Buque_Base + "'";
    }
    if(Viaje == "" || Viaje == "undefined" || Viaje == undefined){
        Viaje = null;
    }else{
        Viaje = "'" + Viaje + "'";
    }
    ...
    $.ajax({
        url: "carpetas_grabar_movimiento_buque.asp",
        data: dataObject,
        type: "post",
        async: true,
        dataType: "json",
        success: function (data) {
            if (data.accion == "OK") {
                //reload tabla ingreso/modificacion buque
                $("#td_msg").html("<center><b><label id='label_msg' name='label_msg' style='font-size:11px; color:#009933; background-color: #EEEEEE;'>Datos actualizados correctamente...</label></b></center>");
                setTimeout(function () { $("#label_msg").fadeOut(2000); }, 3000);
                Cargar_Form_Ingresar_Form_Ingreso_buque();
            }
...
function Actualizar_Factura() {
  ...
  //    str_msg_error = "Ingrese n� aduana"
    ...
    str_msg_error = "Ingrese monto moneda origen"
      ...
      str_msg_error = "Ingrese monto moneda origen"
  else if(parseInt($("#ProrrateoCBM").val()) == 0 && parseInt($("#ProrrateoKG").val()) == 0 && parseInt($("#ProrrateoUSD").val()) == 0 && escape($("#documento_respaldo").val()) == "Z")
      str_msg_error = "Ingrese al menos un porcentaje de Prorrateo (CBM,KG,USD)"
...
if (confirm("�Est� seguro que desea actualizar los datos de esta factura?")) {
    $.ajax({
        url: "carpetas_grabar_detalle.asp",
        data: "accion=update" +
                  "&documento_respaldo=" + $("#documento_respaldo").val() +
        ...
        //"&numero_aduana=" + escape($("#NumeroAduana").val()) +
                  ...
                  "&GrupoID=" + $("#GrupoID").val() +
                  "&numero_interno_carpeta_detalle=" + $("#numero_interno_carpeta_detalle").val(),
        type: "post",
        async: true,
        dataType: "json",
        success: function (data) {
...
function Calcular_Monto_Total_Pesos() {
  if($("#monto_total_usd").val() == "")
    $("#monto_total_usd").val("0.00")
  //if($("#monto_moneda_origen").val() == "")
  //  $("#monto_moneda_origen").val("0.00")
  //if ($("#paridad").val() == "")
  //  $("#paridad").val("0.0000")
  ...
  $("#monto_moneda_origen").val(roundNumber(v_monto_moneda_origen, 2).toFixed(0));
...
function Eliminar_Factura(v_numero_linea){
  Cerrar_Form_Ingreso();
  if(confirm("�Est� seguro que desea eliminar esta factura?")) {
    $.ajax({
      url       : "carpetas_grabar_detalle.asp",
      data      : "accion=delete"         +
                  "&documento_respaldo="  + $("#documento_respaldo").val()  +
...
function Agregar_Carpeta(){
  if(confirm("�Est� seguro que desea crear una nueva carpeta?"))
  {
    $.ajax({
      url       : "carpetas_grabar.asp",
      data      : "accion=insert"         +
    ...
    if (auxDocumento_respaldo.value == "TU") {
        ...
        if (flgError == "N") {
            if (fLlegad != "") {
                ...
                if (dLlegada <= dSalida) { 
					flgError = "S"; msgError = "La Fecha de Llegada no puede ser menor que la Fecha de Salida."; 
				}
				if (dLlegada < now) { 
					flgError = "S"; msgError = "La Fecha de Llegada debe ser Mayor a la Fecha de Hoy."; 
				}
            }
        }
        if (flgError == "N") {
            if (fRecepc != "" && fLlegad != "") {
                if (dRecepcion < dLlegada) { 
				flgError = "S"; msgError = "La Fecha de Recepci�n no puede ser menor que la Fecha de Llegada."; 
				}
            }
        }
    }
