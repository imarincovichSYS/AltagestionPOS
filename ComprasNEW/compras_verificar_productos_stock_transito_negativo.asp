...
strSQL="select producto, cantidad = sum(cantidad_entrada), Item= max(numero_de_linea_en_RCP_o_documento_de_compra), " &_
       "stock_en_transito= ((select sum(stock_en_transito) from productos_en_bodegas b with(nolock) where m.producto=b.producto and bodega='0010' group by producto)), " &_
       "Dif= ((select sum(stock_en_transito) " &_
       "from productos_en_bodegas b with(nolock) where m.producto=b.producto and bodega='"&bodega&"' group by producto)) - sum(cantidad_entrada) " &_
       ...
       "having ((select sum(stock_en_transito) from productos_en_bodegas b with(nolock) " &_
