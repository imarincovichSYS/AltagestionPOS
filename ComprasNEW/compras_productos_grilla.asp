...
Porcentaje_ila                          = Request.Form("Porcentaje_ila")
...
width     = "1220"
if vista = "PEDIDO" then width = "1160"
height    = "442" '296
TH_Height = "26"
...
if accion = "NUEVO" then
  ...
  'ITEMES
  strSQL="select IsNull(max(numero_de_linea_en_rcp_o_documento_de_compra),0) num_linea from "&nom_tabla&" with(nolock) where empresa='SYS' and " &_
  ...
  for i=1 to items
    ...
    strSQL="insert into "&nom_tabla&"(Año_recepcion_compra,Bodega, cantidad_mercancias, " &_
           ...
           "Cantidad_x_caja, Peso_x_caja, Delta_CPA_US$, unidad_de_medida_origen, cantidad_origen, Porcentaje_ila) values(" &_
           ""&anio&"," &_
           "'"&bodega&"'," &_
           "0,1,0," &_
           "'"&v_proveedor&"'," &_
           ...
           "'P'," &_
           "'UN','UN', " &_
           ""&paridad&",0,0,0,0,1,1,0,0,'UN',1,"&Porcentaje_ila&")"
...
if documento_respaldo = "DS" then strCols = "Round(A.costo_cif_ori_us$,7) costo_cif_ori_us$, A.total_cif_ori, "
Lista_LP_BASE = Get_Codigo_Lista_Precio_LP_BASE
paridad_mg    = cdbl(GetParidad_Para_Margen)
TasaImpAdu    = Replace(Trim(cdbl(Get_Tasa_Impuesto_Aduanero)/100),",",".")
...
if documento_no_valorizado = "TCP" then
  ...
  if vista = "NORMAL" then
    if documento_respaldo = "Z" then
      strSQL="select '', A.numero_de_linea_en_rcp_o_documento_de_compra, A.numero_de_linea_en_rcp_o_documento_de_compra_padre, " &_
             ...
             "D.porcentaje_ila, A.codigo_arancelario, B.codigo_postal, A.proveedor_origen, A.codcaja, A.Codigo_antiguo, A.producto_proveedor, " &_
             "A.cantidad_origen, A.unidad_de_medida_origen, A.costo_ex_fab_moneda_origen, A.total_ex_fab, /*A.costo_fob_us$, A.total_fob_us$,*/ " &_
             "A.numero_de_linea_en_rcp_o_documento_de_compra item_1 from " &_
             ...
             "codigo_arancelario, producto_proveedor, proveedor_origen, cantidad_origen, unidad_de_medida_origen, " &_
             ...
             "/*Round(costo_fob_us$,7) costo_fob_us$, Round(costo_fob_us$*Cantidad_origen,2) total_fob_us$, */" &_
             "numero_guia_de_despacho_o_factura_proveedor, proveedor, IsNull(Codigo_antiguo,'') as Codigo_antiguo, IsNull(codcaja,'') as codcaja from "&nom_tabla&" with(nolock) where empresa='SYS' and documento_no_valorizado='"&documento_no_valorizado&"' and " &_
             ...
             "(select distinct(codigo_postal), entidad_comercial from entidades_comerciales with(nolock) where empresa='SYS') B, " &_
             "(select producto, IsNull(porcentaje_impuesto_1,0) porcentaje_ila from productos with(nolock) where empresa='SYS') D " &_
    ...
    if documento_respaldo = "Z" then
      strSQL="select '', A.numero_de_linea_en_rcp_o_documento_de_compra, A.numero_de_linea_en_rcp_o_documento_de_compra_padre, " &_
             ...
             "  A.nombre_producto " &_
             "end , " &_
             "A.cantidad_mercancias, A.proveedor_origen, A.codcaja, A.Codigo_antiguo, A.producto_proveedor, " &_
             "A.cantidad_origen, A.unidad_de_medida_origen, A.costo_ex_fab_moneda_origen, A.total_ex_fab, /*A.costo_fob_us$, A.total_fob_us$,*/ " &_
             "A.numero_de_linea_en_rcp_o_documento_de_compra item_1 from " &_
             "(select numero_de_linea_en_rcp_o_documento_de_compra, numero_de_linea_en_rcp_o_documento_de_compra_padre, " &_
             "producto, nombre_producto_proveedor, nombre_producto, " &_
             ...
             "codigo_arancelario, producto_proveedor, proveedor_origen, cantidad_origen, unidad_de_medida_origen, " &_
             "Round(costo_ex_fab_moneda_origen,7) costo_ex_fab_moneda_origen, Round(costo_ex_fab_moneda_origen*Cantidad_origen,2) total_ex_fab, " &_
             "/*Round(costo_fob_us$,7) as costo_fob_us$, Round(costo_fob_us$*Cantidad_origen,2) as total_fob_us$,*/ " &_
             "numero_guia_de_despacho_o_factura_proveedor, proveedor, IsNull(Codigo_antiguo,'') as Codigo_antiguo, IsNull(codcaja,'') as codcaja from "&nom_tabla&" with(nolock) where empresa='SYS' and documento_no_valorizado='"&documento_no_valorizado&"' and " &_
             "numero_documento_no_valorizado="&numero_documento_no_valorizado&" and tipo_documento_de_compra='"&documento_respaldo&"' and " &_
             "numero_documento_de_compra="&numero_documento_respaldo&") A, " &_
             "(select distinct(codigo_postal), entidad_comercial from entidades_comerciales with(nolock) where empresa='SYS') B, " &_
  ...
  '####################################################################################
  '------ LOS PRECIOS SE OBTIENEN DESDE "productos_en_listas_de_precios" (LP_BASE)----- JAIME
  '####################################################################################
  if documento_respaldo = "Z" then
    strSQL="select '', A.numero_de_linea_en_rcp_o_documento_de_compra, A.numero_de_linea_en_rcp_o_documento_de_compra_padre, " &_
           ...
           "D.porcentaje_ila, A.codigo_arancelario, B.codigo_postal, A.proveedor_origen, A.codcaja, A.Codigo_antiguo, A.producto_proveedor, " &_
           "A.cantidad_origen, A.unidad_de_medida_origen, A.costo_ex_fab_moneda_origen, A.total_ex_fab, /*A.costo_fob_us$, A.total_fob_us$,*/ " &_
           "A.numero_de_linea_en_rcp_o_documento_de_compra as item_1 from " &_
           "(select /*costo_fob_us$,Round(costo_fob_us$*Cantidad_origen,2) as total_fob_us$, */ numero_de_linea_en_rcp_o_documento_de_compra, numero_de_linea_en_rcp_o_documento_de_compra_padre, " &_
           "producto, nombre_producto_proveedor, nombre_producto, " &_
           "unidad_de_medida_compra, cantidad_mercancias, cantidad_um_compra_en_caja_envase_compra, " &_
           "unidad_de_medida_consumo, cantidad_x_un_consumo, cantidad_entrada, IsNull(alto,0) alto, IsNull(ancho,0) ancho, IsNull(largo,0) largo, " &_
           "IsNull(Cantidad_x_caja,0) Cantidad_x_caja, IsNull(Round( (alto*ancho*largo)/1000000 / Cantidad_x_caja,6),0) volumen, " &_
           ...
           "numero_documento_de_compra="&numero_documento_respaldo&") A, " &_
           "(select distinct(codigo_postal), entidad_comercial from entidades_comerciales with(nolock) where empresa='SYS') B, " &_
           "(select producto, valor_unitario Precio_de_lista_modificado from productos_en_listas_de_precios with(nolock) " &_
