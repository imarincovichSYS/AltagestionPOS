...
var C_PROD_PROV = 30,   C_PROV_ORI    = 31,   C_EX_FAB      = 32,   C_FOB         = 33
var C_PROD_PROV_VISTA_PEDIDO = 7, C_PROV_ORI_VISTA_PEDIDO = 8, C_FOB_VISTA_PEDIDO = 9, C_EX_FAB_VISTA_PEDIDO = 10
...
/*
//No se está utilizando como función --> Este ajax se llama cuando se está validando la compra
function Set_PARIDAD_para_validacion_segun_FECHA_RECEPCION(){
  var ajx_par = new Ajax(RutaProyecto+"funciones_SYS_AJAX/AJAX_Get_Paridad_X_Fecha.asp",{
    method:'post', 
    async:false,
    data: 'fecha='+$("fecha_recepcion").value,
    onComplete: function(respuesta){
      //alert(respuesta)
      var z_array_paridades = respuesta.split("~")
      $("paridad_segun_fecha_recepcion").value = z_array_paridades[0]
    }
  });
  ajx_par.request();
}
...
function Editar_Compra(){
  ...
  if(v_documento_no_valorizado=="TCP")
  {
    ...
    //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    //%%%%%Cargar Total CIF Adu desde las facturas asociadas a la carpeta %%%%%
    //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    /*
    if($("documento_respaldo").value=="Z")
    {
...
function Generar_Informe(v_para){
  ...
  parametros+="&fecha_recepcion="+v_fecha_recepcion+"&fecha_emision="+v_fecha_emision+"&fecha_factura="+v_fecha_factura+"&paridad="+v_paridad+"&total_cif_ori="+v_total_cif_ori+"&total_cif_adu="+v_total_cif_adu+"&carpeta="+v_carpeta
...
function Set_Calcula_Margen_Item(x_fila){
  ...
  x_producto      = $("celda_" + v_num_grid + "_" + x_fila + "_" + C_PRODUCTO).value
  ...
  var ajx1 = new Ajax("Get_superfamilia_x_producto.asp",{
    method:'post', 
    async:false,
    data: 'producto=' + x_producto,
    onComplete: function(respuesta){
      x_superfamilia  = respuesta
      x_factor_rubro  = Get_Tasa_Rubro(respuesta);
      //console.log(x_superfamilia + "x_factor_rubro: " + x_factor_rubro)
      ...
      //console.log("x_delta_cpa: " + x_delta_cpa + ", x_volumen: " + x_volumen + ", x_costo_flete_transporte: " + x_costo_flete_transporte + ", x_cif_adu: " + x_cif_adu + ", x_factor_rubro: " + x_factor_rubro)
...
function Set_Calcula_Margen_y_CPA_Item(x_fila){
  ...
  var ajx1 = new Ajax("Get_superfamilia_x_producto.asp",{
    ...
    onComplete: function(respuesta){
      ...
      //Modificacion: Cmartinez, 20160825
      //x_cpa         = (x_volumen * x_costo_flete_transporte + x_cif_adu) * x_factor_rubro
      //x_cpa         = roundNumber(x_cpa,7)
      //x_cpa         = x_cpa + x_delta_cpa
      
      col_tmp         = C_VOLUMEN + col_dif + y_col_todos_2
      v_volumen       = parseFloat(Replace($("celda_" + v_num_grid + "_" + x_fila + "_" + col_tmp).value,",",""))
      ...
      x_cpa           = (v_volumen * v_costo_flete_transporte + parseFloat(x_cif_adu)) * x_factor_rubro * v_factor_volumen_en_cero
      x_cpa           = roundNumber(x_cpa,7)
      if($("documento_respaldo").value!="R" && $("documento_respaldo").value!="DS")
        x_cpa = roundNumber(x_total_cif_adu/x_cantidad,7) * x_factor_rubro
      
      col_tmp         = C_CPA + col_dif + y_col_todos_2
      x_cpa           = roundNumber(x_cpa,4)
      x_cpa           = x_cpa.toFixed(4)
      cell_cpa        = $("celda_" + v_num_grid + "_" + x_fila + "_" + col_tmp);
      cell_cpa.value  = Formateo_Completo_Numero(x_cpa).toString()
      cell_cpa.setAttribute("o_value",Formateo_Completo_Numero(x_cpa).toString());
...
function Set_Totales_Costos(){
  ...
  $("label_total_cif_ori").innerHTML=Formateo_Completo_Numero(parseFloat(TOTAL_COSTO_CIF_ORI).toFixed(2))
    ...
    var ajx_par = new Ajax(RutaProyecto+"funciones_SYS_AJAX/AJAX_Get_Paridad_Facturacion_X_Fecha.asp",{
      ...
      onComplete: function(respuesta){
        ...
        {          
          ...
          //Verifica Productos con Stock en Transito Negativo
          var ajx_stock_neg = new Ajax("compras_verificar_productos_stock_transito_negativo.asp",{
            method:"post", 
            async:false,
            data: "numero_interno_documento_no_valorizado="+$("numero_interno_documento_no_valorizado").value+"&bodega="+$("bodega").value,
            onComplete: function(respuesta_stock_negativo){
              //console.log(respuesta_stock_negativo)
              if(respuesta_stock_negativo=="OK")
              {
                $("bot_informacion_to_RCP").style.visibility = "hidden"
                $("bot_terminar_compra").style.visibility    = ""
                Config_Msg_Accion(" Datos de compra válidos...ya puede finalizar al compra",5000,$("msg_accion_3"))       
              ...
              {
                $("bot_informacion_to_RCP").title            = respuesta
                $("bot_informacion_to_RCP").setAttribute("campo_error","")
                $("bot_informacion_to_RCP").style.visibility = ""
                Config_Msg_Accion(" "+$("bot_informacion_to_RCP").title+"...",5000,$("msg_accion_3"))
                return;
...
function SaveChanges(cell, c_row, c_col, c_type, c_name, c_scale, c_maxlength, c_null, n_tabla){
  ...
  factor_rubro  = Get_Tasa_Rubro($("superfamilia").value)
  if($("documento_respaldo").value=="TU" || $("documento_respaldo").value=="DS")
    col_dif = 2
  ...
  {
    z_valor_cantidad_mercancias = Replace($("celda_" + v_num_grid + "_" + c_row + "_" + C_TRASP_CANT_M).value,",","")
    z_valor_cantidad_traspaso   = Replace($("celda_" + v_num_grid + "_" + c_row + "_" + C_TRASP_CANT_TRASPASO).value,",","")
  ...
  if(c_name=="unidad_de_medida_compra" || c_name=="cantidad_mercancias" || c_name=="cantidad_um_compra_en_caja_envase_compra" || c_name=="unidad_de_medida_consumo" || c_name=="cantidad_x_un_consumo")
  {
    ...
    {//Vista Normal
      ...
      /*if(v_cant_m==0 || v_cant_x_m==0 || v_cant_x_un==0)
      {
        ...
        Config_Msg_Accion(" Las cantidades deben ser mayor que 0...",5000,$("msg_accion_3"))
        cell.value = cell.getAttribute("o_value");
        cell.focus();
        return;
      }*/
      ...
      if(v_cantidad > 0)
      {
        ...
        */
        ...
        Set_Calcula_Margen_y_CPA_Item(c_row)
      }
      //alert("v_cpa: "+v_cpa+"; v_total_cif_adu: "+v_total_cif_adu+"; v_cantidad: "+v_cantidad+"; factor_rubro: "+factor_rubro)
    }
    else
    {
      c_name3             = "cantidad_entrada"
  ...
  else if(c_name=="cantidad_entrada")
  {
    ...
    //Modificacion: Cmartinez, 20160825 --> El cálculo del cpa se hace en la funcion Set_Calcula_Margen_y_CPA_Item()
    /*
    col_tmp         = C_VOLUMEN + col_dif + y_col_todos_2
...
function Set_Deshabilitar_Celdas_Edicion_Items(c_row,v_band){
  ...
  if(!$("checkbox_vista_pedidos").checked)
  {
    ...
    Bloquea_Celda(c_row,col_tmp,v_band,false)
    //Costo EXFAB Moneda Origen
    col_tmp = C_EX_FAB + col_dif + x_col_todos_2
    Bloquea_Celda(c_row,col_tmp,v_band,false)
    //Costo FOB US$
