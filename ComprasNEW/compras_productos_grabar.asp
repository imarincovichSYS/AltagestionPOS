...
if len( fecha_recepcion ) = 0 then fecha_recepcion = Request.Form("fecha_factura")
if len( fecha_recepcion ) = 0 then fecha_recepcion = Request.Form("fecha_emision")
...
valor       = Replace(Ucase(trim(Unescape(Request.Form("valor")))),"§","+")
...
nom_campo7  = Unescape(Request.Form("nom_campo7"))
valor7      = Ucase(trim(Unescape(Request.Form("valor7"))))
nom_campo8  = Unescape(Request.Form("nom_campo8"))
valor8      = Ucase(trim(Unescape(Request.Form("valor8"))))

vista      = Request.Form("vista")

Porcentaje_ila = 0

if fecha_recepcion = "" then fecha_recepcion = cdate(now())
...
if accion = "actualizar" then
    strSQL="select nombre, nombre_para_boleta, nombre_producto_proveedor_habitual, superfamilia, familia, subfamilia, " &_
           ...
           "IsNull(unidad_de_medida_consumo,'UN') unidad_de_medida_consumo, " &_
           "IsNull(Cantidad_um_compra_en_caja_envase_compra,1) Cantidad_um_compra_en_caja_envase_compra," &_
           "IsNull(cantidad_x_un_consumo,1) cantidad_x_un_consumo, Producto_proveedor_habitual, Proveedor_origen " &_
           "from productos with(nolock) where empresa='SYS' and producto='"&producto&"'"
  ...
  if nom_campo = "producto" then
    if not rs.EOF then 
      ...
      unidad_de_medida_consumo            = trim(rs("unidad_de_medida_consumo"))
      Cantidad_um_compra_en_caja_envase_compra  = trim(rs("Cantidad_um_compra_en_caja_envase_compra"))
      cantidad_x_un_consumo                     = trim(rs("cantidad_x_un_consumo"))
      producto_proveedor_habitual               = trim(rs("producto_proveedor_habitual"))
      proveedor_origen                          = trim(rs("proveedor_origen"))
      ...
      strAtributosProd  = strAtributosProd  & unidad_de_medida_consumo            & delimiter
      strAtributosProd  = strAtributosProd  & Cantidad_um_compra_en_caja_envase_compra  & delimiter
      strAtributosProd  = strAtributosProd  & cantidad_x_un_consumo                     & delimiter
      strAtributosProd  = strAtributosProd  & producto_proveedor_habitual               & delimiter
      strAtributosProd  = strAtributosProd  & proveedor_origen                          & delimiter
      ...
      strSet=" "&nom_campo&"='"&valor&"', Año_recepcion_compra="&year(cdate(fecha_recepcion))&", " &_
             "proveedor='"&v_proveedor&"', nombre_producto='"&nombre&"', nombre_para_boleta='"&nombre_para_boleta&"', " &_
             ...
             "Producto_proveedor='"&producto_proveedor_habitual&"', " &_
             "unidad_de_medida_compra='"&unidad_de_medida_compra&"', " &_
             "unidad_de_medida_consumo='"&unidad_de_medida_consumo&"'," &_
             "Cantidad_um_compra_en_caja_envase_compra="&Cantidad_um_compra_en_caja_envase_compra&", " &_
             "cantidad_x_un_consumo="&cantidad_x_un_consumo&", " &_
             "cantidad_mercancias=0, alto="&alto&", ancho="&ancho&", largo="&largo&", peso="&peso&", " &_
             "cantidad_x_caja="&cantidad_x_caja&", peso_x_caja="&peso_x_caja&", Delta_CPA_US$=0, " &_
             "codigo_arancelario='"&codigo_arancelario&"', unidad_de_medida_origen='UN', cantidad_origen=1"
      'strSet = strSet & ", fecha0 = " & fecha_recepcion
      'strSet = strSet & ", fecha1 = " & Request.Form("fecha_recepcion")
      'strSet = strSet & ", fecha2 = " & Request.Form("fecha_factura")
      'strSet = strSet & ", fecha3 = " & Request.Form("fecha_emision")
    else
      Response.Write "NO_EXISTE"
      Response.End
    end if
  else
    if nom_campo = "numero_de_linea_destino" and valor <> "" then
      '######################################################################
      '--------- Chequear si el ítem destino No Existe  ---------------------
      '######################################################################
      ...
      strSQL="select producto, Numero_de_linea_en_RCP_o_documento_de_compra from "&nom_tabla&" with(nolock) " &_
             "where empresa='SYS' and " &_
             "documento_no_valorizado='TCP' and " &_
             ...
             "Numero_de_linea_en_RCP_o_documento_de_compra="&valor&" and " &_
             "producto<>'00000000'"
      'Response.Write strSQL
      'Response.End
      set rs = Conn.Execute(strSQL)
      if not rs.EOF then
        Response.Write "LINEA_BLOQUEADA" 'Existe no vacía (tiene ingresado un producto <> '00000000')
        Response.End
      end if
    end if
    ...
    if nom_campo = "precio_de_lista_modificado" then
      ...
      'if nom_campo = "producto_proveedor"         then nom_campo_t_prod = "producto_proveedor_habitual"
      'if nom_campo = "proveedor_origen"           then nom_campo_t_prod = "proveedor_origen"
      
      if nom_campo_t_prod <> "costo_cif_adu_us$" and nom_campo_t_prod <> "costo_cif_ori_us$" and nom_campo_t_prod <> "costo_cpa_us$" and nom_campo_t_prod <> "costo_fob_us$" then
        if tipo_dato="0" then
      ...
      if nom_campo <> "costo_cif_adu_us$" and nom_campo <> "costo_cif_ori_us$" then
        ...
        if tipo_dato="0" then 
          strSet = " "&nom_campo&"="&valor&" "
          if nom_campo = "numero_de_linea_destino" then
            if valor = "" then 
              strSet = " "&nom_campo&"=null, cantidad_traspaso=0"
            else
              strSet = strSet & ", cantidad_traspaso=" & valor1
            end if
          end if
        elseif tipo_dato="2" and valor = "" then
      ...
      if nom_campo3 <> "" or nom_campo = "costo_cif_adu_us$" or nom_campo = "costo_cif_ori_us$" then 'Se recalculó la cantidad o se ingresaron costos_totales
        if nom_campo3 <> "" then
          if documento_respaldo = "TU" or documento_respaldo = "DS" then 'Se debe actualizar cantidad, cif_ori (valor1) y cif_adu (valor2) por separado (y todos los costos_*)
        ...
        if vista = "NORMAL" then
          ...
          if nom_campo3 <> "" or (nom_campo = "costo_cif_adu_us$" and documento_respaldo <> "TU" and documento_respaldo <> "DS") or nom_campo = "costo_cif_ori_us$" then
            '############ ORI ############
      ...
      'if nom_campo = "producto_proveedor" or nom_campo = "proveedor_origen" then
      '  strSet_t_prod = strSet_t_prod & ", "&nom_campo_t_prod&"='"&valor&"' "
      'end if
      
      if nom_campo = "costo_ex_fab_moneda_origen" then '-->Modificacion 20161004
        '############ EX FAB ############
        valor_cl = Round(valor * paridad,7)
        strSet = strSet & ", Costo_EX_FAB_US$                                     = " & valor     'Costo_EX_FAB_US$

        strSet_t_prod     = strSet_t_prod & ", Ultimo_costo_EX_FAB_moneda_origen  = " & valor     'Ultimo_costo_EX_FAB_moneda_origen
        strSet_t_prod     = strSet_t_prod & ", Ultimo_costo_EX_FAB_US$            = " & valor     'Ultimo_costo_EX_FAB_US$

      end if
      
      if nom_campo7 <> "" then 'Se actualizó EX FAB desde un cálculo de ingreso de cantidades de origen -->Modificacion 20161004
        if strSet <> "" then strSet = strSet & ", "  
        valor_cl = Round(valor7 * paridad,7)
        strSet = strSet & " "&nom_campo7&"                                        = " & valor7    'Costo_EX_FAB_moneda_origen
        strSet = strSet & ", Costo_EX_FAB_US$                                     = " & valor7    'Costo_EX_FAB_US$

        strSet_t_prod     = strSet_t_prod & ", Ultimo_costo_EX_FAB_moneda_origen  = " & valor7    'Ultimo_costo_EX_FAB_moneda_origen
        strSet_t_prod     = strSet_t_prod & ", Ultimo_costo_EX_FAB_US$            = " & valor7    'Ultimo_costo_EX_FAB_US$

      end if
      
      if nom_campo = "costo_fob_us$" then '-->Modificacion 20161004
        '############ FOB ############
        valor_cl = Round(valor * paridad,7)
        strSet = strSet & ", Costo_FOB_moneda_origen                          = " & valor     'Costo_FOB_moneda_origen

        strSet_t_prod     = strSet_t_prod & ", Ultimo_costo_FOB_moneda_origen = " & valor     'Ultimo_costo_FOB_moneda_origen
        strSet_t_prod     = strSet_t_prod & ", Ultimo_costo_FOB_US$           = " & valor     'Ultimo_costo_FOB_US$

      end if
      
      if nom_campo8 <> "" then 'Se actualizó FOB desde un cálculo de ingreso de cantidades de origen -->Modificacion 20161004
        if strSet <> "" then strSet = strSet & ", "  
        valor_cl = Round(valor8 * paridad,7)
        strSet = strSet & " "&nom_campo8&"                                    = " & valor8    'costo_fob_us$
        strSet = strSet & ", Costo_FOB_moneda_origen                          = " & valor8    'Costo_FOB_moneda_origen
        
        strSet_t_prod     = strSet_t_prod & ", Ultimo_costo_FOB_moneda_origen = " & valor8    'Ultimo_costo_FOB_moneda_origen
        strSet_t_prod     = strSet_t_prod & ", Ultimo_costo_FOB_US$           = " & valor8    'Ultimo_costo_FOB_US$
    ...
    if strSet <> "" then strSet = strSet & ", "
    strSet = strSet & "producto=producto "     'Para no gatillar los trigger
    
    if nom_campo <> "precio_de_lista_modificado" then    
      '#############################################################################
      '------------ Actualizar los datos en el MAESTRO DE PRODUCTOS ---------------
      '#############################################################################      
      if nom_campo = "nombre_producto" or nom_campo = "nombre_producto_proveedor" then
        strSQL="update productos set "&strSet_t_prod&" where empresa='SYS' and producto='"&producto&"'"
        set rs = Conn.Execute(strSQL) '--> !!!!!! No se tiene que actualizar la tabla de productos en TCP, sólo cuando se traspase a RCP excepto para los campos ((nombres de productos))
      end if
    end if
  end if
  '#################################################################################
  ...
  '#################################################################################
 if nom_campo = "Porcentaje_ila" then
     strSet = "producto=producto, Porcentaje_ila=" &porcentaje_impuesto_1
 end if
...
''  Response.End
  ...
  '############### DEVOLVER EL TOTAL DE CIF ORI, TOTAL CIF ADU, TOTAL EX FAB Y TOTAL FOB '--> Modificacion 20161004
...
elseif accion = "eliminar_ultimo" then
  ...
  strSQL="Select count(*) as cantidadBultos from bultos b " &_
        "inner join  Bultos_detalle bd on b.BultosID = bd.BultosID " &_
        "where b.numero_interno_documento_no_valorizado ="&numero_interno_documento_no_valorizado&" and " &_
        "bd.numero_de_linea = " & numero_linea
  set rs = Conn.Execute(strSql)

  if cdbl(rs("cantidadBultos")) = 0 then
  ...
  'Response.Write strSQL
    set rs = Conn.Execute(strSQL)
    Response.Write "Item eliminado" 'Existe no vacía (tiene ingresado un producto <> '00000000')
    Response.End
  else
    Response.Write "No se puede eliminar item (registro pesa)" 'Existe no vacía (tiene ingresado un producto <> '00000000')
    Response.End
  end if
elseif accion = "limpiar_items" then
  ...
  for j=0 to Ubound(tot_Num_Lineas)
    ...
    strSQL="update "&nom_tabla&" set Numero_de_linea_en_RCP_o_documento_de_compra_padre=null, producto='00000000', " &_
           ...
           "temporada=null, porcentaje_ila=0, codigo_arancelario=null, fecha_impresion=null, Cantidad_x_caja=1, Peso_x_caja=0, " &_
           "Delta_CPA_US$=0, Obs_delta_CPA_US$=null, proveedor='"&proveedor&"', Producto_proveedor = 0, Proveedor_origen = null, costo_fob_US$ = 0 " &_
           "where empresa='SYS' and " &_
           "documento_no_valorizado='"&documento_no_valorizado&"' and " &_
           "numero_documento_no_valorizado="&numero_documento_no_valorizado&" and " &_
           "Tipo_documento_de_compra='"&documento_respaldo&"' and " &_
...
elseif accion = "agregar" then 'Agregar ítemes después de una línea (item)
  ...
  for i=1 to items
    ...
    strSQL="insert into "&nom_tabla&"(Año_recepcion_compra,Bodega, cantidad_mercancias, " &_
           ...
           "Valor_paridad_moneda_oficial, alto, ancho, largo, peso, Cantidad_x_un_consumo, Cantidad_x_caja, Peso_x_caja, Delta_CPA_US$, Porcentaje_ila) values(" &_
           ""&anio&"," &_
           "'"&bodega&"'," &_
           "0,1,0," &_
           "'"&v_proveedor&"'," &_
           ...
           "'P'," &_
           "'UN','UN', " &_
           ""&paridad&",0,0,0,0,1,1,0,0,"&Porcentaje_ila&")"
''    Response.Write strSQL&"<br><br>"
''    Response.End
    set rs = Conn.Execute(strSQL)
  next
