        ...
        <td>
          ...
          <div name="capaEncListaCarpeta" id="capaEncListaCarpeta" style="width: <%=width-18%>px; border-left: 1px solid #CCC; border-top: 1px solid #CCC; border-right: 1px solid #CCC; overflow: auto; overflow-x:hidden;">          
            <table style="width:<%=width-18%>px;" border=0 cellpadding=0 cellspacing=0">
              ...
              if pend <> "true" then
                if documento_respaldo = "R" then
                  strSQL =  "Select Numero_documento_respaldo, Codigo_postal, documento_no_valorizado, Fecha_recepcion, fecha_emision, entidad_comercial, " &_
                            ...
                            "prov_mop = (select top 1 Codigo_postal from movimientos_productos mop with(nolock), entidades_comerciales ent with(nolock) " &_ 
		                        "where mop.empresa='SYS' and documento_no_valorizado in ('TCP','RCP')   " &_
		                        "and mop.numero_interno_documento_no_valorizado = dnv.numero_interno_documento_no_valorizado " &_
                            ...
                            "where  case when dnv.Proveedor_2 is not null and dnv.Proveedor <> dnv.Proveedor_2 then dnv.Proveedor_2 else  dnv.Proveedor end = ent.entidad_comercial and dnv.empresa='SYS' and documento_no_valorizado in ('TCP','RCP') and YEAR(fecha_emision) >=   " & anio-1 &" " &_  
                  ...
                  strSQL =  "Select Numero_documento_respaldo, Codigo_postal, documento_no_valorizado, Fecha_recepcion, fecha_emision, entidad_comercial, " &_
                            ...
                            "prov_mop = (select top 1 Codigo_postal from movimientos_productos mop with(nolock), entidades_comerciales ent with(nolock) " &_ 
		                        ...
		                        "and mop.numero_documento_de_compra = dnv.Numero_documento_respaldo " &_
		                        "and mop.proveedor = ent.entidad_comercial), " &_
                            "Fisico = (select  isnull(COUNT(*),0) from carpetas_final_detalle cfd with(nolock) where rtrim(ltrim(cfd.documento_respaldo)) + '-' + cast(YEAR(cfd.anio_mes) as varchar(4)) + '-' +cast(Month(cfd.anio_mes) as varchar) + '-' + cast(cfd.num_carpeta as varchar) = dnv.Carpeta and id_tipo_factura = 1 and Numero_aduana = dnv.Numero_documento_respaldo and Fisico = 'S') " &_
                  ...
                  strSQL =  "Select Numero_documento_respaldo, Codigo_postal, documento_no_valorizado, Fecha_recepcion, fecha_emision, entidad_comercial, " &_
                            ...
                            "from DOCUMENTOS_NO_VALORIZADOS dnv with(nolock), entidades_comerciales ent with(nolock) " &_
                            "where  case when dnv.Proveedor_2 is not null and dnv.Proveedor <> dnv.Proveedor_2 then dnv.Proveedor_2 else  dnv.Proveedor end = ent.entidad_comercial and dnv.empresa='SYS' and documento_no_valorizado in ('TCP') and YEAR(fecha_emision) >=   " & anio-1 &" " &_ 
  	                        "and (documento_respaldo='R' or documento_respaldo='DS') " &_
  	                        " and carpeta='" & rs_carpetas("carpeta") & "' ORDER BY Codigo_postal, Numero_documento_respaldo"
                  else
                    strSQL =  "Select Numero_documento_respaldo, Codigo_postal, documento_no_valorizado, Fecha_recepcion, fecha_emision, entidad_comercial, " &_
              ...
              <tr height="15" OnDblClick="Generar_Informe('<%=rs("Numero_documento_respaldo")%>','<%=rs("documento_no_valorizado")%>','<%=documento_respaldo%>','<%=rs("Fecha_recepcion")%>','<%=rs("entidad_comercial")%>','<%=rs("nombre_proveedor")%>','<%=rs("fecha_emision")%>','<%=rs("paridad")%>','<%=rs("total_cif_ori")%>','<%=rs("total_cif_adu")%>','<%=v_carpeta%>')">
                <%if  rs("documento_no_valorizado") = "RCP" then%>
                  <td style="width:50px;" id="grid_TD_1"><%if Cint(rs("Fisico")) > 0 then%>
	        ...
	        strSQL="Select P.Superfamilia , sum(Cantidad_entrada * Costo_CIF_ADU_US$) as total_rubro " &_ 
					"from movimientos_productos MOP with(nolock) inner join Productos P with(nolock) on P.Producto = MOP.producto where MOP.empresa='SYS' " &_
