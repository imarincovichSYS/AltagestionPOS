...
strSQL="select vista_modulo_compras from entidades_comerciales with(nolock) " &_
...
<tr bgcolor="#FFFFFF">
  ...
  <tr id="texto_11" height="30" bgcolor="#DDDDDD">
    ...
    <option value="R">R</option><option value="TU">TU</option><option value="Z">Z</option><option value="DS" selected="true">DS</option>
    ...
    <%end if%>
    id="buscar_label_check_TCP" name="buscar_label_check_TCP">DA</label></td>
    <td style="width:24px;">
    <input 
    <%if perfil = "PEDIDO" then%> disabled <%end if%>
    <%if perfil = "NORMAL" then%>
...
var paridad_mg = "<% = paridad_mg %>";
if (paridad_mg == 0) {alert("El valor moneda para el calculo del marge esta en cero.\nIngreselo en Tesoreria / Valores Moneda.");}
</script>
