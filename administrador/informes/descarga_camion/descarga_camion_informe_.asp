...
function Tiene_Cantidades_Entrada_Descuadradas(x_documento_no_valorizado, x_numero_interno_documento_no_valorizado)
  if x_documento_no_valorizado = "RCP" then
    strSQL="select RCP.numero_interno_movimiento_producto, RCP.numero_de_linea_en_RCP_o_documento_de_compra, RCP.producto, RCP.Nombre_producto, " &_
           ...
           "from detalle_productos_etiquetados  WITH (nolock)  where numero_interno_documento_no_valorizado = RCP.numero_interno_documento_no_valorizado and numero_de_linea = RCP.numero_de_linea) " & _
           "from " &_
           ...
           "from movimientos_productos where documento_no_valorizado='RCP' and numero_interno_documento_no_valorizado = " & x_numero_interno_documento_no_valorizado & ") RCP " &_
    ...
    strSQL="select numero_interno_movimiento_producto, numero_de_linea_en_RCP_o_documento_de_compra, producto, Nombre_producto, " &_
           ...
           "MaxCajas_TotXCaja = (select (convert(varchar(3),count(*)) + '|' + convert(varchar(5),IsNull(sum(cantidad_x_caja),0))) " &_
           "from detalle_productos_etiquetados where Numero_interno_movimiento_producto = Mov.Numero_interno_movimiento_producto) " &_
           "from movimientos_productos Mov where documento_no_valorizado='TCP' and numero_interno_documento_no_valorizado = " & x_numero_interno_documento_no_valorizado & " " &_
...
if Numero_documento_respaldo > 0 then
  ...
  "from documentos_no_valorizados     A (nolock) ," & _
  "     entidades_comerciales         B (nolock) ," & _
  "     detalle_productos_etiquetados c (nolock) " & _
...
<%
  ...
  if documento_no_valorizado = "RCP" then
    strSQL="select RCP.numero_interno_movimiento_producto, RCP.numero_de_linea_en_RCP_o_documento_de_compra, " &_
           ...
           "RCP.cantidad_mercancias, RCP.unidad_de_medida_compra, RCP.cantidad_um_compra_en_caja_envase_compra, " &_
           "RCP.Cantidad_entrada, RCP.unidad_de_medida_consumo, RCP.cantidad_x_un_consumo, " &_
           "MaxCajas_TotXCaja = (select (convert(varchar(4),count(*)) + '|' + convert(varchar(5),IsNull(sum(cantidad_x_caja),0))) " &_
           "from detalle_productos_etiquetados  WITH (nolock)  where numero_interno_documento_no_valorizado = RCP.numero_interno_documento_no_valorizado and numero_de_linea = RCP.numero_de_linea) " & _
           "from " &_
           ...
           "from movimientos_productos where documento_no_valorizado='RCP' and numero_interno_documento_no_valorizado = " & numero_interno_documento_no_valorizado & ") RCP " &_
    ...
    strSQL="select numero_interno_movimiento_producto, numero_de_linea_en_RCP_o_documento_de_compra, " &_
           ...
           "MaxCajas_TotXCaja = (select (convert(varchar(3),count(*)) + '|' + convert(varchar(5),IsNull(sum(cantidad_x_caja),0))) " &_
           "from detalle_productos_etiquetados where Numero_interno_movimiento_producto = Mov.Numero_interno_movimiento_producto) " &_
           "from movimientos_productos Mov where documento_no_valorizado='TCP' and numero_interno_documento_no_valorizado = " & numero_interno_documento_no_valorizado & " " &_
