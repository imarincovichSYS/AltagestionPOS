...
if len(trim( Session( "DataConn_ConnectionString") )) > 0 then
    ...
    if Request("Moneda") = "$" Then
        Titulo = "Detalle Saldo Inicial Contable $"
    	cSql = "Exec DOV_Saldo_inicial_contable_detalle_flujo_caja "
    else
        Titulo = "Detalle Saldo Inicial Contable US$"
    	cSql = "Exec DOV_Saldo_inicial_contable_detalle_flujo_caja_US "
    end if
...
<form name="Listado">
    	<table width="95%" border=0 align=center cellpadding=0 cellspacing=0>
    		<tr height=50 class="FuenteTitulosFunciones">
    			<td colspan=8 align=center NOWRAP >&nbsp;<a href="JavaScript:window.close()"><%=Titulo%></a>&nbsp;</td>
    		</tr>
    	</table>
<%  if Error = "N" Then %>
<%      if Not Rs.Eof then %>
...
<%	Else
    	...
    	<table Width=95% border=0 cellspacing=2 cellpadding=2 >
		    <tr class="FuenteEncabezados">
				<td class="FuenteEncabezados" width=20% align=left><B><%=MsgError%></B></td>
			</tr>
		</table>
<%  End If %>
</form>
