...
Mensajes('');
	...
	function Buscar()
	{
		...
		if ( Error == "N" )
		{
			if ( ! validEmpty(FechaDesde) )
			{
				if ( ! validaFecha(FechaDesde) )
				{
					...
					parent.top.frames[1][0].document.Listado.Fecha_desde.focus();
				}
			}
		}
		...
		if ( Error == "N" )
		{
			if ( ! validEmpty(FechaHasta) )
			{
				if ( ! validaFecha(FechaHasta) )
				{
					...
					parent.top.frames[1][0].document.Listado.Fecha_hasta.focus();
				}
			}
		}
		...
		if ( Error == "N" )		
		{
			if ( Fecha_Universal_2(FechaDesde) > Fecha_Universal_2(FechaHasta) )
			{
				Error = "S";
				Mensajes ( "La fecha hasta no debe ser menor que la fecha desde, revise por favor." );
				parent.top.frames[1][0].document.Listado.Fecha_hasta.focus();
			}
		}
		...
		if ( Error == "N" )		
		{
			if(parent.top.frames[1][0].document.Listado.Salida.value=='I')
			{
				parent.top.frames[1][0].document.Listado.action="Imprimir.asp";
				parent.top.frames[1][0].document.Listado.target="imprimir";
			}
			else if(parent.top.frames[1][0].document.Listado.Salida.value=='P')
			{
				parent.top.frames[1][0].document.Listado.action="List_InfVtas.asp";
				parent.top.frames[1][0].document.Listado.target="Listado";
			}
			else if(parent.top.frames[1][0].document.Listado.Salida.value=='A')
			{
...
<body bgcolor="<%=Session("ColorFondo")%>" leftmargin=0 topmargin=0 text="#000000" background="../../<%=Session("ImagenFondo")%>">
	<Form name="Listado_Botones" method="post" action="Save_InfVtas.asp" target="Trabajo">
