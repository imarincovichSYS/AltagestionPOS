...
<script language="JavaScript">
	...
	function Buscar()
	{
		...
		var error="N";
		var cChar = "'/\|,ç+-" + '"' ;
		if(validEmpty(parent.top.frames[1][0].document.Formulario.Cuenta.value))
		{
			Mensajes("La cuenta contable en un dato requerido");
			error="S";
			parent.top.frames[1][0].document.Formulario.Cuenta.focus();
		}
		if(validEmpty(parent.top.frames[1][0].document.Formulario.Tipo_cambio.value))
		{
			Mensajes("El tipo de cambio en un dato requerido");
			error="S";
			parent.top.frames[1][0].document.Formulario.Tipo_cambio.focus();
		}
		if ( validaCharNoPermitidos(parent.top.frames[1][0].document.Formulario.Centro_de_venta.value) )
		{
			Mensajes ( "El centro de venta tiene carácteres no permitidos (" + cChar + ")." );
			error="S";
			parent.top.frames[1][0].document.Formulario.Centro_de_venta.focus();
		}
		if(!validaDecimales( parent.top.frames[1][0].document.Formulario.Tipo_cambio.value, parent.top.frames[1][0].document.Formulario.Tipo_cambio ))
		{
			Error = "S";
			Mensajes ( "Ingrese solamente datos numericos." );
			parent.top.frames[1][0].document.Formulario.Tipo_cambio.focus();
		}
		if(error == "N")
		{
			parent.top.frames[1][0].document.Formulario.action="javascript:imprimir()";
//			parent.top.frames[1][0].document.Formulario.action="Listado_DifeCambio.asp";
